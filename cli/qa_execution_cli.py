...
def setup_cli_logger(verbose: bool = False) -> logging.Logger:
    ...
    return setup_logger("qa_cli", level=level)
...
def display_qa_report(report: dict, verbose: bool = False) -> None:
    ...
    if verbose:
        ...
        if 'test_generation' in report:
            ...
            print(f"  Test Generation: {gen.get('successful', 0)} successful, {gen.get('failed', 0)} failed")
        
        if 'static_analysis' in report:
            analysis = report['static_analysis']
            print(f"  Static Analysis: {analysis.get('files_analyzed', 0)} files analyzed")
...
def test_langgraph_integration(task_id: str, outputs_dir: str, verbose: bool = False) -> dict:
  ...
  python cli/qa_execution_cli.py --task BE-07 --export report.json
    ...
    try:
        ...
        if args.test_langgraph:
            result = test_langgraph_integration(args.task, str(outputs_path), args.verbose)
