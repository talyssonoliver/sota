...
class EnhancedQAAgent:
    ...
    def generate_comprehensive_tests(self, source_files: List[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for source_file in source_files:
                try:
                    ...
                    self.logger.error(f"Failed to generate test for {source_file}: {e}")
        ...
        for pattern in ["**/*.py", "**/*.js", "**/*.ts", "**/*.jsx", "**/*.tsx"]:
            for file_path in self.project_root.glob(pattern):
                ...
                if not any(skip in str(file_path) for skip in [
                    "test_", "_test", ".test.", "__pycache__", "node_modules", ".git"
                ]):
    ...
    def _generate_test_for_file(self, source_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "status": "success",
                "test_count": test_content.count("def test_") + test_content.count("test(")
            }
    ...
    def _determine_test_framework(self, source_file: str) -> TestFramework:
        ...
        return TestFramework.PYTEST  # Default fallback
    
    def _get_test_file_path(self, source_file: str, framework: TestFramework) -> Path:
    ...
    def _calculate_quality_metrics(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        successful_tests = len([t for t in results["generated_tests"] if t["status"] == "success"])
        
        coverage_data = results.get("coverage_analysis", {})
        
        return {
            "test_generation_success_rate": (successful_tests / total_files * 100) if total_files > 0 else 0,
            "estimated_coverage_improvement": coverage_data.get("improvement_potential", 0),
            "integration_gap_count": len(results.get("integration_gaps", [])),
            "quality_score": self._calculate_overall_quality_score(results),
            "total_generated_tests": sum(t.get("test_count", 0) for t in results["generated_tests"]),
    ...
    def _calculate_overall_quality_score(self, results: Dict[str, Any]) -> float:
        ...
        test_score = results.get("quality_metrics", {}).get("test_generation_success_rate", 0) * 0.4
    ...
    def _generate_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        failed_tests = [t for t in results["generated_tests"] if t["status"] == "error"]
        if failed_tests:
        ...
        coverage_data = results.get("coverage_analysis", {})
        if coverage_data.get("overall_quality_score", 50) < self.config["quality_gates"]["min_test_coverage"]:
            recommendations.append(
    ...
    def validate_quality_gates(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        validations = {
            "coverage_gate": {
                "passed": quality_metrics.get("test_generation_success_rate", 0) >= gates["min_test_coverage"],
                "current": quality_metrics.get("test_generation_success_rate", 0),
                "threshold": gates["min_test_coverage"]
            },
            "integration_gate": {
                "passed": quality_metrics.get("integration_gap_count", 0) <= 10,
                "current": quality_metrics.get("integration_gap_count", 0),
                "threshold": 10
            },
            "overall_quality_gate": {
                "passed": quality_metrics.get("quality_score", 0) >= 75,
                "current": quality_metrics.get("quality_score", 0),
                "threshold": 75
            }
        }
...
def create_enhanced_qa_workflow(project_root: str = ".", config_file: str = None) -> EnhancedQAAgent:
