...
Examples:
  ...
  python orchestration/review_context.py --task BE-07 --export context_review.json
    ...
    parser.add_argument('task_id', nargs='?', help='Task ID to review context for')
    parser.add_argument('--task', help='Task ID (alternative to positional argument)')
    parser.add_argument('--interactive', action='store_true', help='Enable interactive review mode')
    parser.add_argument('--export', help='Export review to specified file')
    parser.add_argument('--summary-only', action='store_true', help='Show summary only, no interaction')
    ...
    try:
        ...
        if args.interactive or (not args.export):
            print(f"\nDo you want to review and modify the context? (y/n): ", end="")
            response = input().strip().lower()
            
            if response in ['y', 'yes']:
                print("[INFO] Interactive review mode not fully implemented in this demo")
        ...
        print(f"[INFO] Final context: {context_data['estimated_tokens']} tokens, {len(context_data['documents'])} documents")
