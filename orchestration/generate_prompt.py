...
from prompts.utils import load_prompt_template, format_prompt_with_context
...
def get_task_context(task_id):
    ...
    try:
        ...
        logger.info(f"Loaded task metadata for {task_id}: {task_metadata.get('title', 'Unknown')}")
        ...
        if 'context_topics' in task_metadata and task_metadata['context_topics']:
            logger.info(f"Building context using topics: {task_metadata['context_topics']}")
...
def generate_prompt(task_id, agent_id, output_path=None):
    ...
    try:
        ...
        raise FileNotFoundError(f"Task metadata file not found: tasks/{task_id}.yaml")
    ...
    template_vars = {
        ...
        "goal": task_metadata.get('description', ''),  # Use description as goal if no specific goal
        ...
        "file_references": ", ".join(task_metadata.get('artefacts', []))  # Use artifacts as file references
    }
...
Examples:
  ...
  python orchestration/generate_prompt.py --task BE-07 --agent backend-agent --verbose
    ...
    parser.add_argument("agent_id", nargs="?", help="Agent ID (e.g. backend-agent)")
    
    # Named arguments (alternative to positional)
    parser.add_argument("--task", "-t", help="Task ID (alternative to positional argument)")
    parser.add_argument("--agent", "-a", help="Agent ID (alternative to positional argument)")
    parser.add_argument("--output", "-o", help="Output file path (optional, defaults to outputs/[task]/prompt_[agent].md)")
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output with detailed logging")
    ...
    if not task_id:
        parser.error("Task ID is required. Use --task or provide as first positional argument.")
    if not agent_id:
        parser.error("Agent ID is required. Use --agent or provide as second positional argument.")
    ...
    try:
        print(f"🔄 Step 4.2: Generating prompt for task {task_id} with agent {agent_id}...")
