...
class CodeExtractor:
    ...
    def extract_from_task_agent(
        ...
        if not output_file:
            raise FileNotFoundError(f"No output file found for task {task_id}, agent {agent_id}")
        ...
        if code_dir.exists() and not force_reextract and any(code_dir.iterdir()):
            print(f"ℹ️  Code already extracted for {task_id}:{agent_id}. Use --force to re-extract.")
            # Return existing extraction info
            return self._get_existing_extraction_info(task_id, agent_id, str(output_file))
    ...
    def _extract_code_blocks(
        ...
        print(f"✅ Extracted {len(extracted_files)} code files from {total_blocks} blocks")
    ...
    def _commit_to_git(self, task_id: str, agent_id: str, extracted_files: List[str]) -> Optional[str]:
        """Commit extracted code files to Git repository."""
        try:
            # Check if we're in a Git repository
            result = subprocess.run(['git', 'rev-parse', '--is-inside-work-tree'], 
                                  capture_output=True, text=True, cwd=self.base_outputs_dir.parent)
            ...
            for file_path in extracted_files:
                subprocess.run(['git', 'add', file_path], cwd=self.base_outputs_dir.parent)
            
            # Create commit message
            commit_msg = f"Extract code from {task_id}:{agent_id} - {len(extracted_files)} files"
    ...
    def _get_existing_extraction_info(self, task_id: str, agent_id: str, source_file: str) -> CodeExtractionResult:
    ...
    def extract_from_all_agents(self, task_id: str, commit_to_git: bool = False) -> Dict[str, CodeExtractionResult]:
    ...
    def batch_extract(self, task_ids: List[str], commit_to_git: bool = False) -> Dict[str, Dict[str, CodeExtractionResult]]:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Extract code blocks from AI agent outputs')
    ...
    parser.add_argument('--commit', action='store_true', help='Commit extracted code to Git')
    parser.add_argument('--force', action='store_true', help='Force re-extraction')
    parser.add_argument('--all-agents', action='store_true', help='Extract from all agents')
