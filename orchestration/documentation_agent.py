...
class DocumentationAgent:
    ...
    def _collect_artifacts(self, task_id: str) -> List[TaskArtifact]:
        ...
        artifact_patterns = {
            "code": ["code/**/*.py", "code/**/*.ts", "code/**/*.js", "code/**/*.tsx", "code/**/*.jsx"],
                    ...
                    if file_path.is_file():
                        try:
                            ...
                            artifacts.append(TaskArtifact(
                                ...
                                description=self._generate_artifact_description(file_path, artifact_type)
                            ))
    ...
    def _generate_implementation_notes(self, task_id: str) -> List[str]:
        ...
        for output_file in output_files:
            try:
                ...
                if "implemented" in content.lower():
                    notes.append(f"Implementation details documented in {output_file.name}")
        ...
        if code_dir.exists():
            ...
            if code_files:
                ...
                if service_files:
                    notes.append(f"Service layer: {', '.join([f.stem for f in service_files])}")
                
                model_files = [f for f in code_files if "model" in f.name.lower()]
                if model_files:
                    notes.append(f"Data models: {', '.join([f.stem for f in model_files])}")
        ...
        try:
            ...
            if pr_data.get("success") and pr_data.get("data"):
                for pr in pr_data["data"]:
                    # Check if PR is related to this task (by title, body, or branch name)
                    ...
                    if (task_id_lower in pr_title or 
                        ...
                        # Also check for common patterns like "be-07" matching "BE-07"
    ...
    def _generate_markdown_report(self, doc_report: DocumentationReport, output_path: Path) -> None:
...
"""
    ...
    def _format_artifacts(self, artifacts: List[TaskArtifact]) -> str:
        ...
        for artifact_type, artifact_list in sorted(by_type.items()):
            ...
            for artifact in artifact_list:
                ...
                formatted.append(f"- **{artifact.name}** ({size_str}) - {artifact.description}")
    ...
    def _generate_artifact_description(self, file_path: Path, artifact_type: str) -> str:
        """Generate description for an artifact"""
        descriptions = {
            "code": f"Source code file implementing {file_path.stem} functionality",
            "documentation": f"Documentation for {file_path.stem}",
            "configuration": f"Configuration file for {file_path.stem}",
            "tests": f"Test file for {file_path.stem}",
            "output": f"Agent output containing implementation details",
            "reports": f"Report file containing {file_path.stem} results"
        }
