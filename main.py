...
from tools.memory_engine import get_relevant_context, memory
...
def run_simple_agent_test():
    ...
    result = agent.invoke(
        {"input": "Use the echo tool to repeat 'Phase 0 setup complete'"})
...
def run_supabase_tool_test():
    ...
    # Run the agent with invoke instead of run
    result = agent.invoke(
        {"input": "Get the database schema for the Artesanato E-commerce project"})
    print("\nResult:", result['output'])
...
def run_memory_test():
    ...
    context = get_relevant_context(
        "What are the RLS policies for products table?")
    print("\nRelevant Context:")
    print(context)
