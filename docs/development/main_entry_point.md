...
- `langchain_community.chat_models.ChatOpenAI`: OpenAI language model interface (legacy).
- `langchain_anthropic.ChatAnthropic`: Claude language model interface (primary).
...
2. Reads API keys from environment:
   ...
   - `CLAUDE_API_KEY` or `ANTHROPIC_API_KEY` (primary)
   If neither is available, prints an error and exits.
...
```python
# Legacy OpenAI implementation (still supported)
llm = ChatOpenAI(temperature=0, model="gpt-4o")  # Updated from gpt-3.5-turbo-16k

# Current implementation uses Claude (Anthropic)
