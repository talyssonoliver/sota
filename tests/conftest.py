...
def fast_test_environment(tmp_path, monkeypatch):
    ...
    with patch('langsmith.client.Client') as mock_langsmith, \
        ...
        mock_langsmith.return_value.multipart_ingest.return_value = {"success": True}
    ...
    class EnhancedWorkflow:
        ...
        def execute_task(self, task_id):
            if self.iteration_count >= self.max_iterations:
                raise Exception(f"Task {task_id} exceeded {self.max_iterations} iterations")
...
def fast_notification_config():
    ...
    with patch('time.sleep'), \
        ...
        mock_notify.return_value = {"success": True, "sent_at": "2025-05-26T10:00:00Z"}
...
def mock_vector_store():
    ...
    store.similarity_search.return_value = [
        {"content": "mock document 1", "metadata": {"score": 0.9, "source": "test"}},
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "expensive: marks tests as expensive (requires RUN_EXPENSIVE_TESTS=true)")
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if any(pattern in item.name for pattern in ['notification_integration', 'resilient_workflow', 'memory_integration']):
