...
Migrated from: tests/integration	est_escalation_system.py
New location: tests/integration	est_escalation_system.py
...
def cleanup_escalation_timers():
    """Automatically clean up any escalation timers after each test"""
    yield
    for engine in _escalation_engines:
        try:
            for timer_id in list(engine.timers.keys()):
                engine.cancel_escalation_timer(timer_id)
        except:
            pass
    _escalation_engines.clear()
...
class TestEscalationRule:
    ...
    def test_escalation_rule_creation(self):
        ...
        rule = EscalationRule(task_type='backend', risk_level='high', trigger_conditions=['timeout', 'rejection'], escalation_path=[EscalationLevel.LEVEL_1, EscalationLevel.LEVEL_2], auto_escalate=True, notification_template='high_risk_escalation')
    ...
    def test_escalation_rule_validation(self):
        ...
        with pytest.raises(ValueError, match='Invalid task type'):
            EscalationRule(task_type='invalid', risk_level='high', trigger_conditions=['timeout'], escalation_path=[EscalationLevel.LEVEL_1])
        with pytest.raises(ValueError, match='Invalid risk level'):
            EscalationRule(task_type='backend', risk_level='invalid', trigger_conditions=['timeout'], escalation_path=[EscalationLevel.LEVEL_1])
...
class TestEscalationEvent:
    ...
    def test_escalation_event_creation(self):
        ...
        event = EscalationEvent(task_id='BE-123', trigger='timeout', level=EscalationLevel.LEVEL_1, triggered_at=datetime.now(), triggered_by='system', context={'review_duration': 120, 'reviewer': 'john.doe'})
    ...
    def test_escalation_event_serialization(self):
        ...
        event = EscalationEvent(task_id='FE-456', trigger='rejection', level=EscalationLevel.LEVEL_2, triggered_at=datetime.now(), triggered_by='reviewer', context={'rejection_reason': 'security_concerns'})
...
class TestEscalationNotifier:
    ...
    def test_send_escalation_notification(self):
        ...
        notifier = EscalationNotifier()
        event = EscalationEvent(task_id='BE-789', trigger='timeout', level=EscalationLevel.LEVEL_1, triggered_at=datetime.now(), triggered_by='system', context={'timeout_duration': 3600})
        recipients = ['team.lead@company.com', 'senior.engineer@company.com']
        channels = ['email', 'slack']
        with patch.object(notifier, '_send_email', return_value=True) as mock_email, patch.object(notifier, '_send_slack', return_value=True) as mock_slack:
    ...
    def test_notification_template_rendering(self):
        ...
        notifier = EscalationNotifier()
        context = {'task_id': 'BE-101', 'trigger': 'timeout', 'timeout_duration': 7200, 'reviewer': 'alice.smith', 'escalation_level': 'Level 2'}
...
class TestEscalationTracker:
    ...
    def test_track_escalation_event(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            tracker = EscalationTracker(storage_path=temp_dir)
            event = EscalationEvent(task_id='QA-202', trigger='rejection', level=EscalationLevel.LEVEL_1, triggered_at=datetime.now(), triggered_by='reviewer', context={'rejection_count': 2})
    ...
    def test_get_escalation_history(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            tracker = EscalationTracker(storage_path=temp_dir)
            for i in range(3):
                event = EscalationEvent(task_id=f'TASK-{i}', trigger='timeout', level=EscalationLevel.LEVEL_1, triggered_at=datetime.now() - timedelta(hours=i), triggered_by='system', context={})
                tracker.track_event(event)
    ...
    def test_escalation_metrics(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            tracker = EscalationTracker(storage_path=temp_dir)
            events = [EscalationEvent('TASK-1', 'timeout', EscalationLevel.LEVEL_1, datetime.now(), 'system', {}), EscalationEvent('TASK-2', 'rejection', EscalationLevel.LEVEL_2, datetime.now(), 'reviewer', {}), EscalationEvent('TASK-3', 'timeout', EscalationLevel.LEVEL_1, datetime.now(), 'system', {})]
            for event in events:
                tracker.track_event(event)
...
class TestEscalationTimer:
    ...
    def test_timer_creation(self):
        ...
        timer = EscalationTimer(task_id='BE-303', timeout_hours=2, callback=lambda: None, level=EscalationLevel.LEVEL_1)
    ...
    def test_timer_expiration(self):
        ...
        timer = EscalationTimer(task_id='BE-404', timeout_hours=1, callback=lambda: None, level=EscalationLevel.LEVEL_1, start_time=datetime.now() - timedelta(hours=2))
        assert timer.is_expired()

    @patch('src.infrastructure.utils.escalation_system.threading.Timer')
    def test_timer_scheduling(self, mock_timer):
        """Test timer scheduling and execution"""
        callback = Mock()
        timer = EscalationTimer(task_id='BE-505', timeout_hours=1, callback=callback, level=EscalationLevel.LEVEL_1)
        timer.start()
...
class TestEscalationPolicy:
    ...
    def test_policy_customization(self):
        ...
        policy = EscalationPolicy()
        custom_rule = EscalationRule(task_type='custom', risk_level='critical', trigger_conditions=['immediate'], escalation_path=[EscalationLevel.LEVEL_4], auto_escalate=True)
...
class TestEscalationEngine:
    ...
    def setup_method(self):
        ...
        self.timers_to_cleanup = []

    def teardown_method(self):
        """Clean up after each test"""
        if hasattr(self, 'engine') and self.engine:
            for timer_id in list(self.engine.timers.keys()):
                try:
                    self.engine.cancel_escalation_timer(timer_id)
                except:
                    pass
        for timer_id in getattr(self, 'timers_to_cleanup', []):
            try:
                if hasattr(self, 'engine') and self.engine and (timer_id in self.engine.timers):
                    self.engine.cancel_escalation_timer(timer_id)
            except:
                pass
    ...
    @patch('src.infrastructure.utils.escalation_system.EscalationNotifier')
    @patch('src.infrastructure.utils.escalation_system.EscalationTracker')
    def test_handle_timeout_escalation(self, mock_tracker, mock_notifier):
        """Test handling timeout-based escalations"""
        engine = EscalationEngine()
        task_data = {'task_id': 'BE-606', 'task_type': 'backend', 'risk_level': 'high', 'reviewer': 'john.doe', 'review_start': datetime.now() - timedelta(hours=3)}
    ...
    def test_handle_rejection_escalation(self, mock_tracker, mock_notifier):
        ...
        engine = EscalationEngine()
        rejection_data = {'task_id': 'FE-707', 'task_type': 'frontend', 'risk_level': 'medium', 'reviewer': 'alice.smith', 'rejection_reason': 'code_quality_issues', 'rejection_count': 2}
    ...
    @patch('src.infrastructure.utils.escalation_system.threading.Timer')
    def test_schedule_escalation_timer(self, mock_timer):
        """Test scheduling escalation timers"""
        engine = EscalationEngine()
        _escalation_engines.append(engine)
        task_data = {'task_id': 'QA-808', 'task_type': 'qa', 'risk_level': 'low', 'timeout_hours': 24}
    ...
    def test_cancel_escalation_timer(self):
        ...
        engine = EscalationEngine()
        task_data = {'task_id': 'UX-909', 'task_type': 'design', 'risk_level': 'low', 'timeout_hours': 12}
        timer_id = engine.schedule_escalation_timer(task_data)
        assert timer_id in engine.timers
...
class TestEscalationIntegration:
    ...
    def test_hitl_integration(self):
        ...
        mock_hitl_instance.get_pending_reviews.return_value = [{'task_id': 'BE-111', 'started_at': datetime.now() - timedelta(hours=3)}, {'task_id': 'FE-222', 'started_at': datetime.now() - timedelta(hours=1)}]
    ...
    def test_feedback_system_integration(self):
        ...
        engine = EscalationEngine()
        escalation_data = {'task_id': 'BE-333', 'escalation_level': EscalationLevel.LEVEL_2, 'escalation_reason': 'timeout'}
    ...
    @patch('src.infrastructure.utils.escalation_system.FeedbackSystem')
    def test_escalation_feedback_capture(self, mock_feedback):
        ...
        engine.feedback_system = mock_feedback_instance
        escalation_event = EscalationEvent(task_id='DATA-444', trigger='timeout', level=EscalationLevel.LEVEL_1, triggered_at=datetime.now(), triggered_by='system', context={'timeout_duration': 3600})
        engine.capture_escalation_feedback(escalation_event)
        mock_feedback_instance.capture_feedback.assert_called_once()
        call_args = mock_feedback_instance.capture_feedback.call_args
        feedback_entry = call_args[0][0]
        assert feedback_entry.task_id == 'DATA-444'
        assert feedback_entry.reviewer == 'system'
        assert feedback_entry.approval_decision is False
        assert feedback_entry.risk_level == 'high'
...
class TestEscalationCLI:
    ...
    def test_cli_manual_escalation(self):
        ...
        result = trigger_manual_escalation(task_id='FE-666', reason='urgent_deadline', level='level_2')
        assert result['success'] is True
...
class TestEscalationEndToEnd:
    ...
    def test_complete_timeout_escalation_workflow(self, mock_tracker, mock_notifier):
        ...
        engine = EscalationEngine()
        task_data = {'task_id': 'BE-777', 'task_type': 'backend', 'risk_level': 'high', 'reviewer': 'john.reviewer', 'review_started': datetime.now() - timedelta(hours=4), 'timeout_threshold': 2}
        timer_id = engine.schedule_escalation_timer(task_data)
        assert timer_id is not None
    ...
    def test_complete_rejection_escalation_workflow(self):
        ...
        engine = EscalationEngine()
        rejection_data = {'task_id': 'FE-888', 'task_type': 'frontend', 'risk_level': 'medium', 'reviewer': 'alice.reviewer', 'rejection_reason': 'design_guidelines', 'rejection_count': 3, 'original_submission': datetime.now() - timedelta(days=2)}
        result = engine.handle_rejection_escalation(rejection_data)
        assert result['escalated'] is True
    ...
    def test_escalation_timer_cascade(self, mock_timer):
        ...
        engine = EscalationEngine()
        level1_data = {'task_id': 'QA-999', 'task_type': 'qa', 'risk_level': 'high', 'timeout_hours': 2}
        ...
        try:
            if timer1 and timer1 in engine.timers:
                engine.cancel_escalation_timer(timer1)
            if timer2 and timer2 in engine.timers:
                engine.cancel_escalation_timer(timer2)
        except:
            pass
