...
class TestEnhancedQAAgent:
    ...
    def test_load_config_with_file(self, temp_project):
        ...
        config_data = {'coverage_thresholds': {'line_coverage': 95}, 'quality_gates': {'min_test_coverage': 90}}
    ...
    def test_get_test_file_path_pytest(self, qa_agent):
        ...
        test_path = qa_agent._get_test_file_path('src/sample.py', TestFrameworkEnum.PYTEST.value)
    ...
    def test_get_test_file_path_jest(self, qa_agent):
        ...
        test_path = qa_agent._get_test_file_path('src/sample.js', TestFrameworkEnum.JEST.value)
    ...
    def test_generate_comprehensive_tests(self, mock_integration, mock_coverage, temp_project):
        """Test comprehensive test generation."""
        mock_coverage_instance = Mock()
        mock_coverage_instance.analyze_coverage_patterns.return_value = {'analysis': {'overall_quality_score': 75}, 'status': 'COMPLETED'}
        mock_coverage_instance.analyze_coverage.return_value = {'line_coverage': 85.0, 'branch_coverage': 75.0}
        mock_coverage.return_value = mock_coverage_instance
        mock_integration_instance = Mock()
        mock_integration_instance.analyze_project.return_value = {'gaps': ['missing_interface_test', 'unvalidated_data_flow']}
        ...
        with patch.object(qa_agent, '_generate_test_for_file') as mock_gen:
            mock_gen.return_value = {'source_file': 'sample.py', 'test_file': 'test_sample.py', 'framework': 'pytest', 'status': 'success', 'test_count': 5}
    ...
    def test_calculate_quality_metrics(self, qa_agent):
        ...
        test_results = {'generated_tests': [{'status': 'success', 'test_count': 5}, {'status': 'success', 'test_count': 3}, {'status': 'error', 'test_count': 0}], 'coverage_analysis': {'overall_quality_score': 80, 'improvement_potential': 20}, 'integration_gaps': ['gap1', 'gap2']}
    ...
    def test_calculate_overall_quality_score(self, qa_agent):
        ...
        results = {'quality_metrics': {'test_generation_success_rate': 80}, 'coverage_analysis': {'overall_quality_score': 75}, 'integration_gaps': ['gap1', 'gap2', 'gap3']}
    ...
    def test_generate_recommendations(self, qa_agent):
        ...
        results = {'generated_tests': [{'status': 'success'}, {'status': 'error', 'error': 'Parse error'}], 'coverage_analysis': {'overall_quality_score': 60}, 'integration_gaps': ['gap1', 'gap2', 'gap3', 'gap4', 'gap5', 'gap6'], 'quality_metrics': {'quality_score': 65}}
    ...
    def test_end_to_end_test_generation(self, mock_generate, temp_project):
        ...
        with patch.object(qa_agent.coverage_analyzer, 'analyze_coverage_patterns') as mock_coverage:
            with patch.object(qa_agent.integration_analyzer, 'analyze_project') as mock_integration:
                mock_coverage.return_value = {'overall_quality_score': 85, 'improvement_potential': 15, 'patterns': []}
