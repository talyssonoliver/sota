...
class TestPhase4FinalValidation(unittest.TestCase):
    ...
    def _create_test_data(self):
        ...
        status_data = {
            "task_id": "BE-07",
            "status": "IN_PROGRESS",
            "last_updated": "2025-06-11T02:00:00Z",
            "progress": 75,
            "completion_report": str(be07_dir / "completion_report.md"),
        }
    ...
    def test_success_criteria_1_tasks_registered(self):
        ...
        self.assertGreater(
            len(declaration_files), 0, "Should have at least one task declaration"
        )
        be07_dir = self.outputs_dir / "BE-07"
        if be07_dir.exists():
            be07_declaration = be07_dir / "task_declaration.json"
            self.assertTrue(
                be07_declaration.exists(), "BE-07 task declaration should exist"
            )
    ...
    def test_success_criteria_3_langgraph_workflow(self):
        ...
        self.assertTrue(
            execute_graph.exists(), "execute_graph.py should exist in new location"
        )
        graph_files = [
    ...
    def test_success_criteria_4_agent_output_processing(self):
        ...
        self.assertTrue(
            register_output.exists(), "register_output.py should exist in new location"
        )
        self.assertTrue(
            extract_code.exists(), "extract_code.py should exist in new location"
        )
    ...
    def test_success_criteria_6_reports_summaries(self):
        ...
        if be07_report.exists():
            ...
            self.assertGreater(
                len(content), 100, "Report should have substantial content"
            )
    ...
    def test_core_functionality_integration(self):
        ...
        try:
            from src.core.workflows.extract_code import CodeExtractor  # noqa: F401
            from src.core.workflows.register_output import AgentOutputRegistry  # noqa: F401
            from src.core.workflows.summarise_task import TaskSummarizer  # noqa: F401
            from src.core.workflows.task_declaration import TaskDeclarationManager  # noqa: F401
