...
def test_individual_function_performance():
    ...
    result, exec_time = measure_execution_time(_calculate_sprint_velocity, progress_generator, target_date)
    print(f'      ⏱️  Execution time: {exec_time:.3f} seconds')
    print(f'      ✅ Result keys: {list(result.keys())}')
    print("   📅 Testing tomorrow's preparation analysis...")
    result, exec_time = measure_execution_time(_analyze_tomorrow_preparation, progress_generator, target_date)
    print(f'      ⏱️  Execution time: {exec_time:.3f} seconds')
    print(f'      ✅ Planned tasks: {result.get('total_planned', 0)}')
    print('   🏥 Testing sprint health assessment...')
    result, exec_time = measure_execution_time(_assess_sprint_health, progress_generator)
    print(f'      ⏱️  Execution time: {exec_time:.3f} seconds')
    ...
    result, exec_time = measure_execution_time(_generate_visual_progress_summary, progress_generator, velocity_data)
    print(f'      ⏱️  Execution time: {exec_time:.3f} seconds')
...
async def test_daily_cycle_integration_performance():
    ...
    result, exec_time = await measure_async_execution_time(orchestrator.run_end_of_day_report)
...
def test_concurrent_report_generation():
    ...
    def generate_report_thread(day, result_queue):
        ...
        try:
            ...
            end_time = time.time()
            result_queue.put({'day': day, 'success': result, 'time': end_time - start_time})
        except Exception as e:
