...
class TestIntegratedWorkflowExecution(unittest.TestCase):
    ...
    def test_workflow_with_task_dependencies(self, mock_build_workflow):
        ...
        mock_workflow.invoke.return_value = {'task_id': 'BE-07', 'status': TaskStatus.DOCUMENTATION, 'dependencies': ['TL-09', 'BE-01'], 'agent': 'backend', 'output': 'Task completed with all dependencies satisfied', 'timestamp': datetime.now().isoformat()}
        mock_build_workflow.return_value = mock_workflow
        executor = EnhancedWorkflowExecutor(workflow_type='basic', notification_level='none', output_dir=str(self.test_output_dir))
    ...
    def test_error_handling_in_workflow(self, mock_load_metadata):
        ...
        mock_load_metadata.return_value = {'id': 'BE-07', 'title': 'Error Test Task', 'description': 'Test error handling in workflow'}
        executor = EnhancedWorkflowExecutor(workflow_type='basic', notification_level='none', output_dir=str(self.test_output_dir))
    ...
    def test_auto_generated_workflow_with_dependencies(self, mock_load_metadata, mock_build_workflow):
        ...
        mock_load_metadata.return_value = {'id': 'BE-07', 'title': 'Implement Service Functions', 'description': 'Add the missing service functions', 'depends_on': ['BE-01', 'TL-09']}
        mock_workflow = MagicMock()
        call_count = {'count': 0}
        ...
        def mock_invoke_side_effect(state):
            ...
            if task_id == 'BE-07':
                if call_count['count'] == 1:
                    return {'task_id': task_id, 'status': TaskStatus.QA_PENDING, 'dependencies': ['BE-01', 'TL-09'], 'agent': 'backend', 'output': f'Task {task_id} completed with dependencies resolved', 'timestamp': datetime.now().isoformat()}
                else:
                    return {'task_id': task_id, 'status': TaskStatus.DONE, 'dependencies': ['BE-01', 'TL-09'], 'agent': 'backend', 'output': f'Task {task_id} is done', 'timestamp': datetime.now().isoformat()}
            return state
        mock_workflow.invoke.side_effect = mock_invoke_side_effect
        mock_build_workflow.return_value = mock_workflow
        executor = EnhancedWorkflowExecutor(workflow_type='auto', notification_level='none', output_dir=str(self.test_output_dir))
        executor = ensure_check_recursion_method(executor)
        with patch.object(executor, 'check_dependencies', return_value=(True, 'All dependencies satisfied')):
            result = executor.execute_task('BE-07')
        self.assertEqual(result['task_id'], 'BE-07')
    ...
    def test_resilient_workflow_retry_logic(self, mock_build_workflow):
        ...
        def mock_invoke_with_retry(state):
            ...
            if invoke_count <= 2:
                ...
                return {'task_id': state['task_id'], 'status': TaskStatus.DOCUMENTATION, 'agent': 'qa', 'output': 'Task completed after retries', 'timestamp': datetime.now().isoformat(), 'retry_count': invoke_count - 1}
        mock_workflow.invoke.side_effect = mock_invoke_with_retry
        mock_build_workflow.return_value = mock_workflow
        executor = EnhancedWorkflowExecutor(workflow_type='dynamic', resilience_config={'max_retries': 3, 'retry_delay': 0.1, 'timeout_seconds': 5}, notification_level='none', output_dir=str(self.test_output_dir))
    ...
    def test_dynamic_routing_based_on_status(self, mock_build_workflow):
        ...
        test_cases = [{'initial_status': TaskStatus.QA_PENDING, 'result_status': TaskStatus.BLOCKED, 'expected_next_agent': 'coordinator', 'task_id': 'BE-07'}, {'initial_status': TaskStatus.QA_PENDING, 'result_status': TaskStatus.DOCUMENTATION, 'expected_next_agent': 'documentation', 'task_id': 'BE-07'}, {'initial_status': TaskStatus.IN_PROGRESS, 'result_status': TaskStatus.HUMAN_REVIEW, 'expected_next_agent': 'human_review', 'task_id': 'BE-14'}]
        for idx, test_case in enumerate(test_cases):
            mock_workflow = MagicMock()
            mock_workflow.invoke.return_value = {'task_id': test_case['task_id'], 'status': test_case['result_status'], 'agent': 'current_agent', 'next_agent': test_case['expected_next_agent'], 'output': f'Task routed to {test_case['expected_next_agent']}', 'timestamp': datetime.now().isoformat()}
            mock_build_workflow.return_value = mock_workflow
            executor = EnhancedWorkflowExecutor(workflow_type='dynamic', notification_level='none', output_dir=str(self.test_output_dir))
            initial_state = {'task_id': test_case['task_id'], 'status': test_case['initial_status'], 'message': f'Test dynamic routing case {idx}'}
            with patch.object(executor, 'execute_task', wraps=lambda task_id, initial_state=None: mock_workflow.invoke(initial_state if initial_state else {'task_id': task_id, 'status': TaskStatus.CREATED})):
