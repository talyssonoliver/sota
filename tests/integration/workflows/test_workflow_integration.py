...
# Temporarily comment out problematic import to isolate the issue
...
from tests.run_tests import Timer
...
class TestIntegratedWorkflowExecution(unittest.TestCase):
    ...
    def test_workflow_with_task_dependencies(self, mock_build_workflow):
        ...
        mock_build_workflow.return_value = mock_workflow        # Create executor with mocked workflow
        executor = EnhancedWorkflowExecutor(
            workflow_type="basic",
            notification_level="none",  # Temporarily using string instead of enum
        ...
        # This allows the anti-freeze logic in ensure_check_recursion_method to
        # work
        with patch.object(executor, 'check_dependencies', return_value=(True, "All dependencies satisfied")):
            ...
            self.assertIn(result["status"], [
                          TaskStatus.DOCUMENTATION, TaskStatus.BLOCKED])
            # The workflow may be invoked multiple times due to iteration, just
        ...
        # Create a patched version of execute_task that mocks the
        ...
        with patch.object(executor, 'check_dependencies', return_value=(False, "Missing dependency: TL-09 is not DONE")):
            ...
            with patch.object(executor, 'execute_task', wraps=executor.execute_task) as patched_execute:
                # Call the function
                result = executor.execute_task("BE-07")
                # If anti-freeze protection triggers, allow BLOCKED with
                # recursion error
                if result["status"] == TaskStatus.BLOCKED and "Recursion/iteration limit" in str(
                        result.get("error", "")):
    ...
    @patch('src.platform.utils.task_loader.load_task_metadata')
    def test_error_handling_in_workflow(self, mock_load_metadata):
        ...
        mock_load_metadata.return_value = {
            "id": "BE-07",
            "title": "Error Test Task",            "description": "Test error handling in workflow"
        }
        ...
        # Override the _build_workflow with a direct mock that will throw a
        # specific exception
    ...
    def test_auto_generated_workflow_with_dependencies(
        ...
        mock_build_workflow.return_value = mock_workflow        # Create executor with patched check_dependencies
        executor = EnhancedWorkflowExecutor(
            workflow_type="auto",
            notification_level="none",  # Temporarily using string instead of enum
            output_dir=str(self.test_output_dir)
    ...
    def test_resilient_workflow_retry_logic(self, mock_build_workflow):
        ...
        mock_build_workflow.return_value = mock_workflow        # Create executor with retry configuration
        executor = EnhancedWorkflowExecutor(
    ...
    def test_dynamic_routing_based_on_status(self, mock_build_workflow):
        ...
        test_cases = [
            ...
            # QA success -> DOCUMENTATION status should route to documentation
            # agent
            {
                "initial_status": TaskStatus.QA_PENDING,
        ...
        for idx, test_case in enumerate(test_cases):
            ...
            mock_build_workflow.return_value = mock_workflow            # Create executor
            executor = EnhancedWorkflowExecutor(
                workflow_type="dynamic",
                notification_level="none",  # Temporarily using string instead of enum
                output_dir=str(self.test_output_dir)
            ...
            # Execute task with dynamic workflow - specify an initial_state
            # parameter
            with patch.object(executor, 'execute_task', wraps=lambda task_id, initial_state=None:
                ...
                result = executor.execute_task(
                    test_case["task_id"], initial_state)
