...
from src.core.agents.factory import AGENT_CONFIGS, create_agent
...
class TestAgentFactoryIntegration(unittest.TestCase):
    ...
    def test_end_to_end_agent_creation_and_initialization(self):
        ...
        with patch(
            ...
            # Verify agent is created and properly configured
    ...
    def test_agent_to_workflow_handoff(self):
        ...
        with patch(
            ...
            with patch(
                "src.core.workflows.execute_workflow.execute_task"
    ...
    def test_agent_context_injection_from_memory(self):
        ...
        tl_agent = create_agent(
            "technical_lead", context_domains=["architecture", "project_management"]
        )

        # Verify agent was created and context domains used
        self.assertIsNotNone(tl_agent)
        # Use test_contexts in test validation
        for domain, context in test_contexts.items():
            self.assertIsInstance(context, dict)
            self.assertGreater(len(context), 0)

        # Create backend agent with context domains
        be_agent = create_agent(
            "backend", context_domains=["backend", "api", "database"]
        )
    ...
    def test_agent_tool_loading_integration(self):
        ...
        with patch(
            "src.infrastructure.tools.core.tool_loader.load_tools_for_agent",
            ...
            with patch(
                ...
                # Add assertions for created agents with tools
                self.assertIsNotNone(tl_agent)
                self.assertIsNotNone(be_agent)
    ...
    def test_error_propagation_during_agent_creation(self):
        ...
        with patch(
            "src.core.agents.factory.Agent", side_effect=Exception("Memory init failed")
        ):
    ...
    def test_concurrent_agent_creation(self):
        ...
        def create_agent_thread(agent_type):
            ...
            try:
                with patch(
                    "src.core.agents.factory.MemoryEngine",
        ...
        for agent_type in agent_types:
            if agent_type in AGENT_CONFIGS:
                thread = threading.Thread(
                    target=create_agent_thread, args=(agent_type,)
                )
    ...
    def test_agent_configuration_validation(self):
        ...
        with patch.dict(
            "src.core.agents.factory.AGENT_CONFIGS", {"test_agent": invalid_config}
        ):
    ...
    def test_performance_agent_creation(self):
        ...
        with patch(
            ...
            self.assertLess(
                creation_time, 0.1, f"Agent creation took {creation_time:.3f}s"
            )
