"""
import sys
test_api_integration_webhooks.py - Optimized Test Structure

Migrated from: tests/integration	est_api_integration_webhooks.py
New location: tests/integration\x07pi	est_api_integration_webhooks.py
...
from src.interfaces.api.external_integrations import GitHubIntegration, SlackIntegration, JIRAIntegration, ExternalSystemConfig, ExternalSystemType
...
class TestWebhookManager:
    ...
    def test_webhook_registration(self):
        ...
        endpoint = WebhookEndpoint(id='test_webhook_1', name='Test Webhook', url='https://example.com/webhook', secret='test_secret', events=[WebhookEventType.REVIEW_CREATED, WebhookEventType.REVIEW_APPROVED])
    ...
    def test_webhook_unregistration(self):
        ...
        endpoint = WebhookEndpoint(id='test_webhook_1', name='Test Webhook', url='https://example.com/webhook', secret='test_secret', events=[WebhookEventType.REVIEW_CREATED])
        success = self.webhook_manager.register_endpoint(endpoint)
        assert success is True
    ...
    def test_webhook_event_creation(self):
        ...
        event_data = {'review_id': 'test_review_123', 'status': 'approved', 'reviewer': 'user@example.com'}
        event = WebhookEventData.review_approved(task_id='test_task_123', checkpoint_id='test_checkpoint_456', approval_data=event_data)
    ...
    def test_webhook_delivery_basic(self):
        ...
        endpoint = WebhookEndpoint(id='test_webhook', name='Test Webhook', url='https://example.com/webhook', secret='test_secret', events=[WebhookEventType.REVIEW_APPROVED])
        self.webhook_manager.register_endpoint(endpoint)
        event_data = {'review_id': 'test_123', 'status': 'approved'}
        event = WebhookEventData.review_approved(task_id='test_task', checkpoint_id='test_checkpoint', approval_data=event_data)
        try:
    ...
    def test_webhook_filtering_by_events(self):
        ...
        endpoint1 = WebhookEndpoint(id='webhook_1', name='Webhook 1', url='https://example1.com/webhook', secret='secret1', events=[WebhookEventType.REVIEW_CREATED, WebhookEventType.REVIEW_APPROVED])
        endpoint2 = WebhookEndpoint(id='webhook_2', name='Webhook 2', url='https://example2.com/webhook', secret='secret2', events=[WebhookEventType.REVIEW_REJECTED])
...
class TestExternalAPIManager:
    ...
    def test_integration_registration(self):
        ...
        self.api_manager.list_available_integrations.return_value = {'test': {'name': 'test_integration'}}
    ...
    def test_request_lifecycle_tracking(self):
        ...
        self.api_manager.get_request_status.side_effect = [{'status': 'pending', 'integration': 'github', 'request_type': 'pr_review'}, {'status': 'completed', 'integration': 'github', 'request_type': 'pr_review', 'result': {'result': 'success'}}]
        request_id = self.api_manager.create_request(integration_name='github', request_type='pr_review', metadata={'pr_number': 123})
...
class TestGitHubIntegration:
    ...
    def setup_method(self):
        """Setup for each test method"""
        config = ExternalSystemConfig(system_id='github_test', system_type=ExternalSystemType.GITHUB, name='GitHub Test', base_url='https://api.github.com', auth_config={'token': 'fake_token'})
...
class TestSlackIntegration:
    ...
    def setup_method(self):
        """Setup for each test method"""
        config = ExternalSystemConfig(system_id='slack_test', system_type=ExternalSystemType.SLACK, name='Slack Test', base_url='https://slack.com/api', auth_config={'token': 'fake_token'})
...
class TestJIRAIntegration:
    ...
    def setup_method(self):
        """Setup for each test method"""
        config = ExternalSystemConfig(system_id='jira_test', system_type=ExternalSystemType.JIRA, name='JIRA Test', base_url='https://test.atlassian.net', auth_config={'username': 'test@example.com', 'api_token': 'fake_token'})
