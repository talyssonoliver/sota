"""
import sys
test_dashboard_integration.py - Optimized Test Structure

Migrated from: tests/integration	est_dashboard_integration.py
New location: tests/integration\\dashboard	est_dashboard_integration.py
...
def tearDown(self):
    ...
    def test_status_breakdown_calculation(self):
        ...
        task_metrics = [{'task_id': 'T1', 'status': 'COMPLETED'}, {'task_id': 'T2', 'status': 'COMPLETED'}, {'task_id': 'T3', 'status': 'IN_PROGRESS'}, {'task_id': 'T4', 'status': 'FAILED'}, {'task_id': 'T5', 'status': 'CREATED'}]
    ...
    def test_completion_chart_data_generation(self):
        ...
        metrics = {'team_metrics': {'total_tasks': 10, 'completed_tasks': 6, 'in_progress_tasks': 2, 'failed_tasks': 1}}
        ...
        self.assertEqual(chart_data['data']['datasets'][0]['data'], [6, 2, 1, 1])

    def test_qa_chart_data_generation(self):
        """Test QA chart data generation"""
        metrics = {'task_metrics': [{'task_id': 'T1', 'qa_status': 'PASSED'}, {'task_id': 'T2', 'qa_status': 'PASSED'}, {'task_id': 'T3', 'qa_status': 'FAILED'}, {'task_id': 'T4', 'qa_status': 'NOT_RUN'}, {'task_id': 'T5', 'qa_status': None}]}
        chart_data = self.updater._create_qa_chart_data(metrics)
        self.assertEqual(chart_data['type'], 'bar')
        self.assertEqual(chart_data['data']['datasets'][0]['data'], [2, 1, 2])

    def test_coverage_chart_data_generation(self):
        """Test coverage chart data generation"""
        metrics = {'progress_metrics': {'coverage_trend': [['2025-05-25', 85.5], ['2025-05-26', 87.2], ['2025-05-27', 89.1]]}}
    ...
    def test_progress_chart_data_generation(self):
        ...
        metrics = {'progress_metrics': {'completion_trend': [['2025-05-25', 3], ['2025-05-26', 2], ['2025-05-27', 4]]}, 'team_metrics': {'completion_rate': 75.0, 'total_tasks': 20, 'qa_pass_rate': 90.0, 'average_coverage': 85.5}}
...
class TestProgressReportGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        (task_dir / 'status.json').write_text(json.dumps({'task_id': 'TEST-01', 'agent_type': 'backend', 'status': 'COMPLETED', 'completion_time': '2025-05-28T10:00:00Z', 'qa_status': 'PASSED', 'coverage': 85, 'tests_passed': 10, 'tests_failed': 0, 'duration_minutes': 45}))
    ...
    def test_qa_insights_generation(self):
        ...
        tasks = [{'task_id': 'T1', 'qa_status': 'PASSED'}, {'task_id': 'T2', 'qa_status': 'PASSED'}, {'task_id': 'T3', 'qa_status': 'FAILED'}]
        insights = self.generator._add_qa_insights(tasks)
        self.assertIn('QA Insights', insights)
        self.assertIn('66.7%', insights)
        self.assertIn('Failed QA Tasks', insights)
        self.assertIn('T3', insights)

    def test_next_steps_generation(self):
        """Test next steps generation"""
        metrics = {'team_metrics': {'completion_rate': 45.0, 'in_progress_tasks': 3, 'failed_tasks': 1, 'qa_pass_rate': 85.0, 'average_coverage': 75.0}}
        next_steps = self.generator._add_next_steps(metrics)
        self.assertIn('Next Steps', next_steps)
        self.assertIn('Maintain momentum', next_steps)
        self.assertIn('Address failures', next_steps)
        self.assertIn('Monitor progress', next_steps)
        self.assertIn('Increase test coverage', next_steps)

    def test_task_recommendations_generation(self):
        """Test task recommendations generation"""
        task_data = {'qa_status': 'FAILED', 'coverage': 60, 'documentation_generated': False, 'archived': False, 'status': 'COMPLETED'}
    ...
    def test_agent_breakdown_generation(self):
        ...
        task_metrics = [{'agent_type': 'backend', 'status': 'COMPLETED', 'qa_status': 'PASSED'}, {'agent_type': 'backend', 'status': 'COMPLETED', 'qa_status': 'FAILED'}, {'agent_type': 'frontend', 'status': 'COMPLETED', 'qa_status': 'PASSED'}, {'agent_type': 'frontend', 'status': 'IN_PROGRESS', 'qa_status': 'NOT_RUN'}]
        ...
        self.assertIn('100.0%', breakdown)
        self.assertIn('50.0%', breakdown)

    def test_project_recommendations_generation(self):
        """Test project recommendations generation"""
        metrics = {'team_metrics': {'completion_rate': 30.0, 'qa_pass_rate': 85.0, 'average_coverage': 70.0, 'failed_tasks': 2}}
        recommendations = self.generator._add_project_recommendations(metrics)
        self.assertIn('Project Recommendations', recommendations)
        self.assertIn('Focus on task completion velocity', recommendations)
        self.assertIn('Implement stricter test coverage', recommendations)
        self.assertIn('Review and address failed tasks', recommendations)

    def test_weekly_insights_generation(self):
        """Test weekly insights generation"""
        weekly_tasks = [{'qa_status': 'PASSED', 'coverage': 80}, {'qa_status': 'PASSED', 'coverage': 90}, {'qa_status': 'FAILED', 'coverage': 60}]
        metrics = {'team_metrics': {'completion_rate': 75.0}}
...
class TestIntegration(unittest.TestCase):
    ...
    @patch('src.infrastructure.utils.completion_metrics.CompletionMetricsCalculator.calculate_all_metrics')
    def test_dashboard_update_integration(self, mock_calculate):
        """Test complete dashboard update workflow"""
        mock_calculate.return_value = {'team_metrics': {'total_tasks': 10, 'completed_tasks': 7, 'in_progress_tasks': 2, 'failed_tasks': 1, 'completion_rate': 70.0, 'qa_pass_rate': 85.7, 'average_coverage': 82.3}, 'progress_metrics': {'completion_trend': [['2025-05-27', 3]], 'coverage_trend': [['2025-05-27', 82.3]]}, 'task_metrics': [{'task_id': 'T1', 'qa_status': 'PASSED'}, {'task_id': 'T2', 'qa_status': 'FAILED'}]}
        updater = DashboardUpdater(str(self.dashboard_dir))
