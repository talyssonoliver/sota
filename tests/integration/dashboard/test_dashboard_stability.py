"""
import sys
test_dashboard_stability.py - Optimized Test Structure

Migrated from: tests/integration	est_dashboard_stability.py
New location: tests/integration\\dashboard	est_dashboard_stability.py
...
class DashboardStabilityTester:
    ...
    def __init__(self):
        ...
        self.test_results = {'api_tests': {}, 'performance_tests': {}, 'stability_tests': {}, 'errors': []}

    def test_api_endpoints(self) -> bool:
        """Test all dashboard API endpoints."""
        print('🔍 Testing API endpoints...')
        endpoints = ['/health', '/api/metrics', '/api/sprint/health', '/api/automation/status', '/api/tasks/recent?limit=5', '/api/progress/trend?days=7', '/api/briefing/latest']
        all_passed = True
        for endpoint in endpoints:
            try:
                ...
                if response.status_code == 200:
                    data = response.json()
                    self.test_results['api_tests'][endpoint] = {'status': 'PASS', 'response_time': round(response_time, 3), 'data_size': len(json.dumps(data))}
                    print(f'  ✅ {endpoint} - {response.status_code} ({response_time:.3f}s)')
                else:
                    self.test_results['api_tests'][endpoint] = {'status': 'FAIL', 'error': f'HTTP {response.status_code}'}
                    print(f'  ❌ {endpoint} - {response.status_code}')
                    all_passed = False
            except Exception as e:
                self.test_results['api_tests'][endpoint] = {'status': 'ERROR', 'error': str(e)}
                print(f'  💥 {endpoint} - {e}')
                all_passed = False
        return all_passed
    ...
    def test_api_performance(self) -> bool:
        ...
        if results:
            ...
            self.test_results['performance_tests'] = {'total_requests': len(results), 'total_time': round(total_time, 3), 'avg_response_time': round(avg_response_time, 3), 'max_response_time': round(max_response_time, 3), 'min_response_time': round(min_response_time, 3), 'requests_per_second': round(len(results) / total_time, 2), 'error_count': len(errors)}
    ...
    def test_memory_usage(self) -> bool:
        ...
        self.test_results['stability_tests']['memory'] = {'initial_memory_mb': round(initial_memory, 1), 'final_memory_mb': round(final_memory, 1), 'memory_increase_mb': round(memory_increase, 1)}
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        print('=' * 50)
        tests = [('API Endpoints', self.test_api_endpoints), ('API Performance', self.test_api_performance), ('Memory Usage', self.test_memory_usage), ('Chart Data Integrity', self.test_chart_data_integrity), ('Error Handling', self.test_error_handling)]
        results = {}
        overall_pass = True
        ...
        self.test_results['summary'] = {'overall_status': 'PASS' if overall_pass else 'FAIL', 'individual_tests': results, 'timestamp': time.strftime('%Y-%m-%d %H:%M:%S')}
