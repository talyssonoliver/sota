...
class TestHITLEngineIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        def mock_create_checkpoint(
            ...
            checkpoint.risk_level = (
                RiskLevel.HIGH
                if "security_config" in (risk_factors or [])
            ...
            if "simple_crud" in (risk_factors or []):
                ...
                )  # Frontend styling is auto-approved
            elif "production_deployment" in (
                risk_factors or []
            ) or "database_config" in (risk_factors or []):
                checkpoint.risk_level = RiskLevel.HIGH

            # Auto-approve disabled checkpoint types
            if (
                checkpoint_type == "documentation"
                and not self.test_config["checkpoint_triggers"]["documentation"][
                    "enabled"
                ]
            ):
                checkpoint.status = CheckpointStatus.APPROVED
        ...
        self.engine.get_pending_checkpoints_for_task = lambda task_id: [
            cp
            for cp in self.engine.checkpoints.values()
            if cp.task_id == task_id and cp.status == CheckpointStatus.PENDING
        ]
        self.engine.get_pending_checkpoints = lambda: [
            cp
            for cp in self.engine.checkpoints.values()
            if cp.status == CheckpointStatus.PENDING
        ]
        self.engine.get_checkpoint = lambda checkpoint_id: self.engine.checkpoints.get(
            checkpoint_id
        )
        ...
        self.engine._get_escalation_policy = lambda risk_level: self.test_config[
            "escalation_policies"
        ].get(
            "high_risk",
            {
    ...
    async def test_notification_template_rendering(self):
        ...
        rendered_subject = template["subject"].format(
            task_id=checkpoint.task_id, checkpoint_type=checkpoint.checkpoint_type
        )
