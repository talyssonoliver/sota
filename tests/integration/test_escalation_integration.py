    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        notification_config = {
            ...
            "priorities": {
                ...
                "high": {"channels": ["slack", "dashboard"], "timeout_minutes": 60},
                "medium": {"channels": ["dashboard"], "timeout_minutes": 120},
            },
        }
...
class TestEscalationEngine(unittest.IsolatedAsyncioTestCase):
    ...
    def test_complete_escalation_workflow(self):
        ...
        if hasattr(self.escalation_system.tracker, "get_events_for_task"):
            existing_events = self.escalation_system.tracker.get_events_for_task(
                "TEST-01"
            )
            self.assertEqual(
                len(existing_events),
                0,
                f"Test isolation failed: found {len(existing_events)} existing events",
            )
        ...
        with patch.object(
            self.escalation_system.notifier, "send_notification"
        ) as mock_send:
        ...
        escalation_event.resolution = (
            "Increased DB connection pool and restarted services"
        )
        ...
        self.assertEqual(
            resolved.resolution, "Increased DB connection pool and restarted services"
        )
    ...
    async def test_escalation_timeout_and_retry(self):
        ...
        escalation_event = EscalationEvent(
            ...
            triggered_by="system",
            context={"timeout_duration": 1, "reviewer": "test_reviewer"},
        )
        self.escalation_system.tracker.track_event(escalation_event)

        # Mock the handle_timeout_escalation method
        with patch.object(
            self.escalation_system, "handle_timeout_escalation"
            ...
            # Verify timeout escalation was processed
            self.assertIsNotNone(timeout_result)
            self.assertTrue(timeout_result.get("escalated", False))

            # Get timed out events from tracker
            timed_out_events = self.escalation_system.tracker.get_events_for_task(
                "TEST-TIMEOUT-01"
            )
    ...
    async def test_hitl_integration(self):
        ...
        mock_hitl_engine = (
            MagicMock()
        )  # Remove spec constraint to allow get_pending_checkpoints
    ...
    def test_concurrent_escalation_handling(self):
        ...
        def create_and_track_event_thread(thread_id):
            ...
            try:
                event = EscalationEvent(
                    ...
                    context={"description": f"Concurrent test event {thread_id}"},
                )
        ...
        for i in range(num_threads):
            thread = threading.Thread(target=create_and_track_event_thread, args=(i,))
    ...
    def test_escalation_metrics_and_reporting(self):
        ...
        for i, (level, status_str, resolution_minutes) in enumerate(test_data):
            event = EscalationEvent(
                ...
                triggered_by="test_user",
                context={"description": "Metrics test event"},
            )
        ...
        level_1_escalations = len(
            [e for e in all_events if e.level == EscalationLevel.LEVEL_1]
        )
        level_2_escalations = len(
            [e for e in all_events if e.level == EscalationLevel.LEVEL_2]
        )
        level_3_escalations = len(
            [e for e in all_events if e.level == EscalationLevel.LEVEL_3]
        )
        level_4_escalations = len(
            [e for e in all_events if e.level == EscalationLevel.LEVEL_4]
        )
        ...
        resolved_events = [
            e for e in all_events if hasattr(e, "resolved_at") and e.resolved_at
        ]
    ...
    def test_error_recovery_and_resilience(self):
        ...
        with patch.object(
            ...
            self.assertFalse(
                results["success"]
            )  # Overall success if other channels work
        ...
        corrupted_file_path = (
            self.escalation_system.tracker.storage_path / "corrupted_escalation.json"
        )
        ...
        self.escalation_system.tracker = EscalationTracker(
            storage_path=str(self.escalation_system.tracker.storage_path)
        )

        # Verify corrupted data is not loaded and no crash occurs
        all_events = self.escalation_system.tracker.get_escalation_history(
            days=100
        )  # Get all events
    ...
    def test_performance_under_load(self):
        ...
        for i in range(num_escalations):
            self.escalation_system.tracker.track_event(
                EscalationEvent(
                    task_id=f"LOAD-{i}",
                    trigger="load_test",
                    level=EscalationLevel.LEVEL_1,
                    triggered_at=datetime.now(),
                    triggered_by="system",
                    context={"description": f"Performance test escalation {i}"},
                )
            )
