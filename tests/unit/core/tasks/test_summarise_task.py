#!/usr/bin/env python3
"""
test_summarise_task.py - Optimized Test Structure

Migrated from: tests/workflows\test_summarise_task.py
New location: tests/unit\core\tasks\test_summarise_task.py
...
class TestTaskSummarizer:
    ...
    def test_analyze_agent_outputs(
            self,
            mock_file_structure,
            sample_task_data):
        ...
        for output in agent_outputs:
            ...
            assert len(output.files_generated) >= 1
    ...
    def test_analyze_code_artifacts(
            self,
            mock_file_structure,
            sample_task_data):
    ...
    def test_determine_completion_status_completed_verified(self):
        ...
        status = summarizer._determine_completion_status(
    ...
    def test_generate_next_steps_completed_verified(self):
        ...
        next_steps = summarizer._generate_next_steps(
            "COMPLETED_VERIFIED", qa_results)

        assert "completed successfully" in next_steps[0].lower()
        assert "integration" in next_steps[1].lower(
        ) or "deployment" in next_steps[1].lower()
    ...
    def test_generate_next_steps_with_issues(self):
        ...
        next_steps = summarizer._generate_next_steps(
            "COMPLETED_WITH_ISSUES", qa_results)
    ...
    def test_determine_file_type_code(self):
        ...
        assert summarizer._determine_file_type(
            Path("components/app.ts")) == 'code'
    ...
    def test_determine_file_type_test(self):
        ...
        assert summarizer._determine_file_type(
            Path("src/test_utils.js")) == 'test'
        assert summarizer._determine_file_type(
            Path("test/component.spec.ts")) == 'test'
    ...
    def test_determine_language(self):
        ...
        assert summarizer._determine_language(
    ...
    def test_calculate_total_code_lines(
            self, mock_file_structure, sample_task_data):
    ...
    def test_analyze_task_completion_integration(
            self, mock_file_structure, sample_task_data):
