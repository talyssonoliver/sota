#!/usr/bin/env python3
"""
test_qa_execution.py - Optimized Test Structure

Migrated from: tests/agents\test_qa_execution.py
New location: tests/unit\core\test_qa_execution.py
...
""")
    ...
    def test_run_static_analysis_python(self, qa_engine):
        ...
        temp_file.write_text(
            "def very_long_function_name_that_exceeds_line_limit_and_should_trigger_warning():\n    print('test')   \n")

        issues = qa_engine._run_static_analysis("BE-07", [str(temp_file)])

        # Check that static analysis executed without error (may or may not
        # find issues)
        assert isinstance(issues, list)
        # Don't assert specific issues as linting rules may vary by environment
    ...
    def test_run_static_analysis_javascript(self, qa_engine):
        ...
        temp_file.write_text(
            "function test() { console.log('this is a very long line that should trigger a line length warning'); }")
        ...
        # find issues)
        assert isinstance(issues, list)
        # Don't assert specific issues as eslint rules may vary by environment
    ...
    def test_generate_qa_report_passed(self, qa_engine):
        ...
        report = qa_engine._generate_qa_report(
            "BE-07", test_results, test_execution, linting_issues, code_files)
    ...
    def test_execute_qa_for_task_success(self, mock_qa_agent, qa_engine):
        ...
        # Check that QA execution completed without error
...
class TestLangGraphQAIntegration:
    ...
    def test_handle_qa_pending_state_success(
            self, mock_qa_engine, temp_outputs_dir):
        ...
        # Create QA integration instance after mock is applied
        qa_integration = LangGraphQAIntegration(str(temp_outputs_dir))

        # Create a temporary task directory to avoid the exception
        task_dir = temp_outputs_dir / "BE-07"
    ...
    @patch('orchestration.langgraph_qa_integration.QAExecutionEngine')
    def test_handle_qa_pending_state_failed(
            self, mock_qa_engine, temp_outputs_dir):
    ...
    def test_determine_next_state(self, qa_integration):
        ...
        assert qa_integration._determine_next_state(
            qa_result) == "DOCUMENTATION"

        # Test FAILED status
        qa_result = {"status": "FAILED"}
        assert qa_integration._determine_next_state(
            qa_result) == "FIXES_REQUIRED"
    ...
    def test_log_state_transition(self, qa_integration):
        ...
        qa_integration._log_state_transition(
            "BE-07", "QA_PENDING", {"agent": "backend"})
