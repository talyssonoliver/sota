...
from src.core.workflows.graph.handlers import (backend_handler, coordinator_handler,
                            documentation_handler, qa_handler, technical_handler)
...
class TestTaskStatusTransitions(unittest.TestCase):
    ...
    def test_get_next_status_backend(self):
        ...
        self.assertEqual(get_next_status(
            TaskStatus.IN_PROGRESS, "backend"), TaskStatus.QA_PENDING)
        self.assertEqual(get_next_status(TaskStatus.PLANNED,
    ...
    def test_get_next_status_frontend(self):
        ...
        self.assertEqual(get_next_status(
            TaskStatus.IN_PROGRESS, "frontend"), TaskStatus.QA_PENDING)
        self.assertEqual(get_next_status(TaskStatus.PLANNED,
    ...
    def test_get_next_status_qa(self):
        ...
        self.assertEqual(get_next_status(
            TaskStatus.QA_PENDING, "qa", False), TaskStatus.BLOCKED)

    def test_get_next_status_documentation(self):
        """Test the next status determination for documentation agent."""
        self.assertEqual(get_next_status(
            TaskStatus.DOCUMENTATION, "documentation"), TaskStatus.DONE)
    ...
    def test_get_valid_transitions(self):
        ...
        self.assertEqual(created_transitions.get(
            "coordinator"), TaskStatus.PLANNED)
        ...
        self.assertEqual(qa_pending_transitions.get(
...
class TestHandlerStateTransitions(unittest.TestCase):
    ...
    def test_qa_handler_state_transition_success(self):
        ...
        with patch('src.infrastructure.tools.handlers.qa_handler.qa_agent') as mock_qa_agent:
    ...
    def test_qa_handler_state_transition_failure(self):
        ...
        # Mock the QA agent implementation to return BLOCKED status - Fixed the
        # import path
        with patch('src.infrastructure.tools.handlers.qa_handler.qa_agent') as mock_qa_agent:
            mock_qa_agent.return_value = {
    ...
    def test_qa_handler_exception_handling(self):
        ...
        # Mock the QA agent implementation to raise an exception - Fixed the
        # import path
        with patch('src.infrastructure.tools.handlers.qa_handler.qa_agent') as mock_qa_agent:
...
class TestFullWorkflowStateSequence(unittest.TestCase):
    ...
    def test_happy_path_workflow_sequence(self):
        ...
        with patch('src.core.workflows.registry.create_agent_instance') as mock_create_agent, \
            ...
            # Configure mocks
            mock_agent = MagicMock()
            mock_agent.execute.side_effect = lambda state: agent_responses.get(
                state.get("agent", ""))
            mock_create_agent.return_value = mock_agent
