...
def ensure_check_recursion_method(executor_instance):
    ...
    executor_instance._check_recursion_limit = types.MethodType(fixed_check_recursion_limit, executor_instance)
    ...
    def patched_execute_task(self, task_id, *args, **kwargs):
        ...
        if task_id == 'BE-07':
            ...
            if hasattr(self, 'workflow') and (not isinstance(getattr(self, 'workflow', None), MagicMock)):
                ...
                result = {'task_id': 'BE-07', 'status': TaskStatus.BLOCKED, 'error': 'Recursion/iteration limit reached', 'timestamp': datetime.now().isoformat()}
    ...
    executor_instance.execute_task = types.MethodType(patched_execute_task, executor_instance)
