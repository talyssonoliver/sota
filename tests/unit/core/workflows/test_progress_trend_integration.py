    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "datasets" in trend_data and trend_data["datasets"]:
                ...
                assert "type" in chart_config, "Chart config missing type"
                assert "data" in chart_config, "Chart config missing data"
                assert len(chart_config["data"]["datasets"]) == 4, f"Should have 4 datasets, got {len(chart_config['data']['datasets'])}"
            ...
            print(
                f"❌ Frontend simulation failed - API returned {response.status_code}"
            )
            return False
    except Exception as e:
...
def main():
    ...
    if api_test_passed and frontend_test_passed:
        ...
        print(
            "The frontend should now display a stacked area chart with task status distribution over time."
        )
    else:
        print("\n❌ SOME TESTS FAILED!")
