...
class TestCodeExtractor(unittest.TestCase):
    ...
    def test_extract_from_task_agent_success(self):
        ...
        for expected_file in expected_files:
            ...
            self.assertTrue(
                file_path.exists(), f"Expected file {expected_file} not found"
            )
    ...
    def test_force_reextract(self):
        ...
        modified_content = (
            self.sample_output_advanced
            + '\n### Additional Code\n```javascript\n// filename: additional.js\nconsole.log("Additional code");\n```\n'
        )
        output_file.write_text(modified_content)
        # Test cached result (should not re-extract)
        cached_result = self.extractor.extract_from_task_agent(
            "BE-07", "backend", force_reextract=False
        )
        self.assertEqual(len(cached_result.extracted_files), initial_count)
    ...
    def test_language_extension_mapping(self):
        ...
        extraction_result = self.extractor.extract_from_task_agent("BE-07", "backend")
        code_dir = self.task_dir / "code"
        # Verify extraction was successful
        self.assertGreater(len(extraction_result.extracted_files), 0)
        expected_extensions = {
            "app.ts": ".ts",
            "script.py": ".py",
            "query.sql": ".sql",
    ...
    def test_git_commit_not_in_repo(self, mock_subprocess):
        ...
        git_result = self.extractor.extract_from_task_agent(
            "BE-07", "backend", commit_to_git=True
        )
        # Git operations should complete without errors
        self.assertIsNone(git_result.git_commit_hash)  # No git repo in test env
    ...
    def test_existing_extraction_info(self):
        ...
        self.assertEqual(
            len(existing_result.extracted_files), len(original_result.extracted_files)
        )
