...
class TestStep43WorkflowExecution:
    ...
    def test_build_task_state_success(self, mock_task_metadata):
        ...
        with patch(
            "src.core.workflows.execute_graph.load_task_metadata"
        ) as mock_load_task, patch(
            "src.core.workflows.execute_graph.get_relevant_context"
        ) as mock_get_context:
    ...
    def test_run_task_graph_dry_run(self):
        """Test dry run mode."""
        with patch(
            "src.core.workflows.execute_graph.build_task_state"
        ) as mock_build_state:
            mock_state = {
                "task_id": "BE-07",
                "title": "Test Task",
    ...
    def test_run_task_graph_advanced_workflow(self, mock_workflow_execution):
        ...
        assert result["task_id"] == expected_result["task_id"]
        # Validate expected result structure
        assert expected_result["result"] == "Mock workflow completed"
...
class TestStep43CLIInterface:
    ...
    def test_main_with_task_argument(self):
        ...
        with patch("sys.argv", test_args), patch(
            "src.core.workflows.execute_graph.run_task_graph"
        ) as mock_run:
