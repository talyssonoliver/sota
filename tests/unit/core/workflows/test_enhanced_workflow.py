...
from src.platform.tools.notifications import NotificationLevel
    ...
    def test_all_components_together(self):
        ...
        def run_monitor():
            ...
            try:
                ...
                pass  # Ignore errors in test monitor
        ...
        try:
            ...
            # Verify the task was executed
            self.assertEqual(result["task_id"], task_id)            # Verify output files were created
            self.assertTrue(
                (self.test_output_dir / task_id / "status.json").exists())
            ...
            # Ensure cleanup of monitor process
