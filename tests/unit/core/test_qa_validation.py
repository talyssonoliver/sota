...
New location: tests/unit\\core	est_qa_validation.py
...
def test_qa_report_exists(tmp_path):
    ...
    qa_report_data = {'task_id': 'BE-07', 'timestamp': '2025-06-11T02:00:00Z', 'tests_passed': 15, 'tests_failed': 2, 'coverage_percentage': 85.5, 'linting_issues': ['Line too long at line 42'], 'type_check_issues': [], 'security_issues': [], 'performance_metrics': {'execution_time': 2.5, 'memory_usage': '45MB'}, 'overall_status': 'PASSED', 'recommendations': ['Fix line length issues', 'Add more edge case tests'], 'next_steps': ['Address failing tests', 'Improve test coverage for error handling']}
    ...
    with qa_report_path.open(encoding='utf-8') as f:
        data = json.load(f)
    required_fields = ['task_id', 'timestamp', 'tests_passed', 'tests_failed', 'coverage_percentage', 'linting_issues', 'type_check_issues', 'security_issues', 'performance_metrics', 'overall_status', 'recommendations', 'next_steps']
