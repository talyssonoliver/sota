...
class FeedbackCollector:
    ...
    @staticmethod
    def print_result(test_name: str, passed: bool, details: Optional[Dict[str, Any]]=None, execution_time: Optional[float]=None) -> bool:
        """
    ...
    @staticmethod
    def save_results(results: List[Tuple[str, bool]], details: Dict[str, Any], output_dir: str='test_results') -> str:
        """
        ...
        results_data = {'timestamp': datetime.now().isoformat(), 'tests_run': len(results), 'tests_passed': sum((1 for _, result in results if result)), 'tests_failed': sum((1 for _, result in results if not result)), 'details': details, 'results': {name: 'passed' if passed else 'failed' for name, passed in results}}
...
def ensure_clean_test_environment():
    ...
    cleanup_targets = [project_root / 'runtime' / 'temp', project_root / 'runtime' / 'logs' / 'daily_cycle', project_root / 'runtime' / 'logs' / 'langgraph', project_root / 'test_outputs', project_root / 'outputs' / 'TEST-*']
                ...
                for item in target.iterdir():
                    if item.name.startswith(('tmp', 'test_', 'mock_')):
                        try:
                            ...
                            pass
            elif any((pattern in target.name for pattern in ['tmp', 'test_', 'mock_'])):
                try:
