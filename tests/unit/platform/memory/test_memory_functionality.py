...
class TestMemoryEngineCore(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_embeddings, self.mock_embeddings_instance = (
            create_mock_openai_embeddings()
        )
        self.patcher = patch(
    ...
    def test_context_retrieval_basic(self):
        ...
        context = self.memory.get_context(
            "What authentication system does the platform use?", k=3, user="test_user"
        )
    ...
    def test_access_control_enforcement(self):
        ...
        self.memory.add_document(self.test_file, user="test_user")
        context = self.memory.get_context(
            "authentication", k=1, user="unauthorized_user"
        )
        self.assertIsInstance(context, str)
    ...
    def test_conversation_context_persistence(self):
        ...
        context1 = self.memory.get_context(
            "What authentication system is used?", k=2, user="test_user"
        )
        context2 = self.memory.get_context(
            "How are JWT tokens handled?", k=2, user="test_user"
        )
...
class TestMemoryEngineIntegration(unittest.TestCase):
    ...
    def test_retrieval_qa_integration(self):
        ...
        try:
            ...
            with patch(
                "src.infrastructure.tools.core.retrieval_qa.get_memory_instance"
            ) as mock_get_memory:
                mock_memory = Mock()
                mock_memory.retrieval_qa.return_value = (
                    "Test answer from knowledge base"
                )
