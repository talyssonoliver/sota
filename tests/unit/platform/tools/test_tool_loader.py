...
def test_load_tool_config():
    ...
    FeedbackCollector.print_section("Tool Configuration Loading")
    timer = Timer().start()
...
def test_agent_tool_mapping():
    ...
    FeedbackCollector.print_section("Agent-Tool Mapping")
    timer = Timer().start()
...
def test_get_tools_for_agent():
    ...
    FeedbackCollector.print_section("Tool Retrieval for Agents")
    timer = Timer().start()
...
def run_all_tests():
    ...
    FeedbackCollector.print_header("Dynamic Tool Loader")
    ...
    exit_code = FeedbackCollector.print_summary(results, test_start)
