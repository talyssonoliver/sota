...
class TestHITLEngineIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.test_config = {'global_settings': {'default_timeout_hours': 24, 'escalation_timeout_hours': 72, 'enable_notifications': True, 'enable_audit_logging': True, 'max_retries': 3}, 'checkpoint_triggers': {'agent_prompt': {'enabled': True, 'risk_threshold': 'medium', 'timeout_hours': 24, 'required_approvers': 1, 'auto_approve_patterns': ['simple_crud', 'documentation_update']}, 'output_evaluation': {'enabled': True, 'risk_threshold': 'high', 'timeout_hours': 12, 'required_approvers': 2, 'critical_patterns': ['security_config', 'database_migration']}, 'qa_validation': {'enabled': True, 'risk_threshold': 'medium', 'timeout_hours': 8, 'required_approvers': 1}, 'documentation': {'enabled': False, 'risk_threshold': 'low', 'timeout_hours': 48, 'required_approvers': 1}, 'task_transitions': {'enabled': True, 'risk_threshold': 'high', 'timeout_hours': 6, 'required_approvers': 2}}, 'task_type_policies': {'backend': {'enabled': True, 'default_risk_level': 'medium', 'auto_approve_low_risk': True, 'risk_patterns': {'high': ['security_config', 'auth_implementation', 'database_migration'], 'medium': ['api_endpoint', 'business_logic', 'integration'], 'low': ['simple_crud', 'data_formatting', 'validation']}}, 'frontend': {'enabled': True, 'default_risk_level': 'low', 'auto_approve_low_risk': True, 'risk_patterns': {'high': ['authentication_ui', 'payment_flow', 'admin_interface'], 'medium': ['complex_form', 'data_visualization', 'user_workflow'], 'low': ['styling', 'static_content', 'simple_component']}}, 'design': {'enabled': False, 'default_risk_level': 'low', 'auto_approve_low_risk': True}, 'infrastructure': {'enabled': True, 'default_risk_level': 'high', 'auto_approve_low_risk': False, 'risk_patterns': {'high': ['production_deployment', 'security_config', 'database_config'], 'medium': ['staging_deployment', 'monitoring_setup', 'backup_config'], 'low': ['documentation', 'logging_config']}}, 'qa': {'enabled': True, 'default_risk_level': 'medium', 'auto_approve_low_risk': True}}, 'escalation_policies': {'high_risk': {'escalation_levels': ['team_lead', 'technical_director', 'cto'], 'notification_channels': ['dashboard', 'email', 'slack'], 'escalation_delay_hours': 8}, 'medium_risk': {'escalation_levels': ['team_lead', 'technical_director'], 'notification_channels': ['dashboard', 'email'], 'escalation_delay_hours': 24}, 'low_risk': {'escalation_levels': ['team_lead'], 'notification_channels': ['dashboard'], 'escalation_delay_hours': 48}}, 'notification_templates': {'checkpoint_created': {'subject': 'Review Required: {task_id} - {checkpoint_type}', 'body': 'A new HITL checkpoint requires your review.\n\nTask: {task_id}\nType: {checkpoint_type}\nRisk Level: {risk_level}\n\nPlease review at: {review_url}'}, 'checkpoint_approved': {'subject': 'Approved: {task_id} - {checkpoint_type}', 'body': 'Checkpoint has been approved by {reviewer_id}.\n\nComments: {comments}'}, 'checkpoint_rejected': {'subject': 'Rejected: {task_id} - {checkpoint_type}', 'body': 'Checkpoint has been rejected by {reviewer_id}.\n\nReason: {comments}'}, 'checkpoint_escalated': {'subject': 'ESCALATED: {task_id} - {checkpoint_type}', 'body': 'Checkpoint has been escalated due to timeout.\n\nOriginal timeout: {timeout_hours} hours\nEscalation level: {escalation_level}'}}, 'integration_settings': {'dashboard': {'enabled': True, 'base_url': 'http://localhost:5000', 'refresh_interval_seconds': 30}, 'email': {'enabled': True, 'smtp_server': 'smtp.company.com', 'smtp_port': 587, 'use_tls': True}, 'slack': {'enabled': True, 'webhook_url': 'https://hooks.slack.com/services/...', 'default_channel': '#engineering-reviews'}}}
