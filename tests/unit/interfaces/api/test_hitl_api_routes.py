...
class TestHITLAPIRoutes(unittest.TestCase):
    ...
    def test_get_checkpoints_with_filters(self):
        ...
        response = self.client.get(
            "/api/hitl/checkpoints?risk_level=high&task_type=backend"
        )
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
    ...
    def test_approve_checkpoint(self):
        ...
        response = self.client.post(
            "/api/hitl/checkpoints/cp-1/approve",
            json={"reviewer_id": "john.doe", "comments": "Looks good to me"},
        ...
        call_kwargs = self.mock_hitl_engine.process_decision.call_args[1]
        self.assertEqual(call_kwargs["checkpoint_id"], "cp-1")
        self.assertEqual(call_kwargs["decision"], "approve")
        self.assertEqual(call_kwargs["user_id"], "john.doe")

    def test_reject_checkpoint(self):
        """Test POST /api/hitl/checkpoints/<checkpoint_id>/reject endpoint."""
        self.mock_hitl_engine.process_decision.return_value = True
        response = self.client.post(
            "/api/hitl/checkpoints/cp-1/reject",
            json={
        ...
        call_kwargs = self.mock_hitl_engine.process_decision.call_args[1]
        self.assertEqual(call_kwargs["decision"], "reject")
        self.assertEqual(call_kwargs["comments"], "Needs improvement in error handling")
    ...
    def test_escalate_checkpoint(self):
        ...
        response = self.client.post(
            "/api/hitl/checkpoints/cp-1/escalate",
            json={
        ...
        self.mock_hitl_engine.escalate_checkpoint.assert_called_once_with(
            "cp-1", "team.lead"
        )
    ...
    def test_approval_engine_failure(self):
        ...
        response = self.client.post(
            "/api/hitl/checkpoints/cp-1/approve",
            json={"reviewer_id": "user", "comments": "Test"},
            content_type="application/json",
        )
    ...
    def test_batch_reject_checkpoints(self):
        ...
        self.mock_hitl_engine.process_decision.return_value = True
        response = self.client.post(
            "/api/hitl/checkpoints/batch/reject",
            json={
                "checkpoint_ids": ["cp-1", "cp-2"],
                "reviewer_id": "batch.reviewer",
    ...
    def test_create_checkpoint_endpoint(self):
        ...
        checkpoint_data = {
            ...
            "context": {"prompt": "Create new service"},
        }
        response = self.client.post(
    ...
    def test_dashboard_data_endpoint(self):
        ...
        mock_dashboard_data = {
            ...
            "metrics": {"total_pending": 2, "high_risk_count": 1},
            "workflow_status": {"active_tasks": 5, "blocked_tasks": 2},
        }
        with patch(
            "src.interfaces.api.hitl_routes.HITLDashboardManager"
    ...
    def test_task_dashboard_data_endpoint(self):
        ...
        with patch(
            ...
            response = self.client.get(
                "/api/hitl/dashboard/tasks/BE-07?task_type=backend"
            )
            self.assertEqual(response.status_code, 200)
            data = json.loads(response.data)
