"""
test_hitl_api_routes.py - Optimized Test Structure

Migrated from: tests/hitl	est_hitl_api_routes.py
New location: tests/unit\\interfaces\x07pi	est_hitl_api_routes.py
...
class TestHITLAPIRoutes(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_checkpoints = [{'checkpoint_id': 'cp-1', 'task_id': 'BE-07', 'checkpoint_type': 'agent_prompt', 'task_type': 'backend', 'status': 'pending', 'risk_level': 'high', 'created_at': datetime.now().isoformat(), 'timeout_at': (datetime.now() + timedelta(hours=24)).isoformat(), 'content': {'prompt': 'Create API endpoint'}, 'risk_factors': ['api_design', 'security']}, {'checkpoint_id': 'cp-2', 'task_id': 'FE-05', 'checkpoint_type': 'output_evaluation', 'task_type': 'frontend', 'status': 'pending', 'risk_level': 'medium', 'created_at': datetime.now().isoformat(), 'timeout_at': (datetime.now() + timedelta(hours=12)).isoformat(), 'content': {'code': 'React component'}, 'risk_factors': ['complex_form']}]
    ...
    def test_get_checkpoints_with_filters(self):
        ...
        filtered_checkpoints = [self.test_checkpoints[0]]
    ...
    def test_approve_checkpoint(self):
        ...
        response = self.client.post('/api/hitl/checkpoints/cp-1/approve', json={'reviewer_id': 'john.doe', 'comments': 'Looks good to me'}, content_type='application/json')
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
    ...
    def test_reject_checkpoint(self):
        ...
        self.mock_hitl_engine.process_decision.return_value = True
        response = self.client.post('/api/hitl/checkpoints/cp-1/reject', json={'reviewer_id': 'jane.smith', 'comments': 'Needs improvement in error handling'}, content_type='application/json')
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertTrue(data['success'])
    ...
    def test_escalate_checkpoint(self):
        ...
        response = self.client.post('/api/hitl/checkpoints/cp-1/escalate', json={'reviewer_id': 'team.lead', 'reason': 'Requires senior architect review'}, content_type='application/json')
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertTrue(data['success'])
    ...
    def test_approve_checkpoint_missing_data(self):
        ...
        response = self.client.post('/api/hitl/checkpoints/cp-1/approve', json={'comments': 'Missing reviewer_id'}, content_type='application/json')
    ...
    def test_approval_engine_failure(self):
        ...
        response = self.client.post('/api/hitl/checkpoints/cp-1/approve', json={'reviewer_id': 'user', 'comments': 'Test'}, content_type='application/json')
    ...
    def test_batch_approve_checkpoints(self):
        ...
        self.mock_hitl_engine.process_decision.return_value = True
        response = self.client.post('/api/hitl/checkpoints/batch/approve', json={'checkpoint_ids': ['cp-1', 'cp-2'], 'reviewer_id': 'batch.reviewer', 'comments': 'Batch approval'}, content_type='application/json')
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertTrue(data['success'])
    ...
    def test_batch_reject_checkpoints(self):
        ...
        self.mock_hitl_engine.process_decision.return_value = True
        response = self.client.post('/api/hitl/checkpoints/batch/reject', json={'checkpoint_ids': ['cp-1', 'cp-2'], 'reviewer_id': 'batch.reviewer', 'comments': 'Batch rejection'}, content_type='application/json')
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertTrue(data['success'])
        self.assertEqual(data['processed_count'], 2)
    ...
    def test_batch_with_partial_failures(self):
        ...
        response = self.client.post('/api/hitl/checkpoints/batch/approve', json={'checkpoint_ids': ['cp-1', 'cp-2'], 'reviewer_id': 'user', 'comments': 'Test batch'}, content_type='application/json')
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertEqual(data['processed_count'], 2)
    ...
    def test_get_metrics_endpoint(self):
        ...
        mock_metrics = {'total_checkpoints': 100, 'pending_count': 5, 'approved_count': 80, 'rejected_count': 10, 'escalated_count': 5, 'average_review_time_hours': 3.5, 'approval_rate': 85.0, 'escalation_rate': 5.0}
    ...
    def test_get_metrics_with_filters(self):
        ...
        mock_metrics = {'total_checkpoints': 50, 'period': '7_days'}
        self.mock_hitl_engine.get_metrics.return_value = mock_metrics
    ...
    def test_get_task_checkpoints(self):
        ...
        task_checkpoints = [self.test_checkpoints[0]]
    ...
    def test_get_audit_trail(self):
        ...
        mock_audit_entries = [{'timestamp': datetime.now().isoformat(), 'action': 'created', 'user_id': 'system', 'details': 'Checkpoint created automatically'}, {'timestamp': datetime.now().isoformat(), 'action': 'approved', 'user_id': 'john.doe', 'details': 'Approved with comments: Looks good'}]
    ...
    def test_create_checkpoint_endpoint(self):
        ...
        checkpoint_data = {'task_id': 'BE-08', 'checkpoint_type': 'agent_prompt', 'task_type': 'backend', 'content': {'prompt': 'Create new service'}, 'risk_factors': ['complex_logic']}
        response = self.client.post('/api/hitl/checkpoints', json=checkpoint_data, content_type='application/json')
    ...
    def test_dashboard_data_endpoint(self):
        ...
        mock_dashboard_data = {'pending_reviews': self.test_checkpoints, 'metrics': {'total_pending': 2, 'high_risk_count': 1}, 'workflow_status': {'active_tasks': 5, 'blocked_tasks': 2}}
    ...
    def test_task_dashboard_data_endpoint(self):
        ...
        mock_task_data = {'task_id': 'BE-07', 'workflow_status': {'current_phase': 'agent_prompt', 'blocked_on_review': True}, 'pending_checkpoints': [self.test_checkpoints[0]]}
        with patch('src.interfaces.api.hitl_routes.HITLDashboardManager') as mock_manager_class:
            mock_manager = mock_manager_class.return_value
    ...
    def test_error_handling(self):
        ...
        response = self.client.post('/api/hitl/checkpoints/cp-1/approve', data='invalid json', content_type='application/json')
        self.assertEqual(response.status_code, 400)
