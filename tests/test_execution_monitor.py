...
from utils.execution_monitor import (
    ExecutionMonitor, get_execution_monitor, monitor_execution,
    LangGraphExecutionHook, CrewAIExecutionHook, DashboardLogger,
    create_langgraph_hook, create_crewai_hook, get_dashboard_logger
)
...
class TestExecutionMonitor(unittest.TestCase):
    ...
    def test_start_agent_execution(self):
        ...
        execution_data = self.monitor.start_agent_execution(task_id, agent, context)
    ...
    def test_complete_agent_execution_success(self):
        ...
        self.monitor.complete_agent_execution(execution_data, "COMPLETED", output)
    ...
    def test_get_execution_stats_with_data(self):
        ...
        for task_id, agent, status in zip(task_ids, agents, statuses):
            ...
            self.monitor.complete_agent_execution(execution_data, status, error=error)
        ...
        self.assertGreaterEqual(stats['average_duration_minutes'], 0.0)  # Allow 0.0 for very fast executions
...
class TestExecutionMonitorDecorator(unittest.TestCase):
    ...
    def test_decorator_success(self, mock_get_monitor):
        ...
        # Mock execution data
        execution_data = {'task_id': 'BE-07', 'agent': 'backend', 'start_time': time.time()}
        ...
        self.mock_monitor.start_agent_execution.assert_called_once_with("BE-07", "backend")
...
class TestExecutionMonitorIntegration(unittest.TestCase):
    ...
    def test_real_time_monitoring_workflow(self):
        ...
        self.assertTrue(os.path.exists(os.path.join(self.logs_dir, f"execution-{task_id}.log")))
        ...
        expected_agents = ["workflow", "coordinator", "backend", "qa", "documentation"]
    ...
    def test_step_4_8_requirements_compliance(self):
        ...
        # Complete execution
        monitor.complete_agent_execution(execution_data, "COMPLETED", {"output": "success"})
        
        # Verify Step 4.8 requirement: logs/execution-BE-07.log
        log_file = os.path.join(self.logs_dir, f"execution-{task_id}.log")
        self.assertTrue(os.path.exists(log_file), "execution-BE-07.log should be created")
        
        # Verify Step 4.8 requirement: reports/execution-summary.csv
        csv_file = os.path.join(self.reports_dir, "execution-summary.csv")
        self.assertTrue(os.path.exists(csv_file), "execution-summary.csv should be created")
        
        # Verify Step 4.8 requirement: Log shows completion time
        with open(log_file, 'r') as f:
            log_content = f.read()
            self.assertIn("completed in", log_content, "Log should show completion time")
            self.assertIn("minutes", log_content, "Log should show time in minutes")
        ...
        self.assertEqual(row['task_id'], task_id)
        self.assertEqual(row['agent'], agent)
        self.assertGreaterEqual(float(row['duration_minutes']), 0.0)  # Allow 0.0 for very fast executions
...
class TestExecutionMonitorStep48:
    ...
    def setup_method(self):
        ...
        self.monitor = ExecutionMonitor(str(self.logs_dir), str(self.reports_dir))
    ...
    def test_basic_execution_monitoring(self):
        ...
        self.monitor.complete_agent_execution(execution_data, "COMPLETED", {"result": "success"})
    ...
    def test_langgraph_execution_hook(self):
        ...
        assert task_id in hook.active_executions or len(hook.active_executions) == 0
    ...
    def test_crewai_execution_hook(self):
        ...
        hook.on_agent_complete("backend_agent", "Service functions implemented", True)
        
        # Test crew result
        hook.on_crew_result({"status": "success", "output": "All tasks completed"})
        
        # Verify monitoring data
        assert task_id in hook.crew_executions or len(hook.crew_executions) == 0
    ...
    def test_dashboard_logger(self):
        ...
        agent = "test_agent"
        dashboard_logger.update_live_dashboard(task_id, agent, "RUNNING", 120.5)
    ...
    def test_execution_summary_report(self):
        ...
        for i in range(3):
            ...
            execution_data = self.monitor.start_agent_execution(task_id, "test_agent")
    ...
    def test_error_handling_in_monitoring(self):
        ...
        task_id = "ERROR-001"
        execution_data = self.monitor.start_agent_execution(task_id, "error_agent")
        
        # Complete with error
        error_message = "Test error occurred"
        self.monitor.complete_agent_execution(execution_data, "FAILED", error=error_message)
