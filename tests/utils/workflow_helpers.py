...
class WorkflowTestHelper:
    ...
    def create_mock_task(self, task_id: str, **kwargs) -> Dict[str, Any]:
        ...
        task = {
            ...
            "description": kwargs.get(
                "description", f"Mock task {task_id} for testing"
            ),
            "agent_type": kwargs.get("agent_type", "test_agent"),
            "dependencies": kwargs.get("dependencies", []),
            "expected_output": kwargs.get("expected_output", "Mock output"),
    ...
    def create_mock_workflow_state(
        self, workflow_id: str = None, **kwargs
    ) -> Dict[str, Any]:
        ...
        state = {
            ...
            "current_step": kwargs.get("current_step", "init"),
            ...
            "metadata": kwargs.get(
                "metadata", {"test_workflow": True, "created_by": "WorkflowTestHelper"}
            ),
        }
    ...
    def simulate_workflow_execution(
        self, workflow_id: str, steps: List[str], step_duration: float = 0.1
    ) -> Dict[str, Any]:
        """Simulate workflow execution through multiple steps."""
        import time

        state = self.mock_states.get(workflow_id) or self.create_mock_workflow_state(
            workflow_id
        )
        for i, step in enumerate(steps):
            ...
            state["results"][step] = {
                "success": True,
                "output": f"Mock output for step {step}",
                "timestamp": datetime.now().isoformat(),
            }
    ...
    def create_test_workflow_config(self, **kwargs) -> Dict[str, Any]:
        ...
        return {
            "name": kwargs.get("name", "test_workflow"),
            "description": kwargs.get("description", "Test workflow for unit testing"),
            "version": kwargs.get("version", "1.0.0"),
            "agents": kwargs.get(
                "agents",
                [
                    {"type": "test_agent", "name": "test_agent_1"},
                    {"type": "qa_agent", "name": "qa_agent_1"},
                ],
            ),
            "tasks": kwargs.get(
                "tasks",
                [
                    {"id": "task_1", "agent_type": "test_agent"},
                    {
                        "id": "task_2",
                        "agent_type": "qa_agent",
                        "dependencies": ["task_1"],
    ...
    def validate_workflow_output(
        self, output: Dict[str, Any], expected_keys: List[str] = None
    ) -> bool:
        ...
        if output.get("status") not in valid_statuses:
            raise AssertionError(
                f"Invalid status '{output.get('status')}'. Must be one of {valid_statuses}"
            )
        if "progress" in output:
            progress = output["progress"]
            if not 0.0 <= progress <= 1.0:
                raise AssertionError(
                    f"Progress must be between 0.0 and 1.0, got {progress}"
                )
        return True
    ...
    def create_test_memory_context(self, context_type: str = "test") -> Dict[str, Any]:
        ...
        return {
            "context_id": f"test_context_{context_type}",
            "type": context_type,
            "content": f"Test {context_type} context for workflow testing",
            "tags": ["test", "workflow", context_type],
            "created_at": datetime.now().isoformat(),
            "metadata": {"test_data": True, "workflow_test": True},
        }
    ...
    def create_test_agent_config(self, agent_type: str) -> Dict[str, Any]:
        ...
        agent_configs = {
            "backend": {
                ...
                "backstory": "Experienced backend developer specializing in API development",
                "tools": ["github_tool", "supabase_tool"],
                "context_domains": ["backend", "api", "database"],
            },
            "frontend": {
                "role": "Frontend Engineer",
                "goal": "Create user interfaces and frontend components",
                "backstory": "Expert frontend developer with React and modern web technologies",
                "tools": ["github_tool", "vercel_tool"],
                "context_domains": ["frontend", "ui", "react"],
            },
            "qa": {
                "role": "Quality Assurance Engineer",
                "goal": "Ensure code quality and test coverage",
                "backstory": "Meticulous QA engineer focused on automated testing and quality",
                "tools": ["testing_tool", "coverage_tool"],
                "context_domains": ["testing", "qa", "quality"],
            },
            "technical_lead": {
                "role": "Technical Lead",
                "goal": "Coordinate technical decisions and architecture",
                "backstory": "Senior technical leader with expertise in system architecture",
                "tools": ["architecture_tool", "planning_tool"],
                "context_domains": ["architecture", "leadership", "planning"],
            },
        }
        config = agent_configs.get(
            agent_type,
            {
                ...
                "goal": f"Execute {agent_type} related tasks",
                "backstory": f"Mock {agent_type} agent for testing",
                "tools": [],
                "context_domains": [agent_type],
            },
        )
        config.update(
            {
...
def validate_workflow_output(
    output: Dict[str, Any], expected_keys: List[str] = None
) -> bool:
    """Validate workflow output structure and content."""
    helper = WorkflowTestHelper()
...
def simulate_workflow_execution(
    workflow_id: str, steps: List[str], step_duration: float = 0.1
) -> Dict[str, Any]:
    """Simulate workflow execution through multiple steps."""
    helper = WorkflowTestHelper()
