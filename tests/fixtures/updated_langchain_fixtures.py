...
try:
    ...
    from test_environment import RunnableChainMock, RunnableLLMMock
...
def create_test_file(content, file_path=None):
    ...
    if file_path is None:
        ...
        file_path = os.path.join(os.path.dirname(
            __file__), "..", "test_data", "context-store", "test_doc.md")
...
class LangChainMockFixture:
    ...
    def __init__(self):
        ...
        # Use RunnableChainMock for RetrievalQA and
        ...
        def qa_invoke(input, *a, **kw):
            if hasattr(
                    self.mock_qa_chain.invoke,
                    'return_value') and self.mock_qa_chain.invoke.return_value is not None:
        ...
        def conv_invoke(input, *a, **kw):
            if hasattr(
                    self.mock_conv_chain.invoke,
                    'return_value') and self.mock_conv_chain.invoke.return_value is not None:
        ...
        self.mocks = {
            ...
            'Chroma': MagicMock(),
            'RetrievalQA': MagicMock(
                side_effect=lambda *a,
                **kw: self.mock_qa_chain),
            'ConversationalRetrievalChain': MagicMock(
                side_effect=lambda *a,
                **kw: self.mock_conv_chain),
