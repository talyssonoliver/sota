...
from orchestration.generate_prompt import generate_prompt, get_task_context, main
...
"""
    ...
    def test_get_task_context_file_not_found(self):
        ...
        with patch('orchestration.generate_prompt.load_task_metadata') as mock_load_task:
            mock_load_task.side_effect = FileNotFoundError("Task file not found")
            
            context = get_task_context('NONEXISTENT')
            
            assert "Task metadata not found for NONEXISTENT" in context
    
    def test_generate_prompt_success(self, mock_task_metadata, mock_prompt_template, temp_dir):
        ...
        with patch('orchestration.generate_prompt.load_prompt_template') as mock_load_template, \
            ...
            mock_load_template.assert_called_once_with("prompts/backend-agent.md")
            mock_load_task.assert_called_once_with('BE-07')
            ...
            template_vars = call_args[0][2]  # Third argument contains template_vars
    ...
    def test_generate_prompt_default_output_path(self, mock_task_metadata, mock_prompt_template):
    ...
    def test_generate_prompt_task_metadata_not_found(self, mock_prompt_template):
        """Test prompt generation when task metadata doesn't exist."""
        with patch('orchestration.generate_prompt.load_prompt_template') as mock_load_template, \
             patch('orchestration.generate_prompt.load_task_metadata') as mock_load_task:
            
            mock_load_template.return_value = mock_prompt_template
            mock_load_task.side_effect = FileNotFoundError("Task metadata not found")
            
            with pytest.raises(FileNotFoundError, match="Task metadata file not found"):
                generate_prompt('NONEXISTENT', 'backend-agent')
    
    def test_agent_id_normalization(self, mock_task_metadata, mock_prompt_template):
    ...
    def test_agent_template_path_generation(self, agent_input, expected_template, mock_task_metadata, mock_prompt_template):
    ...
    def test_cli_with_positional_arguments(self):
        ...
        with patch('orchestration.generate_prompt.generate_prompt') as mock_generate, \
            ...
            with patch('sys.exit'):
                try:
                    main()
                    mock_generate.assert_called_once_with('BE-07', 'backend-agent', None)
                except SystemExit:
    ...
    def test_cli_with_custom_output_path(self):
        ...
        with patch('orchestration.generate_prompt.generate_prompt') as mock_generate, \
            ...
            with patch('sys.exit'):
                try:
                    main()
                    mock_generate.assert_called_once_with('BE-07', 'backend-agent', 'custom/path.md')
                except SystemExit:
                    pass
