...
def display_rich_table(items: List[KanbanItem]):
    ...
    for item in items:
        ...
        deadline_style = (
            "red"
            if item.overdue
            else "yellow" if "h" in str(item.deadline) else "green"
        )
...
def display_simple_table(items: List[KanbanItem]):
    ...
    # Header
    print(
        f"{'Task ID':<8} {'Status':<15} {'Pending Reviewer':<18} {'Deadline':<12} {'Action':<12} {'Priority':<8}"
    )
    print("-" * 100)
    # Rows
    for item in items:
        priority_icon = (
            "[H]" if item.priority >= 10 else ("[!]" if item.priority >= 6 else "•")
        )
        ...
        print(
            f"{item.task_id:<8} {status_text:<15} {item.pending_reviewer:<18} {item.deadline:<12} {item.action:<12} {priority_text:<8}"
        )
    ...
    high_priority = len([item for item in items if item.priority >= 6])
    print(
        f"Summary: Total: {total_items} | Overdue: {overdue_items} | High Priority: {high_priority}"
    )
