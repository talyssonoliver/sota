...
class HITLKanbanBoard:
            ...
            for checkpoint in pending_checkpoints:
                ...
                if checkpoint.status == CheckpointStatus.PENDING:
                    if checkpoint.checkpoint_type in [
                        ...
                        reviewer = self._get_assigned_reviewer(
                            checkpoint.task_id, checkpoint.checkpoint_type
                        )
                        action = "Approve"
                ...
                time_remaining, overdue = self._calculate_time_remaining(
                    checkpoint.timeout_at
                )

                # Determine priority
                priority = self._calculate_priority(
                    checkpoint.risk_level, overdue, checkpoint.created_at
                )
                    ...
                    if review_files or approval_files:
                        ...
                        if datetime.now() - file_time < timedelta(hours=24):
                            try:
                                ...
                                print(f"📋 Kanban item {task_dir.name}: reviewer={reviewer}, approved={approved}")
            ...
            if metadata and metadata.hitl_checkpoints:
                for checkpoint in metadata.hitl_checkpoints:
                    if checkpoint.checkpoint_type == checkpoint_type:
                        return (
                            checkpoint.assigned_reviewers[0]
    ...
    def display_board(
        self, show_completed: bool = False, max_completed: int = 5
    ) -> None:
        """
        ...
        if not show_completed:
            ...
            completed_items = [
                item
                for item in items
                if item.status in [ReviewStatus.COMPLETED, ReviewStatus.APPROVED]
            ]
            other_items = [
                item
                for item in items
        ...
        for item in items:
            ...
            deadline_style = (
                ...
                if item.overdue
                    ...
                    "yellow"
                    if item.time_remaining and "h" in item.time_remaining
                    else "green"
                )
            )
...
📈 Status Breakdown:"""
        ...
        self.console.print(
            Panel(summary_text, title="Board Summary", border_style="blue")
        )
