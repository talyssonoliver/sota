...
def get_gantt_data():
    ...
    try:
        ...
        include_completed = (
            request.args.get("include_completed", "true").lower() == "true"
        )
        ...
        logger.info(
            f"Successfully generated Gantt data with {len(gantt_data.get('tasks', []))} tasks"
        )
    ...
    try:
        ...
        critical_path_analysis = {
            ...
            "metrics": {
                ...
                "critical_path_duration": gantt_data.get("project_timeline", {}).get(
    ...
    try:
        ...
        categorized_recommendations = {
            "timeline_optimization": [
                rec for rec in recommendations if rec.get("category") == "timeline"
            ],
            "resource_optimization": [
                rec for rec in recommendations if rec.get("category") == "resource"
            ],
            "risk_mitigation": [
                rec for rec in recommendations if rec.get("category") == "risk"
            ],
            "quality_improvement": [
                rec for rec in recommendations if rec.get("category") == "quality"
            ],
        }
    ...
    try:
        ...
        if format_type.lower() == "json":
            ...
            return jsonify(
                {
                    "success": True,
                    "data": {"csv_data": csv_data, "format": "csv"},
                    "exported_at": datetime.now().isoformat(),
                }
            )
...
def extract_dependency_network(critical_tasks):
    ...
    for task in critical_tasks:
        for dep_id in task.get("dependencies", []):
            dependencies.append(
                {"from": dep_id, "to": task["id"], "type": "finish_to_start"}
            )
    ...
    for assignee, assignee_tasks in assignees.items():
        ...
        for task in assignee_tasks:
            ...
            start_date = datetime.fromisoformat(
                task["start_date"].replace("Z", "")
            ).strftime("%Y-%m-%d")
            end_date = datetime.fromisoformat(
                task["end_date"].replace("Z", "")
            ).strftime("%Y-%m-%d")

            mermaid_code += (
                f"    {task['name']} :{status}{task['id']}, {start_date}, {end_date}\n"
            )
