...
class WebhookEventData:
    ...
    @staticmethod
    def review_created(
        task_id: str, checkpoint_id: str, review_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    @staticmethod
    def review_approved(
        task_id: str, checkpoint_id: str, approval_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "data": {
                ...
                "completed_at": approval_data.get(
    ...
    @staticmethod
    def review_rejected(
        task_id: str, checkpoint_id: str, rejection_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "data": {
                ...
                "rejected_at": rejection_data.get(
    ...
    def escalation_triggered(
        task_id: str, escalation_data: Dict[str, Any]
    ) -> Dict[str, Any]:
...
class WebhookManager:
            ...
            if self.config_path.exists():
                ...
                for endpoint_data in config.get("endpoints", []):
                    endpoint = WebhookEndpoint(
                        ...
                        events=[
                            WebhookEventType(event) for event in endpoint_data["events"]
                        ],
                        ...
                        created_at=datetime.fromisoformat(
                            endpoint_data.get("created_at", datetime.now().isoformat())
                        ),
                    )
    ...
    def _setup_routes(self):
            ...
            try:
                ...
                if endpoint.secret:
                    ...
                    if not signature or not self._verify_signature(
                        request.data, endpoint.secret, signature
                    ):
        ...
        def create_endpoint():
            ...
            try:
                ...
                return (
                    jsonify({"status": "created", "endpoint_id": endpoint.id}),
                    201,
                )
    ...
    def _process_incoming_webhook(
        self, endpoint_id: str, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Process incoming webhook data."""
        self.logger.info(
            f"Processing incoming webhook from {endpoint_id}: {data.get('event', 'unknown')}"
        )
    ...
    def _handle_external_approval(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print(f"🔗 Processing external approval for task {task_id} by {reviewer}")
    ...
    def _handle_external_rejection(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print(f"❌ Processing external rejection for task {task_id} by {reviewer}: {reason}")

        try:
            # Mock HITLPolicyEngine for webhook handling
            result = {
                "status": "success", 
    ...
    def register_endpoint(self, endpoint: WebhookEndpoint) -> bool:
        ...
        try:
            ...
            self._save_config()
            self.logger.info(
                f"Registered webhook endpoint: {endpoint.name} ({endpoint.id})"
            )
            return True
        except Exception as e:
    ...
    def _create_delivery(
        ...
        delivery_id = (
            f"del_{int(time.time())}_{hash(endpoint.id)}_{len(self.deliveries)}"
        )
        ...
        for attempt in range(endpoint.max_retries + 1):
            try:
                ...
                if attempt < endpoint.max_retries:
                    ...
                    self.logger.warning(
                        f"Webhook delivery failed, retrying in {delay_seconds}s: {e}"
                    )
...
def send_review_created_webhook(
    task_id: str, checkpoint_id: str, review_data: Dict[str, Any]
):
    """Send review.created webhook."""
    event_data = WebhookEventData.review_created(task_id, checkpoint_id, review_data)
    return get_webhook_manager().send_webhook(
        WebhookEventType.REVIEW_CREATED, event_data, task_id
    )


def send_review_approved_webhook(
    task_id: str, checkpoint_id: str, approval_data: Dict[str, Any]
):
    """Send review.approved webhook."""
    event_data = WebhookEventData.review_approved(task_id, checkpoint_id, approval_data)
    return get_webhook_manager().send_webhook(
        WebhookEventType.REVIEW_APPROVED, event_data, task_id
    )


def send_review_rejected_webhook(
    task_id: str, checkpoint_id: str, rejection_data: Dict[str, Any]
):
    """Send review.rejected webhook."""
    event_data = WebhookEventData.review_rejected(
        task_id, checkpoint_id, rejection_data
    )
    return get_webhook_manager().send_webhook(
        WebhookEventType.REVIEW_REJECTED, event_data, task_id
    )
...
def send_escalation_webhook(task_id: str, escalation_data: Dict[str, Any]):
    ...
    return get_webhook_manager().send_webhook(
        WebhookEventType.ESCALATION_TRIGGERED, event_data, task_id
    )
