...
try:
    from src.infrastructure.utils.feedback_system import \
        get_feedback_system  # Add feedback system import
except ImportError:
    pass
try:
        ...
        for checkpoint in checkpoints:
            if hasattr(checkpoint, "to_dict"):
                ...
                for time_field in ["created_at", "timeout_at", "resolved_at"]:
                    if checkpoint_dict.get(time_field) and hasattr(
                        checkpoint_dict[time_field], "isoformat"
                    ):
                        checkpoint_dict[time_field] = checkpoint_dict[
                            time_field
                        ].isoformat()
...
def get_audit_trail():
    ...
    try:
        ...
        audit_entries = hitl_engine.get_audit_trail(
        ...
        for action in actions:
            ...
            try:
                if action_type == "approve":
                    success = hitl_engine.approve_checkpoint(
                        checkpoint_id, reviewer, notes
                    )
                elif action_type == "reject":
                    success = hitl_engine.reject_checkpoint(
                        checkpoint_id, reviewer, notes
                    )
