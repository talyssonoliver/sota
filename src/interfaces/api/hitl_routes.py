...
# Factory function for dashboard manager to enable proper testing mocks
def get_dashboard_manager():
    """Get HITLDashboardManager instance. Separated for mocking in tests."""
    return HITLDashboardManager()
...
def hitl_health():
    ...
    try:
        ...
        hitl_dashboard = get_dashboard_manager()
        widget_status = {}
...
def batch_approve_checkpoints():
    ...
    try:
        ...
        # Extract failed checkpoints for the test expectation
        failed_checkpoints = [result for result in results if result["status"] == "failed"]
