...
class ExternalSystemIntegration(ABC):
    ...
    async def create_review_request(
        self, request: ExternalReviewRequest
    ) -> Dict[str, Any]:
...
class GitHubIntegration(ExternalSystemIntegration):
    ...
    async def cancel_review_request(
        ...
        try:
            ...
            data = {"reviewers": request.reviewers}

            response = requests.delete(url, headers=headers, json=data, timeout=30)
            response.raise_for_status()

            return {"status": "cancelled", "response": response.json()}
...
class JIRAIntegration(ExternalSystemIntegration):
        ...
        try:
            ...
            issue_data = {
                "fields": {
                    ...
                    "assignee": (
                        {"name": request.reviewers[0]} if request.reviewers else None
                    ),
    ...
    async def cancel_review_request(
        ...
        try:
            ...
            return {"status": "cancelled", "response": "Issue closed"}
...
class ExternalAPIManager:
            ...
            if self.config_path.exists():
                ...
                for system_data in config.get("systems", []):
                    system_config = ExternalSystemConfig(
                        ...
                        rate_limit_per_hour=system_data.get(
    ...
    def _create_integration(
        self, config: ExternalSystemConfig
    ) -> Optional[ExternalSystemIntegration]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to create integration for {config.system_id}: {e}"
            )
            return None
    ...
    async def check_request_status(
        self, request_id: str
    ) -> Optional[ExternalReviewRequest]:
        ...
        try:
            ...
            if result["status"] == "approved":
                ...
                from src.interfaces.api.webhook_manager import \
    ...
    async def cancel_request(self, request_id: str) -> bool:
        ...
        try:
            ...
            if result["status"] == "cancelled":
                ...
                self.logger.error(
                    f"Failed to cancel external review request: {request_id}"
                )
                return False
            ...
            try:
                ...
                for request_id, request in list(self.requests.items()):
                    ...
                    if (
                        ...
                        self.logger.warning(
                            f"External review request timed out: {request_id}"
                        )
                        ...
                        try:
                            ...
                            self.logger.error(
                                f"Failed to cancel timed-out request: {e}"
                            )
    ...
    def list_requests(
        self, system_id: str = None, status: IntegrationStatus = None
    ) -> List[ExternalReviewRequest]:
    ...
    def get_system_stats(self, system_id: str) -> Dict[str, Any]:
        ...
        system_requests = [
            r for r in self.requests.values() if r.system_id == system_id
        ]

        total = len(system_requests)
        completed = len(
            [r for r in system_requests if r.status == IntegrationStatus.COMPLETED]
        )
        failed = len(
            [r for r in system_requests if r.status == IntegrationStatus.FAILED]
        )
        pending = len(
            [r for r in system_requests if r.status == IntegrationStatus.IN_PROGRESS]
        )
