...
class EscalationCLI:
    ...
    def list_active_escalations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for escalation in active_escalations:
                ...
                print(
                    f"Started: {escalation['triggered_at'].strftime('%Y-%m-%d %H:%M:%S')}"
                )
        ...
        try:
            ...
            for event in events:
                ...
                if event.resolved_at:
                    print(
                        f"Resolved: {event.resolved_at.strftime('%Y-%m-%d %H:%M:%S')}"
                    )
    ...
    def trigger_manual_escalation(
        self, task_id: str, reason: str, level: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            notification_result = self.engine.notifier.send_notification(
                event,
                escalation_level.recipients,
                escalation_level.notification_channels,
            )
    ...
    def show_escalation_metrics(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            if metrics["avg_resolution_time"]:
                print(
                    f"Average Resolution Time: {metrics['avg_resolution_time']:.2f} hours"
                )
            else:
    ...
    def resolve_escalation(self, task_id: str, resolution: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"Resolved at: {latest_event.resolved_at.strftime('%Y-%m-%d %H:%M:%S')}"
            )
...
def main():
    ...
    list_parser.set_defaults(func=lambda args: list_active_escalations())

    # Get escalation status
    status_parser = subparsers.add_parser(
        "status", help="Get escalation status for a task"
    )
    ...
    check_parser = subparsers.add_parser(
        "check", help="Check pending reviews for escalation"
    )
    check_parser.set_defaults(func=lambda args: check_pending_reviews())
...
def check_pending_reviews() -> Dict[str, Any]:
    """Check pending reviews (for testing)"""
    cli = EscalationCLI()
    return cli.check_pending_reviews()
