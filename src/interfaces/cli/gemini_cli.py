...
try:
    ...
    class AIMessage:  # type: ignore[misc]
        def __init__(self, content):
            self.content = content


try:
    from langgraph.prebuilt import create_react_agent  # type: ignore[assignment]
except ImportError:
    # Mock for testing
    def create_react_agent(*args, **kwargs):  # type: ignore[misc]
        class MockAgent:
            async def ainvoke(self, input):
                return {"messages": [AIMessage("Mock agent response")]}
...
class SystemIntegrationTool(BaseTool):
    ...
    description: str = (
        "Integrates with the AI Agent System for task execution and memory operations."
    )
...
class MemoryTool(BaseTool):
    ...
    description: str = (
        "Interacts with the ChromaDB-based memory engine for context retrieval and storage."
    )
...
class CodeAnalysisTool(BaseTool):
    ...
    description: str = (
        "Analyzes codebases and generates code following the project's architecture patterns."
    )
...
class GeminiCLI:
    ...
    async def chat_interactive(self):
        ...
        if not self.model:
            print(
                "❌ Gemini model not available. Check your API key and configuration."
            )
            return
            ...
            if self.model:
                ...
                if hasattr(response, "content"):
                    if isinstance(response.content, str):
                        ...
                        return "\n".join(str(item) for item in response.content)  # type: ignore[misc]
                    else:
                ...
                raise GeminiCLIError(
                    "Gemini model not initialized. Check your API key and configuration."
                )
