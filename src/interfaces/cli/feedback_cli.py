...
def main():
    ...
    summary_parser = subparsers.add_parser(
        "summary", help="Get feedback summary for a task"
    )
    ...
    report_parser.add_argument(
        "--period", default="7d", help="Time period (e.g., 7d, 30d)"
    )

    # Export command
    export_parser = subparsers.add_parser("export", help="Export feedback data")
    export_parser.add_argument(
        "format", choices=["json", "csv", "summary"], help="Export format"
    )
    export_parser.add_argument("output_path", help="Output file path")
    export_parser.add_argument(
        "--period", default="30d", help="Time period (e.g., 7d, 30d)"
    )

    # Capture command
    capture_parser = subparsers.add_parser(
        "capture", help="Capture feedback interactively"
    )
