...
class HITLCLIManager:
    ...
    def list_pending_checkpoints(
        self, task_id: Optional[str] = None, reviewer: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if task_id and checkpoint_list:
                checkpoint_list = [
                    cp for cp in checkpoint_list if cp.get("task_id") == task_id
                ]

            # Filter by reviewer if specified
            if reviewer and checkpoint_list:
                checkpoint_list = [
                    cp for cp in checkpoint_list if reviewer in cp.get("reviewers", [])
                ]
    ...
    def show_checkpoint_details(self, checkpoint_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "deadline": (
                    checkpoint.deadline.isoformat() if checkpoint.deadline else None
                ),
    ...
    def approve_checkpoint(
        self, checkpoint_id: str, reviewer: str, comments: str = ""
    ) -> bool:
    ...
    def escalate_checkpoint(
        ...
        try:
            ...
            reviewer = validate_reviewer_name(reviewer)
            reason = validate_string_content(reason, max_length=1000)
            escalation_level = validate_integer_range(
                escalation_level, min_val=1, max_val=5
            )
    ...
    def get_reviewer_stats(self, reviewer: str, days: int = 30) -> Dict[str, Any]:
        """Get statistics for a specific reviewer."""
        try:
            reviewer = validate_reviewer_name(reviewer)
            
            # Get all checkpoints for the time period
            checkpoints = self.hitl_engine.get_pending_checkpoints()
            
            # Filter by reviewer and time period
            reviewer_checkpoints = []
            for cp in checkpoints:
                if hasattr(cp, 'to_dict'):
                    cp_dict = cp.to_dict()
                else:
                    cp_dict = cp if isinstance(cp, dict) else vars(cp)
                
                if reviewer in cp_dict.get('reviewers', []):
                    reviewer_checkpoints.append(cp_dict)
            
            # Calculate stats
            total_reviews = len(reviewer_checkpoints)
            approved = sum(1 for cp in reviewer_checkpoints if cp.get('status') == 'approved')
            rejected = sum(1 for cp in reviewer_checkpoints if cp.get('status') == 'rejected')
            pending = sum(1 for cp in reviewer_checkpoints if cp.get('status') == 'pending')
            ...
            self.logger.error(f"Failed to get reviewer stats: {e}")
            return {}

    def import_checkpoint_data(self, input_file: str) -> bool:
        ...
        try:
            ...
            with open(input_path, 'r') as f:
                if input_path.suffix.lower() == '.json':
                    data = json.load(f)
                elif input_path.suffix.lower() in ['.yaml', '.yml'] and YAML_AVAILABLE:
            ...
            if not isinstance(data, dict):
                self.logger.error("Invalid checkpoint data format")
                return False
            
            checkpoint_id = data.get('checkpoint_id')
            if not checkpoint_id:
                self.logger.error("Missing checkpoint_id in imported data")
                return False
            
            # Note: In a real implementation, you would recreate the checkpoint
            # For now, we just log the successful import
            self.logger.info(f"Successfully imported checkpoint data for {checkpoint_id}")
            return True
            
        except Exception as e:
            self.logger.error(f"Failed to import checkpoint data: {e}")
            return False

    def validate_checkpoint_integrity(self, checkpoint_id: str) -> Dict[str, Any]:
        """Validate the integrity of checkpoint data."""
        try:
            checkpoint_id = validate_checkpoint_id(checkpoint_id)
            checkpoint = self.hitl_engine.get_checkpoint(checkpoint_id)
            ...
            if not checkpoint.checkpoint_type:
                errors.append('Missing checkpoint_type')
            if not checkpoint.reviewers:
                warnings.append('No reviewers assigned')
            
            # Validate data consistency
            if checkpoint.deadline and checkpoint.created_at:
                if checkpoint.deadline < checkpoint.created_at:
                    errors.append('Deadline is before creation time')
            ...
            self.logger.error(f"Failed to validate checkpoint integrity: {e}")
            return {
                'valid': False,
...
def cmd_list_checkpoints(args):
    ...
    checkpoints = cli_manager.list_pending_checkpoints(
        task_id=args.task_id, reviewer=args.reviewer
    )
...
def cmd_approve_checkpoint(args):
    ...
    success = cli_manager.approve_checkpoint(
        args.checkpoint_id, args.reviewer, args.comments or ""
    )
...
def cmd_reject_checkpoint(args):
    ...
    success = cli_manager.reject_checkpoint(
        args.checkpoint_id, args.reviewer, args.reason, args.comments or ""
    )
...
def cmd_escalate_checkpoint(args):
    ...
    success = cli_manager.escalate_checkpoint(
        args.checkpoint_id, args.reviewer, args.reason, args.level
    )
...
def cmd_audit_trail(args):
    ...
    audit_entries = cli_manager.show_audit_trail(
        task_id=args.task_id, checkpoint_id=args.checkpoint_id
    )
...
Examples:
  ...
  python cli/hitl_cli.py audit-trail --task-id BE-07  # Show audit trail for task
    ...
    escalate_parser.add_argument(
        "--level", type=int, default=1, help="Escalation level"
    )
    escalate_parser.add_argument(
        "--force", action="store_true", help="Skip confirmation"
    )
