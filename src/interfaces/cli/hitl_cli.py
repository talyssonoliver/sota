...
def validate_checkpoint_id(checkpoint_id: str) -> str:
    ...
    if not checkpoint_id or not checkpoint_id.startswith('hitl_'):
...
def validate_reviewer_name(reviewer: str) -> str:
    ...
    if not reviewer or len(reviewer.strip()) == 0:
        raise ValueError("Reviewer name cannot be empty")
    if len(reviewer) > 100:
        raise ValueError("Reviewer name too long (max 100 characters)")
...
def handle_list_command(args) -> int:
    ...
    try:
        ...
        checkpoints = cli_manager.list_pending_checkpoints(task_id=task_id, reviewer=reviewer)
        ...
        for cp in checkpoints:
            print(f"  - {cp.get('checkpoint_id', 'Unknown ID')}")
...
class HITLCLIManager:
    ...
    def get_reviewer_stats(self, reviewer: str, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            # Get checkpoints for the reviewer in the specified time period
            all_checkpoints = self.hitl_engine.get_pending_checkpoints()
            
            # Filter by reviewer and time period (mock implementation)
            reviewer_checkpoints = [
                cp for cp in all_checkpoints 
                if hasattr(cp, 'reviewers') and reviewer in getattr(cp, 'reviewers', [])
            ]
            
            # Calculate statistics
            total_reviews = len(reviewer_checkpoints)
            approved = sum(1 for cp in reviewer_checkpoints 
                          if getattr(cp, 'status', '') == 'approved')
            rejected = sum(1 for cp in reviewer_checkpoints 
                          if getattr(cp, 'status', '') == 'rejected')
            pending = total_reviews - approved - rejected
    ...
    def import_checkpoint_data(self, file_path: str) -> bool:
        """Import checkpoint data from a file."""
        try:
#             import json  # Consolidated to common_imports
            ...
            # Mock implementation - just validate the data structure
            if isinstance(data, dict) and 'checkpoints' in data:
                self.logger.info(f"Successfully imported checkpoint data from {file_path}")
                return True
            else:
                self.logger.error(f"Invalid checkpoint data format in {file_path}")
                return False
    ...
    def validate_checkpoint_integrity(self, checkpoint_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not hasattr(checkpoint, 'task_id') or not checkpoint.task_id:
                errors.append("Missing task_id")
            
            if not hasattr(checkpoint, 'checkpoint_type') or not checkpoint.checkpoint_type:
                errors.append("Missing checkpoint_type")
            
            if not hasattr(checkpoint, 'created_at') or not checkpoint.created_at:
