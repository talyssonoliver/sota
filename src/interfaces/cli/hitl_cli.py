...
class HITLCLIManager:
    ...
    def get_reviewer_stats(self, reviewer: str, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            approved = sum(
                1 for cp in reviewer_checkpoints if cp.get("status") == "approved"
            )
            rejected = sum(
                1 for cp in reviewer_checkpoints if cp.get("status") == "rejected"
            )
            pending = sum(
                1 for cp in reviewer_checkpoints if cp.get("status") == "pending"
            )
    ...
    def import_checkpoint_data(self, input_file: str) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Successfully imported checkpoint data for {checkpoint_id}"
            )
            return True
