...
def check_feedback_logs() -> List[Dict[str, Any]]:
    ...
    for task_dir in outputs_dir.iterdir():
        ...
        review_files = list(task_dir.glob("*review*.json")) + list(
            task_dir.glob("*approval*.json")
        )
        ...
        for review_file in review_files:
            if review_file.stat().st_mtime > recent_cutoff:
                try:
                    ...
                    print(f"📋 Task {task_dir.name} reviewed by {reviewer}: {'✅' if approved else '❌'}")
...
def display_status_simple(items: List[Dict[str, Any]]):
    ...
    print("=" * 85)
    print(
        f"{'Task ID':<8} {'Status':<15} {'Reviewer':<18} {'Deadline':<12} {'Action':<10}"
    )
    print("-" * 85)
    ...
    for item in items:
        ...
        status = item["status"]
        reviewer = (
            item["pending_reviewer"][:17]
            if len(item["pending_reviewer"]) > 17
            else item["pending_reviewer"]
        )
        deadline = (
            item["deadline"][:11] if len(item["deadline"]) > 11 else item["deadline"]
        )
        action = item["action"]
    ...
    pending = len(
        [
            item
            for item in items
            if item["status"] in ["Awaiting QA", "Awaiting Human", "In Review"]
        ]
    )
    safe_print(
        f"Summary: Total: {total} | Pending: {pending} | Overdue: {overdue} | High Priority: {high_priority}"
    )
    ...
    output = {
        ...
        "summary": {
            ...
            "high_priority_count": len(
                [item for item in items if item["priority"] >= 6]
            ),
        },
        "items": items,
    }
