...
try:
    ...
    pass
from ..config.exceptions import MemoryEngineError
import logging
    ...
    def add_document(self, file_path: str, user: str = "system", 
        ...
        with self._document_lock:
            try:
                ...
                result = self._engine.add_document(file_path, user, content_type, metadata)
        ...
        with self._read_write_lock:
            try:
                ...
                result = self._engine.get_context(
                    query, k, user, similarity_threshold, context_domains, metadata_filter
                )
    ...
    def get_context_by_keys(self, keys: List[str], user: str = "system") -> List[str]:
        ...
        with self._read_write_lock:
            try:
                ...
                logger.debug(f"Context retrieved by keys thread-safely: {len(keys)} keys")
                return result
        ...
        with self._read_write_lock:
            try:
                ...
                result = self._engine.build_focused_context(
                    context_topics, max_tokens, max_per_topic, user, task_id, **kwargs
                )
                
                logger.debug(f"Focused context built thread-safely for {len(context_topics)} topics")
                return result
        ...
        with self._stats_lock:
            with self._read_write_lock:
                try:
                    ...
                    return {'error': str(e), 'thread_safety': {'enabled': True, 'error': True}}
...
class ThreadSafeContextManager:
    ...
    def __init__(self, memory_engine: ThreadSafeMemoryEngine, operation_type: str = "read"):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is None:
            self.success = True
            logger.debug(f"Thread-safe {self.operation_type} operation completed successfully in {duration:.3f}s")
        else:
            logger.error(f"Thread-safe {self.operation_type} operation failed after {duration:.3f}s: {exc_val}")
