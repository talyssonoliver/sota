...
class SecurityManager:
    ...
    def _load_or_generate_key(self) -> bytes:
        ...
        if os.environ.get("ENVIRONMENT", "").lower() == "production":
            logger.error(
                "MEMORY_ENGINE_KEY environment variable is required in production"
            )
        ...
        logger.warning(
            "No MEMORY_ENGINE_KEY found in environment. Generating temporary key for development."
        )
        logger.warning(
            "SECURITY WARNING: This key is NOT persistent and will change on restart!"
        )
        logger.warning(
            "For production or persistent development, set MEMORY_ENGINE_KEY in your .env file."
        )
...
class AccessControlManager:
    ...
    def _initialize_default_permissions(self):
        ...
        self.role_permissions["user"] = {"read", "write"}
        self.role_permissions["readonly"] = {"read"}
...
class AuditLogger:
    ...
    def log_access_attempt(self, user: str, resource: str, success: bool):
        """Log access attempt"""
        self.log_event(
            "access_attempt", user, {"resource": resource, "success": success}
        )
