...
try:
    ...
    class MockPsutil:
        def virtual_memory(self):
            return type(
                "MockMemory", (), {"available": 1024 * 1024 * 1024}
            )()  # 1GB mock

        def disk_usage(self, path):
            return type(
                "MockDisk", (), {"free": 10 * 1024 * 1024 * 1024}
            )()  # 10GB mock
...
class TieredStorageManager:
    ...
    def store_data(self, key: str, data: bytes, metadata: Optional[Dict] = None) -> str:
        ...
        with self.lock:
            ...
            if (
                size_mb < 10
                and self._get_hot_storage_usage() < self.hot_storage_limit_mb
            ):
                tier = "hot"
                storage_path = self.hot_path
            elif (
                size_mb < 100
                and self._get_warm_storage_usage() < self.warm_storage_limit_gb * 1024
            ):
                tier = "warm"
    ...
    def _promote_data(self, key: str) -> bool:
        ...
        if (
            target_tier == "hot"
            and self._get_hot_storage_usage() + size_mb > self.hot_storage_limit_mb
        ):
            return False
        if (
            target_tier == "warm"
            and self._get_warm_storage_usage() + size_mb
            > self.warm_storage_limit_gb * 1024
        ):
            return False
        ...
        try:
            ...
            safe_key = meta.get(
                "safe_key",
                key.replace(os.sep, "_").replace("/", "_").replace("\\", "_"),
            )
    ...
    def migrate(self) -> Dict[str, int]:
        ...
        with self.lock:
            ...
            if hot_usage > self.hot_storage_limit_mb:
                ...
                hot_data = [
                    (k, v)
                    for k, v in self.storage_metadata.items()
                    if v["tier"] == "hot"
                ]
    ...
    def _get_hot_storage_usage(self) -> float:
        ...
        try:
            total_size = sum(
                meta["size_bytes"]
                for meta in self.storage_metadata.values()
                if meta["tier"] == "hot"
            )
    ...
    def get_storage_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            stats = {
                ...
                "hot_items": sum(
                    1 for m in self.storage_metadata.values() if m["tier"] == "hot"
                ),
                "warm_items": sum(
                    1 for m in self.storage_metadata.values() if m["tier"] == "warm"
                ),
                "cold_items": sum(
                    1 for m in self.storage_metadata.values() if m["tier"] == "cold"
                ),
                "hot_usage_mb": self._get_hot_storage_usage(),
                "warm_usage_mb": self._get_warm_storage_usage(),
                "total_size_mb": sum(
                    m["size_bytes"] for m in self.storage_metadata.values()
                )
                / (1024 * 1024),
            }
