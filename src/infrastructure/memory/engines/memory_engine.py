...
# Import platform module normally
try:
    import platform
except ImportError:
    pass
...
from ..config.exceptions import MemoryEngineError, SecurityError
try:
    from ..security.encryption import SecurityManager, AccessControlManager, AuditLogger
except ImportError:
...
class MemoryEngine:
    ...
    def __init__(self, config: Optional[Any] = None):
        ...
        self.security_manager = SecurityManager(self.config) if SecurityManager else None
        self.access_control = AccessControlManager(self.config) if AccessControlManager else None
        self.audit_logger = AuditLogger(self.config) if AuditLogger else None
        ...
        if self.config and getattr(self.config, 'enable_tiered_storage', False):
            ...
            self.partition_manager = None  # Not available
        else:
            self.storage_manager = None
            self.partition_manager = None
        
        self.chunker = ChunkProcessor() if ChunkProcessor else None
        self.adaptive_chunker = ChunkProcessor() if ChunkProcessor else None
    ...
    def get_relevant_context(self, query: str, k: int = 5, user: str = "system",
                           similarity_threshold: Optional[float] = None,
                           context_domains: Optional[List[str]] = None,
                           metadata_filter: Optional[Dict[str, Any]] = None) -> Union[str, List[str]]:
        """
        ...
        Returns:
            Relevant context as string or list of strings
        """
        return self.get_context(query, k, user, similarity_threshold, context_domains, metadata_filter)
