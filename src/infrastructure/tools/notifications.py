...
if not logger.handlers:
    ...
    if jsonlogger:
        formatter = jsonlogger.JsonFormatter(
            '%(asctime)s %(levelname)s %(name)s %(message)s %(event)s %(agent)s %(task_id)s')
    else:
...
class NotificationLevel(Enum):
    ...
    ERROR = "error"
    CRITICAL = "critical"
    # Extended levels for workflow notifications
    ALL = "all"
...
class SlackNotifier:
    ...
    def __init__(
        ...
        if not self.webhook_url and notification_level != NotificationLevel.NONE:
            logger.warning(
                "Slack webhook URL not provided. Notifications will be logged but not sent.")
    ...
    def should_notify(self, event_type: str, state: Dict[str, Any]) -> bool:
        ...
        if self.notification_level == NotificationLevel.COMPLETION:
            return event_type == "node_end" and state.get("status") == "DONE"

        return False

    def format_message(self, event_type: str, node_id: str,
    ...
    def send_notification(self,
        ...
        # Send to Slack if webhook URL is available and requests is available
            ...
            try:
                ...
                if response.status_code != 200:
                    logger.error(
                        ...
                        extra={"event": "notification_error"})
                    return False
    ...
    def state_change_handler(
            self, old_state: Dict[str, Any], new_state: Dict[str, Any]) -> None:
        """
...
def attach_notifications_to_workflow(workflow: Union[Graph, StateGraph] = None,
    ...
    logger.info(
        f"Attached notifications to workflow with level: {notification_level}")

    # Return the workflow unchanged for now
    return workflow

__all__ = ["NotificationManager", "NotificationLevel", "SlackNotifier", "attach_notifications_to_workflow"]
