...
from src.infrastructure.tools.validation.persistence.validation_history import ValidationHistoryTracker
...
def print_validation_summary(report: Dict[str, Any]):
    ...
    if trends:
        ...
        if success_trend:
            print(f"   Success Rate: {success_trend.get('current', 0):.1%} ({success_trend.get('trend', 'stable')})")
        
        time_trend = trends.get("average_time", {})
        if time_trend:
            print(f"   Avg Time: {time_trend.get('current', 0):.1f}s ({time_trend.get('trend', 'stable')})")
        
        issue_trend = trends.get("issue_count", {})
        if issue_trend:
            print(f"   Avg Issues: {issue_trend.get('current', 0):.1f} ({issue_trend.get('trend', 'stable')})")
...
def print_metrics(tracker: ValidationHistoryTracker):
    ...
    if phase_stats:
        ...
        for phase, stats in sorted(phase_stats.items()):
            ...
            print(f"   {phase.title():<15}: {avg_time:>6.1f}s avg ({run_count} runs, {total_time:.1f}s total)")
    ...
    if git_stats:
        ...
        if branches:
            ...
            for branch, count in sorted(branches.items(), key=lambda x: x[1], reverse=True)[:5]:
...
Examples:
  ...
  python validation_report.py --cleanup --days 90
    ...
    parser.add_argument(
        "--summary", "-s",
        action="store_true",
        help="Show validation summary report"
    )
    
    parser.add_argument(
        "--trends", "-t",
        action="store_true", 
        help="Show detailed trend analysis"
    )
    
    parser.add_argument(
        "--metrics", "-m",
        action="store_true",
        help="Show all-time validation metrics"
    )
    
    parser.add_argument(
        "--cleanup", "-c",
        action="store_true",
        help="Clean up old validation data"
    )
    ...
    try:
        if args.cleanup:
            cleanup_days = args.days if args.days != 7 else 90  # Use 90 as default for cleanup
