...
class ValidationHistoryTracker:
    ...
    def record_validation_result(
        ...
        git_info = self._get_git_info() if git_commit is None else {"commit": git_commit}
    ...
    def _calculate_trends(self) -> Dict[str, Any]:
        ...
        trends["success_rate"]["current"] = success_count / len(recent_records) if recent_records else 0
        
        # Calculate average time trend
        total_times = [r["total_time"] for r in recent_records]
        trends["average_time"]["current"] = sum(total_times) / len(total_times) if total_times else 0
        
        # Calculate issue count trend
        issue_counts = [r["issues_summary"]["total_issues"] for r in recent_records]
        trends["issue_count"]["current"] = sum(issue_counts) / len(issue_counts) if issue_counts else 0
        
        # Analyze phase performance
        for phase in ["syntax", "dependencies", "structure", "performance", "ai_patterns"]:
        ...
        trends["most_common_issues"] = [
            ...
            for issue, freq in sorted(issue_frequency.items(), key=lambda x: x[1], reverse=True)[:10]
        ]
        
        # Generate improvement suggestions
        trends["improvement_suggestions"] = self._generate_improvement_suggestions(recent_records)
    ...
    def _calculate_trend_direction(self, data_points: List[float]) -> str:
        ...
        older_avg = sum(data_points[:-5]) / max(1, len(data_points) - 5) if len(data_points) > 5 else recent_avg
    ...
    def _generate_improvement_suggestions(self, records: List[Dict]) -> List[str]:
        ...
        for phase, times in avg_times.items():
            ...
            if avg_time > 60:  # More than 1 minute
                suggestions.append(f"Consider optimizing {phase} validation phase (avg: {avg_time:.1f}s)")
        ...
        if issue_categories.get("dependencies", 0) > len(records) * 5:
            suggestions.append("High dependency issues detected - consider dependency cleanup")
        
        if issue_categories.get("ai_analysis", 0) > len(records) * 10:
            suggestions.append("Many code quality issues found - consider refactoring effort")
    ...
    def _calculate_aggregated_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "average_duration": sum(r["total_time"] for r in all_records) / total_runs if total_runs > 0 else 0,
            "total_issues_found": sum(r["issues_summary"]["total_issues"] for r in all_records),
            "first_run": min(r["timestamp"] for r in all_records) if all_records else None,
            "last_run": max(r["timestamp"] for r in all_records) if all_records else None,
            "phase_statistics": {},
            "issue_statistics": {},
            "git_statistics": {}
        }
        ...
        # Phase statistics
        phase_stats = defaultdict(lambda: {"total_time": 0, "run_count": 0, "avg_time": 0})
        ...
        for phase, stats in phase_stats.items():
            stats["avg_time"] = stats["total_time"] / stats["run_count"] if stats["run_count"] > 0 else 0
    ...
    def get_validation_report(self, days: int = 7) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "success_rate": len(successful_runs) / len(recent_records) if recent_records else 0,
                "average_duration": sum(r["total_time"] for r in recent_records) / len(recent_records) if recent_records else 0
            },
    ...
    def cleanup_old_records(self, days_to_keep: int = 90):
        ...
        try:
            ...
            print(f"📅 Cleaned validation history: kept {records_kept} records, removed {records_removed} old records")
    ...
    def _rotate_if_needed(self):
        ...
        try:
            print(f"📦 Rotating validation history log ({current_size / 1024 / 1024:.1f}MB)")
            ...
            print("📦 Log rotation complete. Old log saved as validation_history.jsonl.1")
