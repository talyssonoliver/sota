...
class ToolInstaller:
    ...
    def install_missing_tools(self, tool_status: Dict[str, bool], auto_install: bool = False) -> bool:
        ...
        if not auto_install:
            response = input("\n❓ Would you like to install missing tools automatically? (y/n): ").lower()
            if response not in ['y', 'yes']:
                print("⚠️  Skipping tool installation. Some validations may not work properly.")
                return False
    ...
    def _install_tool(self, tool_name: str) -> bool:
        ...
        if not package:
            print(f"⚠️  {tool_name} requires manual installation. Please install it manually.")
            if tool_name == "sonar-scanner":
                print("   📖 See: https://docs.sonarqube.org/latest/analysis/scan/sonarscanner/")
            return False
