...
try:
    ...
    # Create a mock ArtesanatoBaseTool for when it's not available
    class ArtesanatoBaseTool(BaseTool):
        """Mock ArtesanatoBaseTool for when base_tool is not available."""
...
class EchoTool(ArtesanatoBaseTool):
    ...
    name: str = "echo_tool"
    description: str = (
        "A simple tool that echoes back the input. Use this when you need to test if the tool system is working."
    )
