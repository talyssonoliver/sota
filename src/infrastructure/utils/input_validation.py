...
class InputValidator:
    ...
    def __init__(self):
        ...
        self.checkpoint_id_pattern = re.compile(
            r"^hitl_[A-Z]{2,4}-\d{1,4}_[a-zA-Z0-9]{8,}$"
        )
        ...
        self.dangerous_compiled_patterns = [
    ...
    def validate_file_path(
        ...
        is_temp_file = (
            ...
            or file_path.startswith("/var/folders/")
            or "temp" in file_path.lower()
            or "tmp" in file_path.lower()
        )
        ...
        if not is_temp_file:
            for pattern in self.dangerous_compiled_patterns:
                if pattern.search(file_path):
                    raise ValidationError(
                        f"Dangerous pattern detected in file path: {file_path}"
                    )
...
#         import tempfile  # Consolidated to common_imports
    ...
    def validate_json_data(
        self, data: Union[str, Dict, List], max_size: int = MAX_JSON_SIZE
    ) -> Dict:
        """
    ...
    def validate_yaml_data(
        self, data: Union[str, Dict], max_size: int = MAX_JSON_SIZE
    ) -> Dict:
        """
    ...
    def validate_integer_range(
        self, value: Union[str, int], min_val: int = 0, max_val: int = 1000000
    ) -> int:
        """
        ...
        if int_value < min_val or int_value > max_val:
            raise ValidationError(
                f"Integer out of range: {int_value} (must be {min_val}-{max_val})"
            )
                ...
                if isinstance(value, str):
                    try:
                        setattr(
                            args,
                            attr_name,
                            str(self.validate_file_path(value, must_exist=False)),
                        )
                    except ValidationError as e:
                        raise ValidationError(f"Invalid {attr_name}: {e}")
            ...
            # Validate string content
                ...
                try:
                    setattr(
                        args,
                        attr_name,
                        self.validate_string_content(value, max_length=1000),
                    )
                except ValidationError as e:
                    raise ValidationError(f"Invalid {attr_name}: {e}")
            ...
            # Validate integers
                ...
                try:
                    setattr(
                        args,
                        attr_name,
                        self.validate_integer_range(value, min_val=0, max_val=10),
                    )
                except ValidationError as e:
                    raise ValidationError(f"Invalid {attr_name}: {e}")
