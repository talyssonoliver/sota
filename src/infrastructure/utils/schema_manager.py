...
class SchemaManager:
    ...
    def __init__(self):
        ...
        self.schema_configs = {
            "ai_agent_task": SchemaConfiguration(
                schema_path="src/core/tasks/task-schema.json",
                schema_type="task",
                relative_from="tasks",
            ),
        ...
        for file_path in yaml_files:
            try:
                self._process_single_file(
                    file_path, config, force_update, validate_content
                )
    ...
    def _process_single_file(
        ...
        if target_directive in content:
            logger.debug(
                f"Skipping {file_path.name} (already has correct schema directive)"
            )
        ...
        if has_existing_directive and not force_update:
            logger.debug(
                f"Skipping {file_path.name} (has different schema directive, use --force to update)"
            )
            self.stats["skipped"] += 1
            return
        ...
        if has_existing_directive and force_update:
            ...
            new_content = re.sub(existing_directive_pattern, target_directive, content)
    ...
    def _compare_schemas(self, schema1: Dict, schema2: Dict) -> List[str]:
        ...
        for field in common_fields:
            ...
            if type1 != type2:
                conflicts.append(
                    f"Field '{field}' has different types: {type1} vs {type2}"
                )
            ...
            if enum1 and enum2 and set(enum1) != set(enum2):
                conflicts.append(
                    f"Field '{field}' has different enum values: {enum1} vs {enum2}"
                )
    ...
    try:
        ...
        if args.analyze_conflicts:
            ...
            if conflicts["incompatible_fields"]:
                ...
                for comparison, conflicts_list in conflicts[
                    "incompatible_fields"
                ].items():
                    print(f"  {comparison}:")
