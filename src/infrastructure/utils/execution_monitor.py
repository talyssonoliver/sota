...
class ExecutionMonitor:
    ...
    def complete_agent_execution(
        self, execution_data, status=None, output=None, result=None, error=None
    ):
    ...
    def get_execution_stats(self, task_id: Optional[str] = None):
        ...
        if task_id:
            ...
            task_executions = [
                e for e in self.executions if e.get("task_id") == task_id
            ]
        else:
        ...
        failed = len(
            [e for e in task_executions if e.get("status") in ["FAILED", "ERROR"]]
        )
    ...
    def get_system_status(self):
        ...
        """
        running_executions = [
            e for e in self.executions if e.get("status") == "running"
        ]
        completed_executions = [
            e for e in self.executions if e.get("status") in ["completed", "COMPLETED"]
        ]
...
class DashboardLogger:
    ...
    def _update_agent_status(self, agent: str, status: str, task_id: str):
        ...
        active_agents = len(
            [a for a in data["agents"].values() if a["status"] == "active"]
        )
