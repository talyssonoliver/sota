...
try:
    ...
    pass
try:
    from src.core.workflows.hitl.policy_engine import HITLPolicyEngine as HITLEngine
    from src.infrastructure.utils.feedback_system import (FeedbackCategory,
                                                          FeedbackEntry,
                                                          FeedbackSystem)
...
class EscalationRule:
    ...
    def __post_init__(self):
        ...
        if self.task_type not in valid_task_types:
            raise ValueError(
                f"Invalid task type: {self.task_type}. Must be one of {valid_task_types}"
            )

        valid_risk_levels = ["low", "medium", "high", "critical"]
        if self.risk_level not in valid_risk_levels:
            raise ValueError(
                f"Invalid risk level: {self.risk_level}. Must be one of {valid_risk_levels}"
            )
...
class EscalationNotifier:
        ...
        for channel in channels:
            try:
                if channel in self.notification_channels:
                    ...
                    method_name = (
                        f"_send_{channel}"
                        if channel not in ["dashboard", "phone", "emergency"]
                        else f"_send_{channel}_notification"
                    )
                    ...
                    if success:
                        ...
                        event.notifications_sent.append(
                            f"{channel}_{datetime.now().isoformat()}"
                        )
                    else:
    ...
    def render_template(self, template_name: str, context: Dict[str, Any]) -> str:
        ...
        template = self.templates.get(
            template_name, self.templates["default_escalation"]
        )
        try:
    ...
    def _send_email(
        self, content: str, recipients: List[str], event: EscalationEvent
    ) -> bool:
...
class EscalationTracker:
        ...
        if self.escalations_file.exists():
            try:
                with open(self.escalations_file, "r") as f:
                    data = json.load(f)
                    return [
                        EscalationEvent.from_dict(event_data) for event_data in data
                    ]
            except Exception as e:
    ...
    def calculate_metrics(self, days: int = 7) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "timeout_escalations": len(
                [e for e in recent_events if e.trigger == "timeout"]
            ),
            "rejection_escalations": len(
                [e for e in recent_events if e.trigger == "rejection"]
            ),
            "level_1_escalations": len(
                [e for e in recent_events if e.level == EscalationLevel.LEVEL_1]
            ),
            "level_2_escalations": len(
                [e for e in recent_events if e.level == EscalationLevel.LEVEL_2]
            ),
            "level_3_escalations": len(
                [e for e in recent_events if e.level == EscalationLevel.LEVEL_3]
            ),
            "level_4_escalations": len(
                [e for e in recent_events if e.level == EscalationLevel.LEVEL_4]
            ),
...
class EscalationTimer:
    ...
    def start(self):
        ...
        self.active = True
        logging.info(
            f"Started escalation timer for {self.task_id}: {self.timeout_hours}h"
        )
...
class EscalationPolicy:
    ...
    def get_escalation_rule(
        self, task_type: str, risk_level: str
...
class EscalationEngine:
    ...
    def handle_timeout_escalation(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        level = self.determine_escalation_level(
            task_type, risk_level, "timeout", task_data
        )
    ...
    def handle_rejection_escalation(
        self, rejection_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def determine_escalation_level(
        ...
        if not rule:
            return (
                EscalationLevel.LEVEL_1
            )  # For timeout triggers, use escalation path based on duration
        if trigger == "timeout":
        ...
        if trigger == "rejection":
            ...
            if rejection_count >= 3:
                ...
                if risk_level == "critical":
                    ...
                    level_index = min(
                        rejection_count - 1, len(rule.escalation_path) - 1
                    )
    ...
    def schedule_escalation_timer(self, task_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            timer_id = f"{task_id}_{random.randint(1000000, 9999999)}"
    ...
    def capture_escalation_feedback(self, event: EscalationEvent):
        ...
        try:
            ...
            from src.infrastructure.utils.feedback_system import FeedbackEntry
            ...
            feedback_entry = FeedbackEntry(
                ...
                comments=[
                    f"Escalation triggered: {event.trigger} at level {event.level.config['name']}"
                ],
