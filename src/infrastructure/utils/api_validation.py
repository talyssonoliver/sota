...
from src.infrastructure.utils.input_validation import InputValidator, ValidationError, get_validator
...
class APIValidator:
    ...
    def validate_json_payload(self, payload: Dict[str, Any], 
        ...
        try:
            ...
            if schema:
                validated_payload = self._validate_against_schema(validated_payload, schema)
        ...
        for key, value in params.items():
            try:
                ...
                if isinstance(value, str):
                    clean_value = self.validator.validate_string_content(value, max_length=1000)
                    ...
                    clean_value = self.validator.validate_string_content(str(value), max_length=1000)
        ...
        for key, value in params.items():
            try:
                if 'task_id' in key.lower():
                    ...
                    validated_params[key] = self.validator.validate_string_content(value, max_length=100)
                else:
                    validated_params[key] = self.validator.validate_string_content(value, max_length=500)
    ...
    def validate_headers(self, headers: Dict[str, str]) -> Dict[str, str]:
        ...
        for key, value in headers.items():
            try:
                ...
                clean_value = self.validator.validate_string_content(value, max_length=2000)
    ...
    def _validate_against_schema(self, payload: Dict[str, Any], 
    ...
    def _validate_field(self, field_name: str, value: Any, spec: Dict[str, Any]) -> Any:
        ...
        if field_type == 'string':
            ...
            return self.validator.validate_string_content(str(value), max_length=max_length)
        
        elif field_type == 'integer':
            min_val = spec.get('minimum', -2147483648)
            max_val = spec.get('maximum', 2147483647)
            return self.validator.validate_integer_range(value, min_val=min_val, max_val=max_val)
