...
Examples:
    ...
    parser.add_argument("task_id", nargs="?", help="Task ID to archive (e.g., BE-07)")
    ...
    parser.add_argument(
        "--list", action="store_true", help="List all available archives"
    )

    parser.add_argument("--extract", type=Path, help="Extract specified archive")
    ...
    if args.list:
        ...
        if archives:
            ...
            for archive in archives:
                ...
                print(
                    f"   {archive['filename']} ({size_mb:.1f} MB) - {archive['created']}"
                )
        else:
