...
"""
        ...
        register_script = os.path.join(
            project_root, "orchestration", "register_output.py"
        )
        ...
        if os.path.exists(extract_script):
            cmd = [sys.executable, extract_script, test_task_id, agent_id]
            success, stdout, stderr = run_command(cmd, cwd=project_root)
        else:
        ...
        summarise_script = os.path.join(
            project_root, "orchestration", "summarise_task.py"
        )
        if os.path.exists(summarise_script):
            cmd = [sys.executable, summarise_script, test_task_id]
            success, stdout, stderr = run_command(cmd, cwd=project_root)
        else:
            # Generate summary
...
"""
        ...
        status_script = os.path.join(
            project_root, "orchestration", "update_task_status.py"
        )
        if os.path.exists(status_script):
            cmd = [sys.executable, status_script, test_task_id, "DONE"]
            success, stdout, stderr = run_command(cmd, cwd=project_root)
        else:
        ...
        print(
            f"   Success Rate: {
                final_stats['successful_executions']}/{
                final_stats['total_executions']}"
        )
        print(
            f"   Average Duration: {
                final_stats['average_duration_minutes']} minutes"
        )
        ...
        monitor.complete_agent_execution(test_execution_data, "FAILED", error=str(e))
