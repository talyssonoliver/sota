...
class DependencyAnalyzer:
    ...
    def build_dependency_graph(self):
        ...
        python_files = [
            f for f in python_files if ".venv" not in str(f) and "htmlcov" not in str(f)
        ]
    ...
    def analyze_duplicates(self) -> Dict[str, Dict[str, any]]:
        ...
        for old_path, new_path in self.duplicate_mappings.items():
            ...
            for old_file in old_files:
                ...
                complexity = (
                    len(deps) + len(dependents) * 2
                )  # Dependents are more complex
...
def main():
    ...
    for i, migration in enumerate(report["migration_order"], 1):
        ...
        print(
            f"   Complexity: {migration['complexity_score']}, Dependents: {migration['dependent_count']}"
        )
        if migration["leaf_files"]:
            print(
                f"   Leaf files: {', '.join(migration['leaf_files'][:3])}{'...' if len(migration['leaf_files']) > 3 else ''}"
            )
        print()
    ...
    for file_path, analysis in critical_sorted[:5]:
        ...
        print(
            f"  Dependencies: {len(analysis['dependencies'])}, Dependents: {len(analysis['dependents'])}"
        )
