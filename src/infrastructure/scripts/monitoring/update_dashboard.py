...
from src.infrastructure.utils.execution_monitor import DashboardLogger  # noqa: E402
...
class DashboardUpdater:
    ...
    def _update_task_completion_status(
        ...
        if task_data:
            ...
            duration_seconds = (
                (duration_minutes * 60) if duration_minutes is not None else 0
            )
    ...
    def _update_dashboard_summary(self, metrics: Dict[str, Any]) -> None:
        ...
        summary = {
            ...
            "status_breakdown": self._calculate_status_breakdown(
                ...
                "datasets": [
                    {
                        "data": [
                            ...
                            max(
                                0,
                                team_metrics.get("total_tasks", 0)
                                - team_metrics.get("completed_tasks", 0)
                                - team_metrics.get("in_progress_tasks", 0)
                                - team_metrics.get("failed_tasks", 0),
                            ),
    ...
    def _create_qa_chart_data(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        qa_not_run = len(
            [t for t in task_metrics if t.get("qa_status") in [None, "NOT_RUN"]]
        )
    ...
    def update_real_time_status(
        self, task_id: str, status: str, agent: str = "system"
    ) -> None:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Update dashboard with completion data"
    )
    parser.add_argument("--task-id", help="Specific task ID to update")
    parser.add_argument(
        "--dashboard-dir", default="dashboard", help="Dashboard directory"
    )
    parser.add_argument(
        "--completion", help="Mark task as completed (use with --task-id)"
    )
