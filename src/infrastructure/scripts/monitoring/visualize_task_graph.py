...
def visualize_task_graph(graph, output_file, color_by="status", layout="spring"):
    ...
    nx.draw_networkx_edges(
        graph, pos, arrows=True, edge_color="gray", width=1.0, alpha=0.5
    )
...
def analyze_graph_metrics(graph):
    ...
    max_in_degree = (
        max(in_degrees.items(), key=lambda x: x[1]) if in_degrees else ("None", 0)
    )
    max_out_degree = (
        max(out_degrees.items(), key=lambda x: x[1]) if out_degrees else ("None", 0)
    )
    ...
    metrics["leaf_tasks"] = [
        node for node, out_degree in out_degrees.items() if out_degree == 0
    ]
...
def main():
    ...
    parser.add_argument(
        "--tasks-dir", default="tasks", help="Directory containing task YAML files"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--color-by",
        choices=["status", "owner"],
        default="status",
        help="Property to color nodes by",
    )
    parser.add_argument(
        "--layout",
        choices=["spring", "spectral", "kamada_kawai", "planar", "circular"],
        default="spring",
