...
try:
    from src.infrastructure.utils.execution_monitor import (
        get_dashboard_logger, get_execution_monitor)
except ImportError:
    pass
...
formatter = jsonlogger.JsonFormatter(
    "%(asctime)s %(levelname)s %(name)s %(message)s %(event)s %(task_id)s"
)
...
class WorkflowMonitor:
    ...
    def add_log_message(
        self, message: str, level: str = "INFO", task_id: Optional[str] = None
    ):
        """
    ...
    def scan_output_directory(self):
        ...
        if self.task_id:
            ...
            try:
                ...
                self.add_log_message(
                    f"Error scanning output directory: {str(e)}", "ERROR"
                )
                return
        ...
        for task_dir in task_dirs:
            ...
            if os.path.exists(status_file):
                try:
                    ...
                    if (
                        status_file in self.last_seen_files
                        and self.last_seen_files[status_file] == mtime
                    ):
                    ...
                    if (
                        task_id in self.last_status
                        and self.last_status[task_id] == current_status
                    ):
                        continue
                    ...
                    self.add_log_message(
                        f"Error processing status file {status_file}: {
                            str(e)}",
                        "ERROR",
                        task_id=task_id,
                    )
            ...
            for agent_role in [
                ...
                if os.path.exists(output_file):
                    ...
                    if (
                        output_file in self.last_seen_files
                        and self.last_seen_files[output_file] == mtime
                    ):
                        # File hasn't changed, skip processing
                        continue
                    ...
                    # Update node status
                    if (
                        agent_role not in self.nodes_status
                        or self.nodes_status[agent_role] != NodeStatus.RUNNING
                    ):
                        self.nodes_status[agent_role] = NodeStatus.COMPLETED

                    # Add log message
                    self.add_log_message(
                        f"Agent {agent_role} completed for task {task_id}",
                        task_id=task_id,
                    )
            ...
            if os.path.exists(error_file):
                ...
                if (
                    error_file in self.last_seen_files
                    and self.last_seen_files[error_file] == mtime
                ):
                    # File hasn't changed, skip processing
                    continue
                ...
                try:
                    ...
                    # Add log message
                    self.add_log_message(
                        f"Error in task {task_id}: {error_message}",
                        "ERROR",
                        task_id=task_id,
                    )
                    ...
                    self.add_log_message(
                        f"Error processing error file {error_file}: {
                            str(e)}",
                        "ERROR",
                        task_id=task_id,
                    )
            ...
            for task_id, agents in dashboard_data["agent_status"].items():
                ...
                for agent_name, agent_info in agents.items():
                    status_icon = (
                        ...
                        else "🔄" if agent_info["status"] == "RUNNING" else "❌"
                    )
                    print(
    ...
    def run_simple_monitor(self):
        ...
        try:
            while True:
                clear_screen()
                print(
                    f"Workflow Monitor - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                )
                ...
                if self.task_id:
                    report = self.execution_monitor.create_execution_summary_report(
                        self.task_id
                    )
...
def draw_ui(stdscr, monitor: WorkflowMonitor, curses_module):
    ...
    if height < 10 or width < 40:
        stdscr.addstr(
            ...
            0,
            "Terminal too small",
            curses_module.color_pair(3) | curses_module.A_BOLD,
        )
        stdscr.refresh()
        return
    ...
    curses_module.init_pair(
        1, curses_module.COLOR_WHITE, curses_module.COLOR_BLACK
    )  # Default
    curses_module.init_pair(
        2, curses_module.COLOR_GREEN, curses_module.COLOR_BLACK
    )  # Success
    curses_module.init_pair(
        3, curses_module.COLOR_RED, curses_module.COLOR_BLACK
    )  # Error
    curses_module.init_pair(
        4, curses_module.COLOR_YELLOW, curses_module.COLOR_BLACK
    )  # Warning/Running
    curses_module.init_pair(
        5, curses_module.COLOR_CYAN, curses_module.COLOR_BLACK
    )  # Info
    curses_module.init_pair(
        6, curses_module.COLOR_MAGENTA, curses_module.COLOR_BLACK
    )  # Special
    ...
    stdscr.addstr(
        ...
        (width - len(title)) // 2,
        title,
        curses_module.color_pair(6) | curses_module.A_BOLD,
    )
    ...
    stdscr.addstr(
        1, width - len(runtime_info) - 1, runtime_info, curses_module.color_pair(5)
    )
    ...
    stdscr.addstr(
        ...
        0,
        "Task Status:",
        curses_module.color_pair(1) | curses_module.A_BOLD,
    )
    status_line += 1
    ...
    stdscr.addstr(
        log_line, 0, "Event Log:", curses_module.color_pair(1) | curses_module.A_BOLD
    )
    log_line += 1
    ...
    log_messages = (
        monitor.message_log[-max_log_messages:] if max_log_messages > 0 else []
    )
    ...
    if height > 2:
        ...
        stdscr.addstr(
            height - 1, (width - len(footer)) // 2, footer, curses_module.color_pair(5)
        )
...
def run_ui_wrapper_fn(stdscr, monitor: WorkflowMonitor, curses_module):
    ...
    try:
        ...
        while True:
            ...
            # to reduce unnecessary updates. Let the watchdog handler handle
            # most updates.
            current_time = time.time()
            if (
                not hasattr(monitor, "last_scan_time")
                or current_time - monitor.last_scan_time >= 3
            ):
                monitor.scan_output_directory()
                monitor.last_scan_time = current_time

            # Sleep for a bit - reduced from 0.5 to 0.2 for better UI
            # responsiveness
            time.sleep(0.2)
    finally:
        observer.stop()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Monitor LangGraph workflow executions in real-time"
    )
    parser.add_argument("--task", "-t", help="Task ID to monitor (e.g., BE-07)")
    parser.add_argument(
        "--output", "-o", help="Directory where task outputs are stored"
    )
    parser.add_argument(
        "--simple",
        "-s",
        action="store_true",
        help="Use simple output instead of curses UI",
    )
    ...
    if use_simple_mode:
        ...
        try:
            ...
            print(
                "Curses module not available. Please run in simple mode (--simple) or install curses (e.g., windows-curses on Windows)."
            )
            sys.exit(1)
        except Exception as e:
