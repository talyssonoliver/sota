...
def generate_daily_report(day: int) -> bool:
    ...
    try:
        ...
        report_file = (
            reports_dir / f"day{day}_report_{target_date.strftime('%Y-%m-%d')}.md"
        )
...
def generate_end_of_day_report(day: int) -> bool:
    ...
    try:
        ...
        try:
            daily_report = progress_generator.generate_daily_report(
                target_date.strftime("%Y-%m-%d")
            )
        ...
        visual_summary = _generate_visual_progress_summary(
            progress_generator, velocity_data
        )
        ...
        eod_report_file = (
            reports_dir / f"day{day}_eod_report_{target_date.strftime('%Y-%m-%d')}.md"
        )
...
def _calculate_sprint_velocity(
    progress_generator: ProgressReportGenerator, target_date: date
) -> Dict[str, Any]:
    ...
    try:
        ...
        for i in range(7):
            ...
            daily_tasks = _get_tasks_for_date(
                task_metrics, check_date.strftime("%Y-%m-%d")
            )
        ...
        # Calculate trend
        recent_velocity = (
            sum(day["tasks_completed"] for day in velocity_history[:3]) / 3
        )
        ...
        trend = (
            ...
            else "decreasing" if recent_velocity < older_velocity * 0.9 else "stable"
        )
        ...
        return {
            ...
            "projected_completion": _project_sprint_completion(
                metrics, recent_velocity
            ),
        }
    except Exception as e:
...
def _analyze_tomorrow_preparation(
    progress_generator: ProgressReportGenerator, target_date: date
) -> Dict[str, Any]:
    ...
    try:
        ...
        preparation_checklist = _generate_preparation_checklist(
            priority_tasks, potential_blockers
        )

        return {
            "total_planned": len(tomorrow_tasks),
            "high_priority": len(
                [t for t in priority_tasks if t.get("priority") == "HIGH"]
            ),
...
def _assess_sprint_health(
    ...
    try:
        ...
        sprint_metrics = (
            progress_generator.metrics_calculator.calculate_sprint_metrics()
        )
...
def _generate_visual_progress_summary(
    progress_generator: ProgressReportGenerator, velocity_data: Dict[str, Any]
) -> str:
    ...
    try:
        ...
        velocity_chart = _create_velocity_chart(
            velocity_data.get("velocity_history", [])
        )
...
def _create_enhanced_eod_report(
    day: int,
    target_date: date,
    daily_report: str,
...
def _get_tasks_for_date(
    task_metrics: List[Dict[str, Any]], date: str
) -> List[Dict[str, Any]]:
        ...
        if task.get("completion_time"):
            try:
                completion_time = datetime.fromisoformat(
                    task["completion_time"].replace("Z", "+00:00")
                )
...
def _project_sprint_completion(
    metrics: Dict[str, Any], current_velocity: float
) -> float:
    """Project sprint completion percentage."""
    team_metrics = metrics.get("team_metrics", {})
    remaining_tasks = team_metrics.get("total_tasks", 0) - team_metrics.get(
        "completed_tasks", 0
    )
...
def _get_planned_tasks_for_tomorrow(
    metrics: Dict[str, Any], target_date: date
) -> List[Dict[str, Any]]:
            ...
            planned_tasks.append(
                {
                    ...
                    "priority": (
                        "HIGH" if task.get("agent_type") == "backend" else "MEDIUM"
                    ),
...
def _generate_preparation_checklist(
    priority_tasks: List[Dict[str, Any]], blockers: List[Dict[str, Any]]
) -> List[str]:
...
def _calculate_velocity_consistency(
    progress_generator: ProgressReportGenerator,
) -> float:
...
def _generate_health_recommendations(
    health_score: float, team_metrics: Dict[str, Any]
) -> List[str]:
    ...
    if health_score < 60:
        recommendations.append(
            "🚨 Critical: Immediate attention needed to sprint health"
        )
...
def _identify_risk_factors(
    team_metrics: Dict[str, Any], sprint_metrics: Dict[str, Any]
) -> List[str]:
...
Examples:
  ...
  python scripts/generate_task_report.py --dashboard            # Update dashboard only
    ...
    parser.add_argument(
        "--end-of-day",
        action="store_true",
        help="Generate enhanced end-of-day report (requires --day)",
    )
    parser.add_argument(
        "--daily",
        type=int,
        help="Generate traditional daily report for specified day",
    )
    parser.add_argument("--task", type=str, help="Generate report for specific task ID")
    parser.add_argument(
        "--dashboard",
        action="store_true",
        help="Update dashboard with latest metrics",
    )
    parser.add_argument(
        "--all", action="store_true", help="Generate all reports (daily + EOD)"
    )

    # Configuration options
    parser.add_argument(
        "--format",
        choices=["markdown", "console", "html"],
        default="markdown",
        help="Output format for reports",
    )
    parser.add_argument(
        "--output-dir",
        default="progress_reports",
    ...
    if not any([args.day, args.daily, args.task, args.dashboard, args.all]):
        parser.error(
            "Must specify one of: --day, --daily, --task, --dashboard, or --all"
        )
