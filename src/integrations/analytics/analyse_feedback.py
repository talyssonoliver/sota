...
class FeedbackAnalyzer:
    ...
    def _get_feedback_content(self, feedback) -> str:
        ...
        if not content and hasattr(feedback, "comments") and feedback.comments:
            content = (
                ...
                if isinstance(feedback.comments, list)
                else str(feedback.comments)
            )
        return content or ""
    ...
    def analyze_task_feedback(self, task_id: str) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "prompt_modifications": self._suggest_prompt_modifications(
                feedback_entries
            ),
            ...
            "fine_tuning_examples": self._generate_fine_tuning_examples(
                feedback_entries
            ),
    ...
    def _analyze_feedback_summary(
        ...
        for feedback in feedback_entries:
            # Handle both possible attribute names for compatibility
            scores_data = getattr(feedback, "category_scores", None) or getattr(
    ...
    def _suggest_prompt_modifications(
        self, feedback_entries: List[FeedbackEntry]
    ) -> List[Dict[str, Any]]:
        ...
        for feedback in feedback_entries:
            ...
            if content:
                ...
                if any(
                    ...
                    for word in ["unclear", "confusing", "ambiguous"]
                ):
                    prompt_issues["clarity"] += 1

                if any(
                    word in content_lower for word in ["missing", "incomplete", "add"]
                ):
                    prompt_issues["completeness"] += 1

                if any(
                    word in content_lower for word in ["specific", "detail", "precise"]
                ):
                    prompt_issues["specificity"] += 1

                if any(
                    word in content_lower
                    for word in ["context", "background", "explain"]
                ):
    ...
    def _suggest_tool_improvements(
        self, feedback_entries: List[FeedbackEntry]
    ) -> List[Dict[str, Any]]:
        ...
        for feedback in feedback_entries:
            ...
            if content:
                ...
                if "tool" in content_lower or "function" in content_lower:
                    if any(
                        word in content_lower
                        for word in ["slow", "performance", "timeout"]
                    ):
                        tool_issues["performance"] += 1

                    if any(
                        word in content_lower for word in ["error", "failed", "broken"]
                    ):
                        tool_issues["reliability"] += 1

                    if any(
                        word in content_lower
                        for word in ["missing", "need", "should have"]
                    ):
    ...
    def _suggest_context_adjustments(
        self, feedback_entries: List[FeedbackEntry]
    ) -> List[Dict[str, Any]]:
        ...
        for feedback in feedback_entries:
            ...
            if content:
                ...
                if any(
                    word in content_lower
                    for word in ["context", "background", "related"]
                ):
    ...
    def _generate_fine_tuning_examples(
        self, feedback_entries: List[FeedbackEntry]
    ) -> List[Dict[str, Any]]:
        ...
        for feedback in feedback_entries:
            if (
                self._get_feedback_content(feedback)
                and len(self._get_feedback_content(feedback)) > 20
            ):
    ...
    def _analyze_global_patterns(
        self, all_feedback: List[FeedbackEntry]
    ) -> Dict[str, Any]:
    ...
    def _analyze_agent_performance(
        self, feedback_by_agent: Dict[str, List[FeedbackEntry]]
        ...
        for agent_name, feedback_list in feedback_by_agent.items():
            ...
            if scores:
                agent_analysis[agent_name] = {
                    ...
                    "improvement_areas": self._identify_agent_improvement_areas(
                        feedback_list
                    ),
                }
    ...
    def _analyze_task_patterns(
        self, feedback_by_task: Dict[str, List[FeedbackEntry]]
    ) -> Dict[str, Any]:
    ...
    def _suggest_system_improvements(
        self, all_feedback: List[FeedbackEntry]
    ) -> List[Dict[str, Any]]:
        ...
        for feedback in all_feedback:
            if self._get_feedback_content(feedback):
                ...
                if any(
                    word in content_lower for word in ["slow", "timeout", "performance"]
                ):
                ...
                improvements.append(
                    {
                        ...
                        "suggestion": self._get_system_improvement_suggestion(
                            issue_type
                        ),
                    }
                )
    ...
    def _generate_overall_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if analysis.get("recurring_edits"):
            recommendations.append(
                "Focus on addressing recurring edit patterns in agent training"
            )

        if analysis.get("prompt_modifications"):
            recommendations.append(
                "Update prompt templates based on identified modification needs"
            )
    ...
    def _identify_agent_improvement_areas(
        ...
        for feedback in feedback_list:
            if self._get_feedback_content(feedback):
                ...
                if any(
                    word in content_lower for word in ["accuracy", "correct", "wrong"]
                ):
                    issues["accuracy"] += 1
    ...
    def _generate_markdown_report(self, analysis: Dict[str, Any]) -> str:
        ...
        if "summary" in analysis:
            ...
            report.append(
                f"- **Average Score:** {summary.get('average_overall_score', 0):.2f}"
            )
        ...
        if "recurring_edits" in analysis and analysis["recurring_edits"]:
            ...
            for edit in analysis["recurring_edits"]:
                report.append(
                    f"- **{edit['pattern']}:** {edit['frequency']} occurrences"
                )
        ...
        if "prompt_modifications" in analysis and analysis["prompt_modifications"]:
            ...
            for mod in analysis["prompt_modifications"]:
                report.append(
                    f"- **{mod['type']}** (Priority: {mod['priority']}): {mod['suggestion']}"
                )
        ...
        if "tool_improvements" in analysis and analysis["tool_improvements"]:
            ...
            for improvement in analysis["tool_improvements"]:
                report.append(
                    f"- **{improvement['type']}:** {improvement['suggestion']}"
                )

        # Overall recommendations
        if (
            "overall_recommendations" in analysis
            and analysis["overall_recommendations"]
        ):
            report.append("\n## Overall Recommendations")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze human feedback for agent refinement"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Period in days for comprehensive analysis",
    )
    parser.add_argument(
        "--format",
    ...
    if "prompt_modifications" in analysis:
        print(
            f"   • Generated {len(analysis['prompt_modifications'])} prompt modification suggestions"
        )
    if "overall_recommendations" in analysis:
        print(
            f"   • Created {len(analysis['overall_recommendations'])} overall recommendations"
        )
