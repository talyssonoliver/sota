...
from .hitl.models import (HITLAuditEntry, HITLCheckpoint, HITLReviewDecision,
                          RiskAssessment)
# Import from the new modular structure
from .hitl.types import (CheckpointStatus, CheckpointType, RiskLevel,
                         TimeoutAction)
...
class HITLPolicyEngine:
    ...
    async def assess_risk(
        ...
        if "content" in task_data:
            ...
            high_risk_patterns = task_policies.get("risk_assessment", {}).get(
                "high_risk_patterns", []
            )
            ...
            medium_risk_patterns = task_policies.get("risk_assessment", {}).get(
        ...
        checkpoint_config = (
            self.policies.get("hitl_policies", {})
            .get("checkpoint_triggers", {})
            .get(checkpoint_type.value, {})
        )
        ...
        auto_approve_eligible = (
            ...
            and task_policies.get("auto_approve_enabled", False)
        )
    ...
    def _check_performance_regression(self, task_data: Dict[str, Any]) -> bool:
        ...
        threshold = (
            ...
            .get("qa_validation", {})
            .get("thresholds", {})
            .get("performance_regression_max", 5)
        )
    ...
    def create_checkpoint(
        ...
        checkpoint_config = self._normalize_policy_access(
            "checkpoint_triggers", checkpoint_type
        )
        if not checkpoint_config.get("enabled", True):
            ...
            if not task_policy:
                ...
                task_policy = self._normalize_policy_access(
                    "task_type_policies", task_type_with_suffix
                )

            if risk_level == RiskLevel.LOW and task_policy.get(
        ...
        timeout_hours = checkpoint_config.get(
            "timeout_hours", global_settings.get("default_timeout_hours", 24)
        )
        ...
        logger.info(
            f"Created checkpoint {checkpoint_id} for task {task_id} with status {status.value}"
        )
        return checkpoint
    ...
    def _send_notification(self, checkpoint: HITLCheckpoint, notification_type: str):
        ...
        try:
            ...
            logger.warning(
                f"Failed to send notification for checkpoint {checkpoint.id}: {e}"
            )
    ...
    async def approve_checkpoint(
        self, checkpoint_id: str, reviewer: str, comments: Optional[str] = None
    ) -> bool:
        ...
        if reviewer not in checkpoint.assigned_reviewers:
            logger.warning(
                f"Reviewer {reviewer} not assigned to checkpoint {checkpoint_id}"
            )
    ...
    def _auto_approve_checkpoint(self, checkpoint: HITLCheckpoint):
        ...
        self._log_audit_event(
            "auto_approved",
            {"checkpoint_id": checkpoint.id, "reason": "timeout"},
        )
    ...
    def _escalate_checkpoint(self, checkpoint: HITLCheckpoint):
        ...
        escalation_policies = self.policies.get("hitl_policies", {}).get(
            "escalation_policies", {}
        )
        ...
        logger.info(
            f"Escalated checkpoint {checkpoint.id} to level {checkpoint.escalation_level}"
        )
    ...
    def _block_checkpoint(self, checkpoint: HITLCheckpoint):
        ...
        self._log_audit_event(
            "blocked", {"checkpoint_id": checkpoint.id, "reason": "timeout"}
        )
    ...
    def _notify_timeout(self, checkpoint: HITLCheckpoint):
        ...
        self._log_audit_event("timeout_notified", {"checkpoint_id": checkpoint.id})
    ...
    def _send_checkpoint_notifications(
        self, checkpoint: HITLCheckpoint, notification_type: str
    ):
        """Send notifications for checkpoint events"""
        for handler in self.notification_handlers:
            try:
                ...
                logger.error(
                    f"Error sending notification via {handler.__class__.__name__}: {e}"
                )
    ...
    def _update_task_metadata(self, task_id: str, metadata: Dict[str, Any]):
        ...
        metadata_file = Path(f"storage/hitl_tasks/{task_id}.json")
        if self.test_mode:
            # In test mode, don't write to filesystem
            return
    ...
    async def process_decision(
        self, decision_or_checkpoint_id, decision_data=None
    ) -> bool:
    ...
    async def escalate_checkpoint(
        self, checkpoint_id: str, escalated_by_or_data
    ) -> bool:
    ...
    def process_timeouts(self) -> List[HITLCheckpoint]:
        ...
        for checkpoint in self.checkpoints.values():
            if (
                ...
                and checkpoint.timeout_at
                and current_time > checkpoint.timeout_at
            ):
                checkpoint.status = CheckpointStatus.ESCALATED
                checkpoint.escalation_level += 1
    ...
    def get_audit_trail(
        ...
        def _convert_entry(entry):
            ...
            if isinstance(entry_copy.get("timestamp"), str):
                entry_copy["timestamp"] = datetime.fromisoformat(
                    entry_copy["timestamp"]
                )
    ...
    def get_metrics(
        self, period_days: int = 30, days: Optional[int] = None
    ) -> Dict[str, Any]:
    ...
    def get_checkpoint_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_checkpoints = [
                cp for cp in self.checkpoints.values() if cp.created_at >= cutoff_date
            ]
            ...
            total_resolved = len(
                [
                    ...
                    if cp.status
                    in [CheckpointStatus.APPROVED, CheckpointStatus.REJECTED]
                ]
            )
            ...
            resolved_checkpoints = [
                cp for cp in recent_checkpoints if cp.resolved_at and cp.created_at
            ]
            ...
            return {
                ...
                "escalated_count": len(
                    [
                        cp
                        for cp in recent_checkpoints
                        if cp.status == CheckpointStatus.ESCALATED
                    ]
                ),
                "pending_count": len(
                    [
    ...
    def get_daily_trends(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            approved_total = len(
                [cp for cp in all_checkpoints if cp.status == CheckpointStatus.APPROVED]
            )
            rejected_total = len(
                [cp for cp in all_checkpoints if cp.status == CheckpointStatus.REJECTED]
            )
            escalated_total = len(
                [
                    cp
        ...
        try:
            ...
            for checkpoint in self.checkpoints.values():
                ...
                if checkpoint.risk_level == "high":
                    ...
                    checkpoint.risk_level == "medium"
                    and workflows[task_id]["risk_level"] != "high"
                ):
    ...
    def _get_escalation_policy(self, risk_level: RiskLevel) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Error getting escalation policy for risk level {risk_level}: {e}"
            )
            return {
                "escalation_levels": [
...
async def create_hitl_checkpoint_for_task(
    task_id: str,
    checkpoint_type: str,
    task_data: Dict[str, Any],
...
async def check_hitl_approval_required(task_id: str, task_data: Dict[str, Any]) -> bool:
    ...
    for checkpoint_type in CheckpointType:
        ...
        if (
            ...
            or not risk_assessment.auto_approve_eligible
        ):
            return True
