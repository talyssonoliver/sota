...
class HITLTaskMetadata:
    ...
    checkpoints: List[HITLCheckpointMetadata] = field(
        default_factory=list
    )  # Using checkpoints instead of hitl_checkpoints
    ...
    def add_hitl_checkpoint(self, checkpoint: HITLCheckpointMetadata) -> None:
        ...
        if (
            risk_hierarchy[checkpoint.risk_level]
            > risk_hierarchy[self.overall_risk_level]
        ):
    ...
    def get_active_checkpoints(self) -> List[HITLCheckpointMetadata]:
        ...
        return [
            cp for cp in self.hitl_checkpoints if cp.status == HITLStatus.HITL_PENDING
        ]

    def get_checkpoint_by_id(
        self, checkpoint_id: str
    ) -> Optional[HITLCheckpointMetadata]:
    ...
    def calculate_risk_score(self) -> float:
        ...
        if self.confidence_metrics:
            avg_confidence = sum(self.confidence_metrics.values()) / len(
                self.confidence_metrics
            )
            confidence_adjustment = (
                1.0 - avg_confidence
            ) * 0.2  # Lower confidence = higher risk
    ...
    def is_blocked_on_hitl(self) -> bool:
        ...
        return (
            self.hitl_enabled
            and self.hitl_status == HITLStatus.HITL_PENDING
            and len(self.get_active_checkpoints()) > 0
        )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "HITLTaskMetadata":
        ...
        if "checkpoints" in data:
            data["checkpoints"] = [
                HITLCheckpointMetadata.from_dict(cp) for cp in data["checkpoints"]
            ]
        elif "hitl_checkpoints" in data:
            # Handle old format for backward compatibility
            data["checkpoints"] = [
                HITLCheckpointMetadata.from_dict(cp) for cp in data["hitl_checkpoints"]
            ]
...
class HITLTaskMetadataManager:
    ...
    def save_task_metadata(self, task_metadata: HITLTaskMetadata) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to save task metadata {task_metadata.task_id}: {e}"
            )
            return False
    ...
    def get_tasks_by_hitl_status(
        self, hitl_status: HITLStatus
    ) -> List[HITLTaskMetadata]:
        """Get tasks by their HITL status."""
        all_tasks = self.get_all_tasks()
        return [task for task in all_tasks if task.hitl_status == hitl_status]

    def get_tasks_by_risk_level(
        self, risk_level: HITLRiskLevel
    ) -> List[HITLTaskMetadata]:
    ...
    def add_checkpoint_to_task(
        self, task_id: str, checkpoint: HITLCheckpointMetadata
    ) -> bool:
    ...
    def update_checkpoint_status(
        ...
        success = task_metadata.update_checkpoint_status(
            checkpoint_id, status, reviewer, notes
        )
        if success:
            return self.save_task_metadata(task_metadata)
        return False
