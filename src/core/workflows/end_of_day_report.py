...
class EndOfDayReportGenerator:
    ...
    def __init__(self):
        ...
        try:
            ...
            logging.warning(f"Failed to initialize metrics calculator: {e}")
            self.metrics_calculator = None
            
        try:
            self.execution_monitor = ExecutionMonitor()
        except Exception as e:
            logging.warning(f"Failed to initialize execution monitor: {e}")
            self.execution_monitor = None
            
        try:
            self.progress_generator = ProgressReportGenerator()
        except Exception as e:
            logging.warning(f"Failed to initialize progress generator: {e}")
            self.progress_generator = None
            
        self.logger = logging.getLogger(__name__)

        # Initialize paths with proper error handling
        try:
    ...
    async def generate_eod_report(
        ...
        if include_automation_stats:
            report_data["automation_stats"] = self._gather_automation_statistics(
                report_date
            )
        ...
        # Save report
        report_filename = self._save_report(
            formatted_report, report_date, output_format
        )
    ...
    def _format_markdown_report(self, data: Dict[str, Any]) -> str:
        ...
        report_date = datetime.strptime(data["date"], "%Y-%m-%d").strftime(
            "%A, %B %d, %Y"
        )
