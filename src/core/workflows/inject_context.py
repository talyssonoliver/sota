...
from src.platform.utils.task_loader import load_task_metadata
...
class ContextInjector:
    ...
    def inject_task_context(
            self, task_id: str, agent_role: str) -> Dict[str, Any]:
        ...
        try:
            ...
            context = self.agent_builder._get_context_for_agent(
                agent_role, task_metadata)
            ...
            self.logger.error(
                f"Failed to inject context for task {task_id}: {e}")
            return {}
    ...
    def prepare_agent_with_context(self, agent_or_task, task_metadata=None):
        ...
        if isinstance(agent_or_task, str):
            ...
            if task_metadata is None or not isinstance(task_metadata, dict):
                task_metadata = load_task_metadata(task_id)
            agent_role = task_metadata.get(
                "role") or task_metadata.get("owner") or "coordinator"
            return self.agent_builder.build_agent(
                role=agent_role, task_metadata=task_metadata)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Inject MCP context into prompts")
    ...
    parser.add_argument(
        "--output",
        "-o",
        help="Output file path (required if input is provided)")
    parser.add_argument(
        "--position",
        "-p",
        choices=[
            ...
            "marker"],
        default="top",
        help="Position to insert context")
    parser.add_argument(
        "--marker",
        "-m",
        default="{{CONTEXT}}",
    ...
    parser.add_argument(
        "--text",
        "-t",
        help="Direct prompt text to inject context into (alternative to file)")
    ...
    try:
        if args.input:
            ...
            inject_file_context(args.input, args.output,
                                args.query, args.position, args.marker)

        elif args.text:
            result = inject_context(
                args.text, args.query, args.position, args.marker)
