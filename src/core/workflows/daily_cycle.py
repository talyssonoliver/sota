...
class DailyCycleOrchestrator:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if config_path:
            try:
                validated_path = validate_file_path(config_path, must_exist=True)
                if validated_path is None:
                    raise ValueError(
                        f"Validation returned None for config path: {config_path}"
                    )
                self.config_path = str(validated_path)
            except ValidationError as e:
                raise ValueError(f"Invalid config path: {e}")
        else:
            self.config_path = "config/daily_cycle.json"

        self.config = self._load_config()

        # Initialize logging flag and logger placeholder
        self._logging_initialized = False
        self._logger = None

        # Initialize components with lazy loading for performance
        self._metrics_calculator = None
        self._execution_monitor = None
        self._briefing_generator = None
        self._eod_report_generator = None
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            "paths": {
                "logs_dir": str(Path(LOGS_DIR) / "daily_cycle"),
    ...
    @property
    def logger(self):
        """Lazy-loaded logger that only initializes when first accessed."""
    ...
    def eod_report_generator(self):
        ...
        if self._eod_report_generator is None:
    ...
    def _setup_logging(self):
        ...
        # Only create log files if we're not in a test environment
        if self._is_test_environment():
            # For tests, use a simple in-memory logger
            self._logger = logging.getLogger(__name__)
    ...
    async def run_morning_briefing(self):
        ...
        try:
            ...
            if self.config.get("email", {}).get("enabled", False):
                email_result = await self.email_integration.send_morning_briefing(
                    briefing_result
                )
    ...
    async def run_midday_check(self):
        ...
        try:
            ...
            if hasattr(metrics, "completed_tasks"):
                completed_count = getattr(metrics, "completed_tasks", 0)
            ...
            self.logger.info(
                f"Midday check completed - {completed_count} tasks " "completed today"
            )
            return {
                "status": "success",
                "metrics": metrics,
                ...
                "tasks_analyzed": (
                    getattr(metrics, "total_tasks", 0)
                    if hasattr(metrics, "total_tasks")
                    else 0
                ),
        ...
        try:
            ...
            progress_result = self._run_cli_command(
                [
                    ...
                    "scripts/generate_task_report.py",
                    "--day",
                    str(self._get_current_day()),
                ]
            )

            self.logger.info(
                f"Enhanced end-of-day report completed: {eod_result.get('output_file')}"
            )
            ...
            return {
                ...
                "sprint_health": eod_result.get("sprint_progress", {}).get(
                    "sprint_health", {}
                ),
    ...
    def schedule_daily_tasks(self):
        ...
        if hasattr(self, "logger") and self.logger:
            self.logger.warning(
                "schedule_daily_tasks is deprecated, using schedule_enhanced_tasks"
            )
        ...
        try:
            while True:
                try:
                    ...
                    error_count += 1
                    self.logger.error(
                        f"Scheduler error ({error_count}/{max_errors}): {e}"
                    )

                    if error_count >= max_errors:
                        self.logger.critical(
                            "Maximum error threshold reached, stopping scheduler"
                        )
                        break
    ...
    def schedule_enhanced_tasks(self):
        ...
        if not automation_config.get("weekend_mode", False):
            ...
            schedule.every().wednesday.at(
                automation_config["morning_briefing_time"]
            ).do(self._run_async_task, self.run_morning_briefing)
            ...
            if "midday_check_time" in automation_config:
                for day in [
                    ...
                    getattr(schedule.every(), day).at(
                        automation_config["midday_check_time"]
                    ).do(self._run_async_task, self.run_midday_check)
        else:
        ...
        self.logger.info(
            f"Enhanced daily tasks scheduled: {len(schedule.get_jobs())} jobs"
        )
        ...
        try:
            ...
            if self.config.get("email", {}).get("enabled", False):
                ...
                await self.email_integration._send_email(
                    ...
                    html_content=f"<h1>Emergency Stop</h1><p>{emergency_data['reason']} at {emergency_data['timestamp']}</p><pre>{emergency_data['system_status']}</pre>",
                    text_content=f"{emergency_data['reason']} at {emergency_data['timestamp']}. System status: {emergency_data['system_status']}",
                )
    ...
    def initialize_for_production(self):
        """
        Explicitly initialize the orchestrator for production use.
        This ensures proper logging setup and component initialization.
        """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Enhanced Daily Cycle Automation Orchestrator"
    )
    parser.add_argument("--config", help="Path to configuration file")
    parser.add_argument(
        "--mode",
        choices=["schedule", "manual", "health", "status"],
        default="manual",
        help="Run mode: schedule (continuous), manual (one-time), health (health check), status (show status)",
    )
    parser.add_argument(
        "--cycle",
        choices=["full", "morning", "midday", "eod", "health", "performance"],
        default="full",
        help="Cycle type for manual mode",
    )
    parser.add_argument(
        "--enable-weekend",
        action="store_true",
