...
class PlanExecutionManager:
    ...
    def __init__(
        ...
        self.part_status = (
            {}
        )  # Store status: {id: "pending" | "running" | "completed" | "failed"}

    def _invoke_coordinator(
        self, feedback_from_completed_tasks: Optional[List[Dict]] = None
    ) -> bool:
        """
        ...
        try:
            ...
            coordinator_output_state = self.workflow_engine.invoke(
                coordinator_input_state
            )
    ...
    def _execute_part(self, part_details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Executing part: {part_id} - {part_details['description']} for agent {part_details['agent_role']}"
        )
        ...
        try:
            ...
            if (
                result_state.get("status") == TaskStatus.BLOCKED
                or error_info
                ...
                error_message = (
                    ...
                    if error_info
                    else result_state.get("error", "Unknown error")
                )
    ...
    def run_plan(self) -> Dict[str, Any]:
        ...
        while current_iteration < max_iterations:
            ...
            for part_details in self.plan:
                ...
                if self.part_status.get(part_id) == "pending":
                    ...
                    if all(
                        self.part_status.get(dep_id) == "completed"
            ...
            if not next_tasks_to_run:
                ...
                if all(
                    ...
                    for status in self.part_status.values()
                ) and len(self.part_status) == len(self.plan):
                    ...
                    self._invoke_coordinator(
                        feedback_from_completed_tasks=final_feedback
                    )
                    break
                else:
                    logger.warning(
                        "No tasks ready to run, but not all tasks are done. Possible deadlock."
                    )
                    ...
                    current_state_feedback = [
                        ...
                        for tid in self.part_status
                        if self.part_status[tid] != "pending"
                    ]
                    if not self._invoke_coordinator(
                        feedback_from_completed_tasks=current_state_feedback
                    ):
            ...
            if completed_tasks_in_iteration:
                if not self._invoke_coordinator(
                    feedback_from_completed_tasks=completed_tasks_in_iteration
                ):
                    logger.error(
                        "Failed to get updated plan/next steps from Coordinator. Aborting."
                    )
                    return {"status": "FAILED", "reason": "Re-planning failed"}

            # Check if coordinator signals completion
            if not self.plan or (
                len(self.plan) == 1 and self.plan[0].get("agent_role") == "END_PLAN"
            ):
        ...
        final_status = (
            "COMPLETED"
            if all(s == "completed" for s in self.part_status.values())
            else "PARTIAL"
        )
