...
<body>
    ...
    async def send_morning_briefing(
        self, briefing_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _prepare_briefing_template_data(
        self, briefing_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _prepare_eod_template_data(self, report_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "tomorrow_prep": report_data.get("tomorrow_prep", {}).get(
                "recommended_actions", []
            )[:5],
            "dashboard_url": "http://localhost:5000/dashboard/realtime_dashboard.html",
    ...
    def _get_briefing_recipients(self) -> List[str]:
        ...
        recipients.extend(
            self.email_config.get("recipients", {}).get("stakeholders", [])
        )
...
async def main():
    ...
    parser.add_argument(
        "--configure", action="store_true", help="Configure email settings"
    )
    ...
    if args.configure:
        ...
        smtp_server = (
            input("SMTP Server (default: smtp.gmail.com): ").strip() or "smtp.gmail.com"
        )
        ...
        print(
            f"SMTP Server: {email_system.email_config.get('smtp_server', 'Not configured')}"
        )
