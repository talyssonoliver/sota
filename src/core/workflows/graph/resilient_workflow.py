...
try:
    ...
    # Create a placeholder for when LangGraph is not available
    class StateGraph:
        ...
        def wrapper(state):
            ...
            try:
                ...
                if current_attempt < max_retries:
                    ...
                    state["error"] = (
                        f"Max retry attempts ({max_retries}) reached: {
                        str(e)}"
                    )
...
def add_resilience_to_graph(
    graph: StateGraph, config: Optional[Dict[str, Any]] = None
) -> StateGraph:
    """
...
def create_resilient_workflow(
    base_graph_builder: Callable[[], StateGraph],
