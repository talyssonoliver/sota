...
class ErrorContext:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "impact_assessment": {
                "affected_tasks": self.impact_assessment.affected_tasks if self.impact_assessment else [],
                "blocked_workflows": self.impact_assessment.blocked_workflows if self.impact_assessment else [],
                "estimated_delay_minutes": self.impact_assessment.estimated_delay_minutes if self.impact_assessment else 0,
                "severity_justification": self.impact_assessment.severity_justification if self.impact_assessment else "",
                "recovery_feasible": self.impact_assessment.recovery_feasible if self.impact_assessment else True
            },
...
class ErrorPropagationManager:
    ...
    def __init__(self):
        ...
        self.retry_policies: Dict[ErrorType, RetryPolicy] = self._default_retry_policies()
        self.circuit_breakers: Dict[str, CircuitBreaker] = {}
        self.recovery_strategies: Dict[ErrorType, RecoveryStrategy] = self._default_recovery_strategies()
        
    def _default_retry_policies(self) -> Dict[ErrorType, RetryPolicy]:
        """Default retry policies for different error types"""
        return {
            ErrorType.INFRASTRUCTURE: RetryPolicy(max_retries=3, base_delay=1.0, max_delay=30.0),
            ErrorType.RESOURCE: RetryPolicy(max_retries=2, base_delay=5.0, max_delay=60.0),
            ErrorType.TIMEOUT: RetryPolicy(max_retries=3, base_delay=2.0, max_delay=30.0),
            ErrorType.INTEGRATION: RetryPolicy(max_retries=2, base_delay=1.0, max_delay=15.0),
            ErrorType.AGENT_FAILURE: RetryPolicy(max_retries=1, base_delay=0.5, max_delay=5.0),
        }
    
    def _default_recovery_strategies(self) -> Dict[ErrorType, RecoveryStrategy]:
    ...
    def classify_error(self, exception: Exception, task_id: str, agent_role: str) -> ErrorType:
        """Classify error based on exception type and context"""
        error_message = str(exception).lower()
        exception_type = type(exception).__name__.lower()
        
        # Infrastructure errors
        if any(keyword in error_message for keyword in ["connection", "network", "timeout", "unavailable"]):
            return ErrorType.INFRASTRUCTURE
        
        # Resource errors
        if any(keyword in error_message for keyword in ["memory", "disk", "rate limit", "quota"]):
        ...
        if any(keyword in error_message for keyword in ["agent", "tool", "initialization"]):
            return ErrorType.AGENT_FAILURE
        
        # Default to task execution error
        return ErrorType.TASK_EXECUTION
    
    def determine_severity(self, error_type: ErrorType, task_id: str, agent_role: str) -> ErrorSeverity:
        """Determine error severity based on type and context"""
        # Critical errors that halt system operation
        if error_type in [ErrorType.AUTHENTICATION, ErrorType.STATE_TRANSITION]:
            return ErrorSeverity.CRITICAL
        
        # High severity for infrastructure and agent failures
        if error_type in [ErrorType.INFRASTRUCTURE, ErrorType.AGENT_FAILURE, ErrorType.TASK_DEPENDENCY]:
            return ErrorSeverity.HIGH
        
        # Medium severity for most execution errors
        if error_type in [ErrorType.TASK_EXECUTION, ErrorType.INTEGRATION, ErrorType.RESOURCE]:
    ...
    def create_error_context(self, exception: Exception, task_id: str, agent_role: str,
        ...
        logger.error(f"Error context created: {error_id}", extra={
            "error_id": error_id,
            "task_id": task_id,
            "agent_role": agent_role,
    ...
    def _assess_impact(self, error_type: ErrorType, severity: ErrorSeverity,
                      task_id: str, agent_role: str) -> ImpactAssessment:
        ...
        if severity == ErrorSeverity.CRITICAL:
            ...
            impact.severity_justification = "High impact error with automated recovery possible"
        else:
            impact.estimated_delay_minutes = 5
            impact.recovery_feasible = True
            impact.severity_justification = "Low impact error with quick recovery expected"
    ...
    def determine_recovery_strategy(self, error_context: ErrorContext) -> RecoveryStrategy:
        ...
        return self.recovery_strategies.get(error_context.error_type, RecoveryStrategy.HUMAN_ESCALATION)
    
    def execute_recovery(self, error_context: ErrorContext, 
        ...
        try:
            if strategy == RecoveryStrategy.RETRY_EXPONENTIAL:
                if recovery_function:
                    retry_policy = self.retry_policies.get(error_context.error_type)
                    if retry_policy and retry_policy.should_retry(error_context.retry_count):
                ...
                service_name = f"{error_context.agent_role}_{error_context.error_type.value}"
        ...
        if success:
            ...
            logger.info(f"Recovery successful for error {error_context.error_id}: {details}")
        else:
            logger.warning(f"Recovery failed for error {error_context.error_id}: {details}")
    ...
    def _escalate_to_human(self, error_context: ErrorContext) -> bool:
        ...
        logger.critical(f"Error escalated to human: {error_context.error_id}", extra={
    ...
    def get_error_metrics(self) -> Dict[str, Any]:
        ...
        escalated_errors = sum(1 for ctx in self.error_contexts.values() 
        ...
        for ctx in self.error_contexts.values():
            error_by_type[ctx.error_type.value] = error_by_type.get(ctx.error_type.value, 0) + 1
            error_by_severity[ctx.severity.value] = error_by_severity.get(ctx.severity.value, 0) + 1
        ...
        return {
            ...
            "escalation_rate": escalated_errors / total_errors if total_errors > 0 else 0,
...
def handle_task_error(exception: Exception, task_id: str, agent_role: str,
                     ...
                     recovery_function: Optional[Callable] = None) -> ErrorContext:
    """
