...
class DashboardNotificationHandler(NotificationHandler):
    ...
    def __init__(self, config=None, test_mode=False):
        ...
        if not self.test_mode:
            ...
            # In test mode, use in-memory storage or temp directory
    ...
    def send_notification(
        ...
        notification = {
            ...
            "metadata": {
                ...
                "risk_level": getattr(
                    checkpoint.risk_level, "value", checkpoint.risk_level
                ),
    ...
    def _get_notification_message(self, checkpoint, notification_type: str) -> str:
        ...
        messages = {
            ...
            "timeout_warning": (
                f"Checkpoint is approaching deadline: {checkpoint.timeout_at.strftime('%Y-%m-%d %H:%M')}"
                if checkpoint.timeout_at
                else "Checkpoint is approaching deadline."
            ),
        }
    ...
    def _update_dashboard_data(
        self, notification: Dict[str, Any], dashboard_url: str = None
    ):
        ...
        if dashboard_url:
            try:
                ...
                if response.status_code == 200:
                    logger.info(
                        f"Successfully sent notification to dashboard: {dashboard_url}"
                    )
                else:
                    logger.warning(
                        f"Dashboard API returned status {response.status_code}"
                    )
    ...
    def _save_to_local_storage(self, notification: Dict[str, Any]):
        ...
        if notification["type"] == "checkpoint_created":
            ...
            dashboard_data["pending_count"] = max(
                0, dashboard_data["pending_count"] - 1
            )
    ...
    def __init__(self, config=None):
        if config:
            ...
            self.from_address = config.get(
                "from_address",
                os.getenv("HITL_FROM_EMAIL", "hitl-system@company.com"),
            )
            self.use_tls = config.get(
...
HITL System
    ...
    def send_notification(
        ...
        try:
            ...
            logger.info(
                f"Email notification sent to {recipients} for checkpoint {checkpoint.id}"
            )
            return True
        except Exception as e:
    ...
    def _get_email_recipients(self, checkpoint, notification_type: str) -> List[str]:
        ...
        for reviewer in checkpoint.assigned_reviewers:
            ...
            email_mapping = {
                ...
                "Frontend Engineer": os.getenv(
                    "FRONTEND_EMAIL", "frontend@company.com"
                ),
    ...
    def _prepare_email_data(self, checkpoint, notification_type: str) -> Dict[str, str]:
        ...
        return {
            ...
            "checkpoint_type": getattr(
                checkpoint.checkpoint_type, "value", checkpoint.checkpoint_type
            )
            .replace("_", " ")
            .title(),
            "risk_level": getattr(
            ...
            "trigger_reason": (
                ", ".join(checkpoint.risk_factors) if checkpoint.risk_factors else "N/A"
            ),
            "deadline": (
                checkpoint.timeout_at.strftime("%Y-%m-%d %H:%M UTC")
                if checkpoint.timeout_at
                else "N/A"
            ),
            "review_items": self._format_review_items(
                checkpoint.mitigation_suggestions or ["Review required"]
            ),
            ...
            "previous_reviewers": (
                ", ".join(checkpoint.assigned_reviewers)
                if checkpoint.assigned_reviewers
                else "N/A"
            ),
            "review_history": self._format_review_history(checkpoint),
            "approver_name": (
                checkpoint.approvals[-1]["reviewer"] if checkpoint.approvals else "N/A"
            ),
            "approval_timestamp": (
                checkpoint.approvals[-1]["timestamp"] if checkpoint.approvals else "N/A"
            ),
            "approval_comments": (
                checkpoint.approvals[-1]["comments"] if checkpoint.approvals else "N/A"
            ),
            "reviewer_name": (
                checkpoint.rejections[-1]["reviewer"]
                if checkpoint.rejections
                else "N/A"
            ),
            "rejection_timestamp": (
                checkpoint.rejections[-1]["timestamp"]
                if checkpoint.rejections
                else "N/A"
            ),
            "rejection_reason": (
                checkpoint.rejections[-1]["reason"] if checkpoint.rejections else "N/A"
            ),
            "rejection_comments": (
                checkpoint.rejections[-1]["comments"]
                if checkpoint.rejections
                else "N/A"
            ),
    ...
    def _format_review_history(self, checkpoint) -> str:
        ...
        for approval in checkpoint.approvals:
            history.append(
                f"✅ {approval['reviewer']} approved at {approval['timestamp']}"
            )

        for rejection in checkpoint.rejections:
            history.append(
                f"❌ {rejection['reviewer']} rejected at {rejection['timestamp']}: {rejection['reason']}"
            )
...
class SlackNotificationHandler(NotificationHandler):
    ...
    def _prepare_slack_message(
        self, checkpoint, notification_type: str
    ) -> Dict[str, Any]:
        ...
        channel = self.channel_mapping.get(
            checkpoint.risk_level.value, "#general-reviews"
        )
        ...
        # Main info block
        fields = [
            {"type": "mrkdwn", "text": f"*Task:* {checkpoint.task_id}"},
            {
                "type": "mrkdwn",
