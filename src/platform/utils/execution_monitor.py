...
try:
    ...
    class JsonFormatter(logging.Formatter):
        ...
        def format(self, record):
            # Create asctime if it doesn't exist
            if not hasattr(record, 'asctime'):
                record.asctime = self.formatTime(record, self.datefmt)
            
            # Build the message with extra fields
            message = record.getMessage()
            extra_fields = []
            
            # Include common extra fields in the log output
            for field in ['task_id', 'agent', 'event', 'duration']:
            ...
            return f"{record.asctime} {record.levelname} {message}"
