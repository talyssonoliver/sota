...
class HITLCLIIntegrationDemo:
    ...
    def test_cli_list_command(self) -> bool:
        ...
        try:
            ...
            task_checkpoints = self.cli_manager.list_pending_checkpoints(
                task_id=self.test_task_id
            )
            if len(task_checkpoints) >= 1:
                print(
                    f"   ✅ Found {len(task_checkpoints)} checkpoints for task {self.test_task_id}"
                )
            else:
    ...
    def test_cli_show_command(self) -> bool:
        ...
        try:
            ...
            if details:
                ...
                print(
                    f"   ❌ Failed to retrieve details for checkpoint {checkpoint_id}"
                )
                return False
    ...
    def test_cli_approval_workflow(self) -> bool:
        ...
        try:
            ...
            approval_success = self.cli_manager.approve_checkpoint(
                checkpoint1, "demo_reviewer", "Approved for demo purposes"
            )
        ...
        try:
            ...
            if metrics:
                ...
                if "checkpoints" in metrics:
                    ...
                    print(
                        f"       Checkpoints created: {cp_metrics.get('total_created', 0)}"
                    )
    ...
    def test_cli_export(self) -> bool:
        ...
        try:
            ...
            checkpoint_id = self.created_checkpoints[0]
            export_success = self.cli_manager.export_checkpoint_data(
                checkpoint_id, temp_path
            )
    ...
    def cleanup_test_data(self):
        ...
        try:
            ...
            print(
                f"   🗑️ Would clean up {len(self.created_checkpoints)} test checkpoints"
            )
    ...
    async def run_full_demo(self) -> bool:
        ...
        test_results.append(
            (
                ...
                await self.create_sample_checkpoints(),
            )
        )
        ...
        test_results.append(
            ("CLI Approval Workflow", self.test_cli_approval_workflow())
        )
        ...
        if success_rate >= 80:
            ...
            print(
                "   python cli/hitl_cli.py reject <checkpoint-id> --reviewer bob --reason 'needs-fixes'"
            )
