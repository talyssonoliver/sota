...
def test_context_review_summary():
    ...
    try:
        ...
        result = subprocess.run(
            [
                sys.executable,
                "orchestration/review_context.py",
                "BE-07",
                "--summary-only",
            ],
            capture_output=True,
        ...
        if result.returncode == 0:
            ...
            print(
                f"❌ Context review failed with return code: {
                    result.returncode}"
            )
...
def test_context_review_help():
    ...
    try:
        # Test help output
        result = subprocess.run(
            [sys.executable, "orchestration/review_context.py", "--help"],
            capture_output=True,
            text=True,
            cwd=project_root,
        )
        ...
        if result.returncode == 0:
            ...
            output = result.stdout
            if (
                "Human-in-the-Loop Context Review Tool" in output
                and "Examples:" in output
            ):
...
def test_context_review_export():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(
                f"❌ Context export failed with return code: {
                    result.returncode}"
            )
            print(f"Error: {result.stderr}")
            return False
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if context_log_path.exists():
                ...
                if log_data.get("agent_role") in [
                    "human_reviewer",
                    "reviewer",
                ] or additional_metadata.get("step_3_8_review"):
                    print(
                        "✅ Step 3.7 integration detected (human review markers found)"
                    )
                    return True
                else:
