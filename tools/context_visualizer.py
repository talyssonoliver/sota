...
try:
    from .context_tracker import get_all_context_logs, analyze_context_usage
except ImportError:
    from context_tracker import get_all_context_logs, analyze_context_usage
...
def analyze_context_coverage() -> Dict[str, Any]:
    ...
    try:
        ...
        if not all_logs:
            logger.warning("No context logs found. Run some tasks with context tracking first.")
            return {
        ...
        return {
            ...
            "summary_stats": {
                "avg_topics_per_task": sum(len(log.get("context_used", [])) for log in all_logs.values()) / len(all_logs),
                "avg_context_length": sum(task_context_lengths.values()) / len(task_context_lengths) if task_context_lengths else 0,
                "most_active_topic": topic_frequency.most_common(1)[0] if topic_frequency else ("none", 0),
                "most_active_agent": agent_frequency.most_common(1)[0] if agent_frequency else ("none", 0)
            }
        }
...
def generate_csv_report(coverage_data: Dict[str, Any], output_path: str = "reports/context-coverage.csv") -> bool:
    """
    ...
    try:
        ...
        with open(report_file, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            writer.writerow(['Total Tasks Analyzed', coverage_data.get("tasks_analyzed", 0)])
            writer.writerow(['Unique Topics', coverage_data.get("unique_topics", 0)])
            writer.writerow(['Unique Documents', coverage_data.get("unique_documents", 0)])
            
            summary_stats = coverage_data.get("summary_stats", {})
            writer.writerow(['Avg Topics per Task', f"{summary_stats.get('avg_topics_per_task', 0):.1f}"])
            writer.writerow(['Avg Context Length', f"{summary_stats.get('avg_context_length', 0):.0f}"])
            
            most_active_topic = summary_stats.get('most_active_topic', ('none', 0))
            writer.writerow(['Most Active Topic', f"{most_active_topic[0]} ({most_active_topic[1]} uses)"])
            
            most_active_agent = summary_stats.get('most_active_agent', ('none', 0))
            writer.writerow(['Most Active Agent', f"{most_active_agent[0]} ({most_active_agent[1]} tasks)"])
...
def generate_html_report(coverage_data: Dict[str, Any], output_path: str = "reports/context-coverage.html") -> bool:
    """
...
def generate_json_report(coverage_data: dict, output_path: str) -> bool:
    ...
    try:
        ...
        json_data = {
            "generated_at": coverage_data.get("generated_at", datetime.now().isoformat()),
            "stats": {
                "tasks_analyzed": coverage_data.get("tasks_analyzed", 0),
                "unique_topics": coverage_data.get("unique_topics", 0),
                "avg_topics_per_task": coverage_data.get("summary_stats", {}).get("avg_topics_per_task", 0),
                "avg_context_length": coverage_data.get("summary_stats", {}).get("avg_context_length", 0),
            },
            "heatmapData": [],
            "taskLabels": [],
            "topicLabels": [],
...
def generate_context_coverage_report(format: str = "both", 
    ...
    try:
        ...
        if success:
            logger.info(f"✅ Step 3.9 context coverage visualization completed successfully!")
            ...
            print(f"   Most active topic: {summary_stats.get('most_active_topic', ('none', 0))[0]}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Step 3.9 Context Coverage Visualization")
