...
class SyntaxRevertFixer:
    ...
    def fix_malformed_try_blocks(self, content: str) -> str:
        ...
        while i < len(lines):
            ...
            elif i > 0 and not line.strip().startswith(('import ', 'from ', 'try:', 'except', '#', '"""', "'''")) and ' import ' in line:
                # This might be an import statement incorrectly placed
                if 'import' in line and not any(x in line for x in ['# import', '"import', "'import"]):
    ...
    def fix_indentation_errors(self, content: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if i > 0 and lines[i-1].strip().endswith(':'):
                ...
                if i + 1 < len(lines) and lines[i+1].strip() and not lines[i+1].strip().startswith(('elif', 'else:', 'except', 'finally')):
    ...
    def fix_all_syntax_errors(self):
        ...
        error_files = [
            ...
            "scripts/generate_complete_tree.py",
        ...
        logger.info(f"🔧 Attempting to fix {len(error_files)} files with syntax errors...")
