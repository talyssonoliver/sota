        ...
        for task_dir in task_dirs:
            ...
            if os.path.exists(status_file):
                try:
                    ...
                    if status_file in self.last_seen_files and self.last_seen_files[
                            status_file] == mtime:
                    ...
                    self.add_log_message(
                        f"Task {task_id} status updated: {
                            self.task_status[task_id]}", task_id=task_id)

                except Exception as e:
                    self.add_log_message(
                        f"Error processing status file {status_file}: {
                            str(e)}", "ERROR", task_id=task_id)
            ...
            for agent_role in [
                ...
                if os.path.exists(output_file):
                    ...
                    if output_file in self.last_seen_files and self.last_seen_files[
                            output_file] == mtime:
                        # File hasn't changed, skip processing
                        continue
                    ...
                    # Update node status
                    if agent_role not in self.nodes_status or self.nodes_status[
                            agent_role] != NodeStatus.RUNNING:
...
def draw_ui(stdscr, monitor: WorkflowMonitor, curses_module):
    ...
    curses_module.init_pair(1, curses_module.COLOR_WHITE,
                            curses_module.COLOR_BLACK)  # Default
    curses_module.init_pair(2, curses_module.COLOR_GREEN,
                            curses_module.COLOR_BLACK)  # Success
    curses_module.init_pair(3, curses_module.COLOR_RED,
                            curses_module.COLOR_BLACK)    # Error
    curses_module.init_pair(4, curses_module.COLOR_YELLOW,
                            curses_module.COLOR_BLACK)  # Warning/Running
    curses_module.init_pair(5, curses_module.COLOR_CYAN,
                            curses_module.COLOR_BLACK)   # Info
    curses_module.init_pair(6, curses_module.COLOR_MAGENTA,
                            curses_module.COLOR_BLACK)  # Special
    ...
    stdscr.addstr(1, width - len(runtime_info) - 1,
                  runtime_info, curses_module.color_pair(5))

    # Draw status info
    status_line = 3
    stdscr.addstr(status_line, 0, "Task Status:",
    ...
    stdscr.addstr(node_line, 0, "Agent Status:",
                  curses_module.color_pair(1) | curses_module.A_BOLD)
    node_line += 1
    ...
    stdscr.addstr(log_line, 0, "Event Log:",
                  curses_module.color_pair(1) | curses_module.A_BOLD)
    log_line += 1
    ...
    if height > 2:
        ...
        stdscr.addstr(height - 1, (width - len(footer)) //
                      2, footer, curses_module.color_pair(5))
...
def run_ui_wrapper_fn(stdscr, monitor: WorkflowMonitor, curses_module):
    ...
    try:
        ...
        while True:
            ...
            if not hasattr(
                    monitor,
                    'last_scan_time') or current_time - monitor.last_scan_time >= 3:
                monitor.scan_output_directory()
...
def main():
    ...
    parser.add_argument(
        "--task", "-t", help="Task ID to monitor (e.g., BE-07)")
    parser.add_argument(
