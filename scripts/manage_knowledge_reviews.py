            ...
            if file.endswith(".md") or file.endswith("-summary.md"):
                ...
                try:
                    ...
                    if "**Reviewed: Not yet reviewed**" in content or "Reviewed: Not yet reviewed" in content:
                        domain = os.path.relpath(root, CONTEXT_STORE_DIR).split(os.sep)[0]
                        recommended_expert = DOMAIN_EXPERTS.get(domain, "Technical Lead")
...
def mark_as_reviewed(filepath: str, reviewer_name: str, role: Optional[str] = None) -> bool:
    ...
    try:
        ...
        print(f"✅ Marked '{filepath}' as reviewed by {reviewer_info} on {today}")
        return True
    except Exception as e:
        print(f"❌ Error marking review for {filepath}: {e}")
        return False

def log_review(filepath: str, reviewer_name: str, role: Optional[str], date: str) -> None:
    ...
    with open(REVIEWS_LOG_FILE, 'a', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=["date", "file", "reviewer", "role", "timestamp"])
        if not file_exists:
            writer.writeheader()
...
def main() -> None:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Manage knowledge summary reviews")
    subparsers = parser.add_subparsers(dest="command", help="Command to execute")
    
    # List command
    list_parser = subparsers.add_parser("list", help="List summaries needing review")
    list_parser.add_argument("--domain", help="Filter by domain (db, patterns, technical, etc.)")
    
    # Show command
    show_parser = subparsers.add_parser("show", help="Show details of a specific summary")
    show_parser.add_argument("filepath", help="Path to the summary file")
    
    # Review command
    review_parser = subparsers.add_parser("review", help="Mark a summary as reviewed")
    review_parser.add_argument("filepath", help="Path to the summary file")
    review_parser.add_argument("--reviewer", required=True, help="Name of the reviewer")
    ...
    if args.command == "list" or not args.command:
        ...
        for i, summary in enumerate(unreviewed, 1):
            print(f"{i}. {summary['path']} - Recommended reviewer: {summary['recommended_reviewer']}")
