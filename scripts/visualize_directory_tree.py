...
def count_lines_in_file(file_path: Path) -> Optional[int]:
    ...
    try:
        ...
        text_extensions = {'.py', '.md', '.txt', '.yaml', '.yml', '.json', '.js', '.ts', '.css', '.html', '.xml', '.sql', '.sh', '.bat', '.ps1'}
...
def generate_tree_structure(
    ...
    if exclude_patterns is None:
        exclude_patterns = {
            ...
            '.tox', 'dist', 'build/archives', 'runtime/cache',
            'runtime/temp', 'runtime/logs', 'logs/', 'test_logs/', '.log'
        }
    ...
    def _traverse_directory(path: Path, prefix: str = "", depth: int = 0) -> None:
        ...
        try:
            ...
            for i, item in enumerate(items):
                ...
                # Format item name with metadata
                if item.is_dir():
...
def update_directory_structure_files(
    ...
    simple_output = root_path / "docs" / "setup" / "directory-structure.md"
...
"""
    ...
    complete_output = root_path / "docs" / "setup" / "complete-directory-structure.md"
...
def main():
    ...
    tree_structure, file_count, dir_count, total_lines = generate_tree_structure(
        str(root_path),
    ...
    if args.update_docs:
        update_directory_structure_files(
            tree_structure, file_count, dir_count, total_lines, root_path, exclude_patterns
        )
    elif args.output:
        # Save to custom file
