        ...
        for status, color in {
            ...
            status_handles.append(
                plt.Line2D(
                    ...
                    markersize=15,
                    label=status))
        plt.legend(handles=status_handles,
                   title="Task Status", loc="upper right")
    else:  # color_by == "owner"
        owner_handles = []
        for owner, color in {
            "backend": "lightcoral",
            ...
            owner_handles.append(
                plt.Line2D(
                    ...
                    markersize=15,
                    label=owner))
        plt.legend(handles=owner_handles,
                   title="Task Owner", loc="upper right")
...
def analyze_graph_metrics(graph):
    ...
    max_in_degree = max(in_degrees.items(),
                        key=lambda x: x[1]) if in_degrees else ("None", 0)
    max_out_degree = max(out_degrees.items(),
                         key=lambda x: x[1]) if out_degrees else ("None", 0)
    ...
    metrics["connected_components"] = list(
...
def print_metrics(metrics):
    ...
    print(
        f"\nTask with most dependencies: {
            metrics['most_dependent_task']} ({
            metrics['most_dependent_count']} dependencies)")
    print(
        f"Task with most dependents: {
            metrics['most_dependents_task']} ({
            metrics['most_dependents_count']} dependents)")
...
def main():
    ...
    parser.add_argument(
        "--output", default="reports/task_graph.png", help="Output file path")
    parser.add_argument(
        "--color-by",
        choices=[
            "status",
            "owner"],
        default="status",
        help="Property to color nodes by")
    parser.add_argument(
        "--layout",
        choices=[
            "spring",
    ...
    # Visualize graph
    print(
        f"Visualizing graph (colored by {
