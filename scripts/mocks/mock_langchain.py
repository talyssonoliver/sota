...
def setup_langchain_mocks():
    ...
    sys.modules["langchain.vectorstores"] = types.ModuleType(
        "langchain.vectorstores")
    sys.modules["langchain.embeddings"] = types.ModuleType(
        "langchain.embeddings")
    sys.modules["langchain.chat_models"] = types.ModuleType(
        "langchain.chat_models")
    sys.modules["langchain.document_loaders"] = types.ModuleType(
        "langchain.document_loaders")
    sys.modules["langchain.text_splitter"] = types.ModuleType(
    ...
    class MockChroma:
        ...
        def as_retriever(self, search_type=None, search_kwargs=None):
            ...
            mock_retriever.get_relevant_documents.return_value = [
                type(
                    'Document', (), {
                        'page_content': 'The orders table has RLS rules that restrict access to a user\'s own orders.'})]
    ...
    class MockDirectoryLoader:
        ...
        def load(self):
            return [
                type(
                    'Document', (), {
                        'page_content': 'Test directory content'})]
